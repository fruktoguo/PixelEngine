...
| 13 Showcase Demo Game | - [!] | Showcase Demo Game 代码与自动化主链已大量落地；仍缺真实窗口完整路线、Web-first 透明 HTML UI Runtime 展示、听感、手感与人工验收证据 |
...
| 19 Unity-like Editor | - [!] | M13 顶层壳/GameObject authoring/build-player 子进程编排、默认 dock 布局、层级/Inspector/gizmo/prefab/.scene v2、玩家包解耦与 Play/Edit 机器证据已闭合；M14 Project Window 资产模型/删除确认/只读预览摘要、Scene 资产移动同步 Settings/Build、广义 Inspector 资产字段类型等自动化切片已补齐，Settings/UX Contract 仍有未完成项，M15 人工 UX 证据仍阻塞 |
| 20 Web-first 透明 HTML UI Runtime | - [!] | ManagedFallback/RmlUi 基线、GL 合成、C#↔UI、IME composition 抽象、降级与 #10 门控修订已落地；Demo HUD 输入仲裁自动化已覆盖真实 `hud.xhtml` capture/pass-through 到玩法消费者；仍阻塞于透明 UI 真实窗口产品验收、RmlUi ANGLE/GLES profile、真实平台 IME composition 事件/预编辑可视化、Ultralight 可选后端本体/许可/发行证据 |
