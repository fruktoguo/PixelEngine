...
| 19 Unity-like Editor | - [!] | M13 顶层壳/GameObject authoring/build-player 子进程编排、默认 dock 布局、层级/Inspector/gizmo/prefab/.scene v2、玩家包解耦与 Play/Edit 机器证据已闭合；M14 Project Window 资产模型/搜索过滤排序/删除确认/只读预览摘要、按钮+新路径输入的 move/rename 请求入口、Scene 资产移动同步 Settings/Build、广义 Inspector 资产字段类型、Game View viewport/input/output target 自动化切片已补齐，真实窗口拖拽移动/人工 UX 证据仍阻塞 |
