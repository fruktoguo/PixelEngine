...
| 17 | `17-roadmap-execution-order.md` | 执行顺序/依赖图/里程碑映射(M0–M14)/提交节点总表 | 全部 |
...
**17** 给出精确的依赖图与里程碑（M0–M14）映射，实际编码以 17 为节奏表。
...
| 12 编辑器 UI | - [~] | plan/12 节点 1–11 已完成；节点 12 的多级输入仲裁 / HTML UI 共存合成次序已闭合，剩余独立 `ui.upload` 真实口径待 native/Ultralight 脏矩形上传切片 |
...
| 17 路线图 | - [!] | M0-M14 总退出标准仍受外部证据阻塞 |
| 18 宿主/运行时 | - [!] | Editor 人工验收与 native leak detector 证据待补 |
| 19 独立编辑器应用 | - [!] | 顶层壳/GameObject authoring/build-player 子进程编排、主菜单/默认 dock 布局、层级完整交互、编辑器内 Build/Build-And-Run 验证、失败诊断、取消后重跑、构建 UI 响应/设置持久化、shell 依赖边界、玩家包解耦、plan/12 复用面板与 Play/Edit 短跑冒烟已落地；人工验收待补 |
| 20 交互界面/HTML UI | - [!] | ManagedFallback/RmlUi 基线、GL 合成、C#↔UI、降级与 #10 门控修订已落地；仍阻塞于 RmlUi ANGLE/GLES profile、完整 IME、Ultralight 可选后端与 native/Ultralight 脏矩形上传 |
...
| build-player 产物校验 | `tools/build-player.ps1`/`.sh`（NDJSON `schema=pixelengine.build/v1` + `build-result.json`；`audit` 分 dev-audit 结构存在性+player-only 断言 与严格 `audit-release-artifacts`） | build-player 产物校验+player-only 闭包审计已落地：拒绝 app/ 含 `PixelEngine.Editor.dll` 及编辑器专属面板闭包，允许玩家 HUD 所需 `Hexa.NET.ImGui`；仍不替代 GitHub Release / macOS 签名公证 / 完整 RID 证据 |
