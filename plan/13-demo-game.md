...
- [!] 玩家可跑 / 跳 / 蹬墙，**站在 settled 沙堆与 RigidOwned 刚体 stamp 像素上不穿不陷**：`PlayerControllerIntegrationTests` 已用 headless 真实 `Engine` / `PhysicsSystem` / `ScriptInputApi` 验证跑、跳、蹬墙、普通 settled 地面、非平整粉末堆横移与空材质 `RigidOwned` 像素承载；`playable-world` 120 帧真实窗口探针已把 `player_left_ground=True`、`player_air_control=True`、`player_ground_samples=62`、`player_air_samples=57`、`player_visual=present`、`particles=0` 纳入预检 marker，覆盖窗口链路中的跑跳、空中控制、玩家可见层和粒子残留清理；阻塞：仍缺真实设备手感评价与“掉落的木 / 金属刚体”场景级站立验收。〔§3.3、架构 §8.3〕
