...
玩家与相机
- [!] `PlayerController : Behaviour`：源码已落地，经 `ICharacterController.Create/SetPosition/MoveNow/GetState` 创建、传送并移动 AABB，实现跑 / 跳 / 空中水平控制 / 贴墙滑落 / 蹬墙、重力、coyote-time 与 jump-buffer；headless 路径已能由 Hosting 自动注入脚本后端并驱动场景 Behaviour，`AttachWindowRuntime()` 已装配 `SilkInputPhaseDriver` 将窗口键盘快照写入 `ScriptInputApi`，脚本化窗口短跑已注入 `D`/`Space` 并保持真实窗口相位链路；`playable-world` 120 帧真实窗口探针已输出 `player_left_ground=True`、`player_air_control=True`、`player_air_x_range=75.65`、`player_y_range=67.75`，证明跑跳与空中横移在窗口链路中发生；阻塞：缺少真实设备输入手感与可玩控制人工验收。〔plan/06、plan/08 输入；§3.3〕
...
- [!] 玩家可跑 / 跳 / 蹬墙，**站在 settled 沙堆与 RigidOwned 刚体 stamp 像素上不穿不陷**：`PlayerControllerIntegrationTests` 已用 headless 真实 `Engine` / `PhysicsSystem` / `ScriptInputApi` 验证跑、跳、蹬墙、普通 settled 地面、非平整粉末堆横移与空材质 `RigidOwned` 像素承载；`playable-world` 120 帧真实窗口探针已把 `player_left_ground=True`、`player_air_control=True`、`player_ground_samples=62`、`player_air_samples=57` 纳入预检 marker，覆盖窗口链路中的跑跳与空中控制；阻塞：仍缺真实设备手感评价与“掉落的木 / 金属刚体”场景级站立验收。〔§3.3、架构 §8.3〕
