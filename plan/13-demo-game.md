...
- [!] `materials.json` 的 `AudioCues` 已覆盖 impact/fire/splash/ambient/explosion/shatter，玩法脚本可经 `Audio.PlayOneShot`/`PlayAt` 请求音效；Hosting 已能从 `content/audio` 预加载 19 个 wav clip 并注入脚本上下文，`audio/cues.json` 已把材质事件 cue 句柄接到 `MaterialAudioPlayer`/已加载 clip buffer，粒子事件也接入 `Context.Events`，`World.Explode` 已可触发 explosion cue；脚本化窗口短跑已观测 `audio_loaded=19`、`audio_drained=10`，但 `audio_played=0`，只能证明事件进入音频相位并被抽取；阻塞：缺少窗口态定位音频/ambient/sizzle/corrosion 满屏限频与真实听感验收。〔plan/04、plan/10;§3.10〕
...
- [x] `PauseMenu : Behaviour.OnGui`：源码已落地，经 `IGuiContext` 绘制暂停窗口，Esc 切换暂停/继续，`Context.Runtime` 请求退出、请求打开已接入的完整 Editor dockspace、经重开关卡快照后端恢复首个脚本 tick 后的 world/script 基线，`Context.Diagnostics` 切换 dirty rect / parity / KeepAlive / temperature / owned body / particle / CCL 调试叠层。〔plan/12、架构 §17.2;§3.12〕
