...
- [ ] 六类武器：HitscanBurst（重构 `PlayableProjectileTool` 走 `DamageCircle`）、PlacedExplosive、ThrownExplosive（+`GrenadeProjectile` 脚本抛物线积分）、LaserBeam（`DamageBeam`+`AddHeat`+束 overlay）、Excavator、Constructor。〔plan/05、plan/04、plan/11；§3.14〕
- [ ] `WeaponHud : Behaviour.OnGui` + `MaterialLegendHud : Behaviour.OnGui`：经既有 `IGuiContext`（`PixelEngine.Gui` 中性 host）+ `IOverlayApi` + `MaterialSwatchProvider` 显示武器名 / 弹药 / 冷却 / 过热 / 准星与材质图例；plan/20 HTML UI 为可选升级。〔plan/11、plan/12；§3.14〕
- [ ] `MissionDirector : Behaviour`：状态机（Playing/Won/Lost）+ 目标进度（`crystalsCollected/3`）+ 时限 / 熔岩水位 + 计分（用时 + 剩余弹药 + 未受伤加成）+ 胜负判定；订阅 `MineYield` 采集事件（Core 事件总线，不进 sim 热循环）。〔plan/11、plan/02；§3.15〕
...
- [ ] 六类武器均可用真实窗口 / headless 触发并产生差异化效果：炸弹破石、激光烧穿木 / 慢烧金属、手榴弹抛物线延时爆、单点射击小当量命中、挖掘清 cell、建造放材质；破坏差异来自 `materials.json` 抗性 + `weapons.json` 参数（改数值即改表现，无写死）。〔§3.14〕
- [ ] 武器切换 / 弹药 / 冷却 / 过热 / reload 正确；所有写操作合规（单线程输入相位 + dirty + KeepAlive），不驱动额外 sim step。〔§3.14〕
- [ ] `WeaponHud` 显示武器 / 弹药 / 冷却 / 过热 / 准星，`MaterialLegendHud` 显示当前关卡 `LegendVisible=true` 材质的 swatch + `DisplayName` + 分组；均经 `IGuiContext`（`PixelEngine.Gui`）而非直接 ImGui。〔§3.14〕
