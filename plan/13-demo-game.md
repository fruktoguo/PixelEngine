...
- [ ] `content/scenes/lava-mine.scene` 扩展为熔岩矿洞逃生可玩循环场景（`LevelDirector` + `MissionDirector`/`RisingHazardDirector`/`ObjectiveCrystal`×3/`ExtractionTrigger`），经 `--scene` 或关卡选择进入；默认启动仍为 `playable-world.scene`。〔plan/07；§3.15〕
