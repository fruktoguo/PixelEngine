...
玩家与相机
- [!] `PlayerController : Behaviour`：源码已落地，经 `ICharacterController.Create/SetPosition/MoveNow/GetState` 创建、传送并移动 AABB，实现跑 / 跳 / 空中水平控制 / 贴墙滑落 / 蹬墙、重力、coyote-time 与 jump-buffer；headless 路径已能由 Hosting 自动注入脚本后端并驱动场景 Behaviour，`AttachWindowRuntime()` 已装配 `SilkInputPhaseDriver` 将窗口键盘快照写入 `ScriptInputApi`，脚本化窗口短跑已注入 `D`/`Space` 并保持真实窗口相位链路；`PlayerVisual` 已改为纯 overlay/灯光绘制玩家、准星和弹道，不再向真实粒子系统写入玩家发光标记，测试断言玩家可见层不会生成自由粒子，并对越界准星 / 弹道做 viewport 裁剪防止残留点；`playable-world` 430 帧真实窗口路线探针已输出 `player_left_ground=True`、`player_air_control=True`、`player_air_x_range=371.12`、`player_y_range=106.83`、`player_visual=present`、`player_visual_overlays=5`、`render_overlays=5`、`particles=0`，并产出 `artifacts/demo-debug-route-verified.bmp` 截图用于核对单一玩家、地形、准星与 HUD；阻塞：缺少真实设备输入手感与可玩控制人工验收。〔plan/06、plan/08 输入；§3.3〕
...
- [!] `DemoHud : Behaviour.OnGui` / `PlayableHud : Behaviour.OnGui`：源码已落地，经 `IGuiContext` 显示玩家生命、当前材质色块、笔刷半径、爆破次数、目标状态与 FPS/FrameMs/SimHz/活跃 chunk/自由粒子/刚体数；窗口 runtime 已在普通 playable 窗口中为脚本 GUI 挂载 ImGui host，不再要求 `--editor` 才能绘制脚本 HUD；Playable HUD 已直接显示 `Render FPS`、帧耗时、`Bodies`、`Particles` 与 `Collapse` 状态，避免需要滚动或猜测刷新率 / 刚体转换；脚本化窗口短跑已输出 `hud_blocked=none`、`fps=60.0`、`sim_hz=60.0`，`artifacts/demo-debug-route-verified.bmp` 已截图确认 HUD 可见 `Render FPS 60.0 16.7 ms`、`Bodies 1` 与 `Collapse 1 converted 251,213,91x64`；阻塞：缺真实窗口 HUD 像素布局与可读性验收。〔plan/11、plan/12、plan/02;§3.12〕
...
- [!] 玩家可跑 / 跳 / 蹬墙，**站在 settled 沙堆与 RigidOwned 刚体 stamp 像素上不穿不陷**：`PlayerControllerIntegrationTests` 已用 headless 真实 `Engine` / `PhysicsSystem` / `ScriptInputApi` 验证跑、跳、蹬墙、普通 settled 地面、非平整粉末堆横移与空材质 `RigidOwned` 像素承载；`playable-world` 430 帧真实窗口探针已把 `player_left_ground=True`、`player_air_control=True`、`player_ground_samples=374`、`player_air_samples=55`、`player_visual=present`、`render_overlays=8`、`particles=0` 纳入预检 marker，并截图核对未出现镜像玩家；`RenderPhaseDriverRefreshesRenderBufferAfterCpuParticleStamp` 与 RenderPipeline 中间目标逐帧清空已覆盖 render-only / 复用 render target 时清理上一帧 CPU 粒子 stamp 和 framebuffer 残留，避免暂停/跳帧时留下粒子残影或镜像旧画面；阻塞：仍缺真实设备手感评价与“掉落的木 / 金属刚体”场景级站立验收。〔§3.3、架构 §8.3〕
