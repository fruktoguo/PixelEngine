...
| 按材质抗性的区域伤害（破坏工具 / 炸弹 / 单点射击） | `IWorldEffects.DamageCircle(center,radius,damage,falloff,DamageKind)`（消费 Integrity/Durability + per-cell Damage lane + 碎屑 + MineYield 采集事件；`Explode` 内部改组合此入口） | plan/05 + plan/11 | 需引擎补 API |
| 光束逐 cell 钻孔伤害（激光枪） | `IWorldEffects.DamageBeam(x,y,dx,dy,length,damagePerCell,heatPerCell,DamageKind)`（沿束逐 cell 伤害 + 注热，按 Hardness 穿透） | plan/05 + plan/11 | 需引擎补 API |
...
- **PlacedExplosive（炸弹）**：大半径 `DamageCircle(damage,radius,falloff,DamageKind.Blast)` + 邻近刚体径向冲量，大 damage 可破 stone、抛大量碎屑；弹药 / 长冷却 / 屏震。
...
- [!] `ExplosiveTool : Behaviour`：源码已落地，中键按鼠标世界坐标调用公开 `Context.World.Explode`，并请求 fog reveal 与点光反馈；Scripting/Simulation/Physics 测试已覆盖爆炸复合入口在安全相位触发 cell 抛射、explosion 音频事件、`RigidOwned` damage 通知与邻近刚体径向冲量；`PlayerControllerIntegrationTests.ExplosiveToolMiddleClickPushesNearbyRigidBody` 已用真实 Demo `ExplosiveTool`、Hosting 物理相位和动态 stone 刚体验证中键爆破会把左侧邻近刚体推出并产生正向线速度；脚本化窗口短跑已在真实窗口链路中触发 1 次爆破、自由粒子与点光；阻塞：缺少真实中键设备操作、画面反馈、音频定位与刚体推动人工验收。〔plan/05、plan/06、plan/10;§3.5〕
- [ ] `ExplosiveTool` / 破坏工具改走 `IWorldEffects.DamageCircle`（抗性感知区域伤害），`World.Explode` 内部收敛为 `DamageCircle` + 邻近刚体冲量 + 粒子组合（对外签名保留）；写 cell 前查 `CellFlags.RigidOwned`，命中经 `IRigidDamageSink.OnOwnedCellDamaged` 触发重建，绝不累加刚体像素 Damage（守 #5）；破坏后标 dirty + 跨界 KeepAlive（守 #1/#2/#3/#6）。〔plan/05、plan/06、plan/03；§3.5〕
- [ ] Explode 语义迁移登记：既有基于「无条件抛射半径内全部 cell」的抛射断言测试（如 `ExplosiveToolMiddleClickEjectsCellsAndQueuesLighting`、`PlayableProjectile*` 系列）随破坏驱动新语义迁移为「按材质抗性破坏 + 碎屑」断言。〔plan/14；§3.5〕
