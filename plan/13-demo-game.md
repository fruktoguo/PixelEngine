...
- [!] 火花 / 血 / 碎屑发射：`PlayerHealth` 已用 `Particles.Burst` 喷血，`MaterialEmitter` 已用 `Particles.Spawn` 做喷口粒子，`SparkEmitter` 已接入 lava 区域 fire 火花，刚体小碎片可由 Physics damage 重建写入自由粒子；`World.Explode` 已可把 cell 抛射为粒子并推动邻近刚体；可玩破坏弹已移除持久 fire 弹道粒子，改由 overlay 弹道显示，避免窗口上方残留未知点；`PlayableProjectileLeftClickQueuesAudioAndParticles` 与 `PlayableProjectileAutoFiresWhileLeftMouseIsHeld` 已验证左键射击和按住左键自动连续开火，修复只响应第一帧点击边沿导致只能打一发的问题；`PlayableProjectileConvertsDetachedSolidIslandIntoRigidBody`、`PlayableProjectileConvertsTopBorderDetachedIslandIntoRigidBody`、`PlayableProjectileContinuesCollapseScanAfterFirstConvertedIsland` 与 `PlayableProjectileConvertsImpactFracturedShelfIntoRigidBody` 已验证爆破后局部悬空 solid 岛会经公开 `IRigidBodyApi.CreateFromRegion` 转成刚体、扫描框上边界接触不会再误跳过上方悬空石块、首个碎块转换后会分帧继续扫描后续悬空块，且弹坑上沿局部悬空石块不再长期静态浮空；`ScriptedPlayableDemoFirstLeftClickTargetsCollapseTerrain` 已锁定普通 scripted demo 第一枪命中右侧坍塌目标而非起点刷子输入，普通 playable 窗口短跑已改为按住左键并输出 `playable_shots=3`、`playable_collapsed_islands=30`、`active_bodies=14`；`PlayableWorldDirectorDisablesDebugOverlaysOnSceneStart` 已验证真实可玩场景启动时显式关闭 dirty rect / 粒子轨迹等调试叠层，避免诊断点污染画面；本轮 AI 地图窗口短跑 `artifacts/demo-fixed-720x480-window.bmp` 已核对破坏弹不再产生上方持久未知点或镜像玩家，并输出 `particles=73`、`player_air_control=True`；`lava-mine-particle-light-probe.scene` 已在真实窗口相位中输出 `particle_light_probe_spawned=96`、`particle_light_probe_max_active=96`、`particle_light_probe_tail_max=0`、`particle_light_probe_last_active=0` 与 `particle_light_probe_depleted=True`，证明短寿命火花粒子可进入系统并按 lifetime 退场；阻塞：缺少真实窗口爆炸粒子视觉、刚体推动画面与长跑无粒子泄漏视觉/玩法验收。〔plan/05;§3.8〕
- [x] 本轮可玩性回归修复：`PlayableProjectileTool` 默认只转换小型、明确脱离的 solid 岛，粉末 / 液体不再参与坍塌候选，受支撑主地形与弹坑边缘兜底刚体化默认关闭；`PlayableProjectileDoesNotConvertSupportedTerrainSlabByDefault` 已锁定一枪不会把仍连接主地形的大块转成刚体导致玩家穿模；`PlayerVisual` 默认揭示整个当前视口，避免测试 Demo 被圆形 fog 范围遮住；`CameraFollow` 在整数 zoom 下对齐整数 cell origin，使 720×480 / 2x 默认视图进入渲染快速路径；窗口短跑 `artifacts/demo-playability-fix-window.bmp` 输出 `playable_shots=4`、`playable_collapsed_islands=0`、`active_bodies=0`、`player_left_ground=True`、`player_air_control=True`、`render_camera=(412.00,66.00,0.500,720x480)` 与 `fps=50.0`，截图未见持久黄线、圆形遮罩或大块刚体化。〔§3.4、§3.7、§3.8、§3.9〕
