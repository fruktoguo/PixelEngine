...
- [!] 火花 / 血 / 碎屑发射：`PlayerHealth` 已用 `Particles.Burst` 喷血，`MaterialEmitter` 已用 `Particles.Spawn` 做喷口粒子，`SparkEmitter` 已接入 lava 区域 fire 火花，刚体小碎片可由 Physics damage 重建写入自由粒子；`World.Explode` 已可把 cell 抛射为粒子并推动邻近刚体；可玩破坏弹已移除持久 fire 弹道粒子，改由 overlay 弹道显示，避免窗口上方残留未知点；`PlayableProjectileConvertsDetachedSolidIslandIntoRigidBody`、`PlayableProjectileConvertsTopBorderDetachedIslandIntoRigidBody`、`PlayableProjectileContinuesCollapseScanAfterFirstConvertedIsland` 与 `PlayableProjectileConvertsImpactFracturedShelfIntoRigidBody` 已验证爆破后局部悬空 solid 岛会经公开 `IRigidBodyApi.CreateFromRegion` 转成刚体、扫描框上边界接触不会再误跳过上方悬空石块、首个碎块转换后会分帧继续扫描后续悬空块，且弹坑上沿局部悬空石块不再长期静态浮空；`ScriptedPlayableDemoFirstLeftClickTargetsCollapseTerrain` 已锁定普通 scripted demo 第一枪命中右侧坍塌目标而非起点刷子输入；`PlayableWorldDirectorDisablesDebugOverlaysOnSceneStart` 已验证真实可玩场景启动时显式关闭 dirty rect / 粒子轨迹等调试叠层，避免诊断点污染画面；本轮 AI 地图窗口短跑 `artifacts/demo-ai-map-refresh-tuned.bmp` 已核对破坏弹不再产生上方持久未知点或镜像玩家，并输出 `playable_collapsed_islands=10`、`active_bodies=6`、`particles=73`、`player_air_control=True`；`lava-mine-particle-light-probe.scene` 已在真实窗口相位中输出 `particle_light_probe_spawned=96`、`particle_light_probe_max_active=96`、`particle_light_probe_tail_max=0`、`particle_light_probe_last_active=0` 与 `particle_light_probe_depleted=True`，证明短寿命火花粒子可进入系统并按 lifetime 退场；阻塞：缺少真实窗口爆炸粒子视觉、刚体推动画面与长跑无粒子泄漏视觉/玩法验收。〔plan/05;§3.8〕
