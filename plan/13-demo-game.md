...
**本轮修订（M13 纯玩家运行时）**：`Program.cs` 移除 `--editor` in-demo 内嵌编辑器启动分支与相关编辑器装配；Demo 收敛为纯玩家运行时，GUI 一律经 `PixelEngine.Gui` 中性 host 承载（`DemoHud`/`PauseMenu` 不再经 `PixelEngine.Editor`）。原先依赖 Demo `--editor` 启动的编辑器窗口证据入口——plan/18 §5 的 editor-window scripted probe、`tools/demo-manual-acceptance-preflight.ps1` 的 `hudMenuEditorVideo` scope、`PerformanceHardeningToolingDisciplineTests` 中断言 `editor_enabled`/`editor_running`/`editor_panels`/`editor_bridge_frames` 的锁定测试——整体**等价迁移**到 `apps/PixelEngine.Editor.Shell`（plan/19）的编辑器进程证据链，Demo 侧不再保留任何编辑器探针（迁移登记见 §4）。默认启动场景仍为 `playable-world.scene`（Procedural 生成器键 `PlayableWorldDirector`）；『熔岩矿洞逃生』为本轮**新增**的可玩循环内容（§3.15），经 `--scene content/scenes/lava-mine.scene` 或关卡选择进入，**非默认场景**。
...
| 富速度分布粒子发射（枪口火花 / 碎屑锥） | `IParticleSpawner.Emit(origin,count,velCone,materialId,life)` | plan/05 + plan/11 | 已登记并由 plan/05/11 接纳；当前公开 API 已有 `Spawn(in ParticleSpawnDesc)` 与 `Burst(...)`，富速度分布 `Emit` 仍是后续扩展缺口，不以假实现填充 |
...
### 3.14 数据驱动武器 / 道具库（WeaponCatalog / WeaponController / 六类武器）
...
`WeaponKind` 枚举与六类作用方式（全走 Simulation 公开编辑 API + 温度场 + 粒子；写操作**只在单线程输入相位**、写后标 dirty + KeepAlive，**不驱动额外 sim step** 守架构 §4.4）：

- **HitscanBurst（单点射击，保留）**：重构现有 `PlayableProjectileTool` 走 `IWorldEffects.DamageCircle` 小当量命中——Raycast 命中点小半径低伤，消费 Hardness/Damage lane，弹道 overlay，无限弹药 / 短冷却。
- **PlacedExplosive（炸弹）**：大半径 `DamageCircle(damage,radius,falloff,DamageKind.Impact)` + 邻近刚体径向冲量，大 damage 可破 stone、抛大量碎屑；弹药 / 长冷却 / 屏震。
- **ThrownExplosive（手榴弹）**：`GrenadeProjectile : Behaviour` 脚本实体在相位 [1] game logic 按 `dt` 积分抛物线（重力 + `bounce`，`ISolidSampler.Raycast` 检碰撞，不驱动额外 sim step），`fuseSeconds` 到期触发 `DamageCircle`；弹药 / 冷却 / 右键蓄力抛速。
- **LaserBeam（激光枪）**：每 tick `IWorldEffects.DamageBeam(x,y,dx,dy,length,beamDps*dt,heatPerCell,DamageKind.Thermal)` 沿束逐 cell 伤害 + 注热，按 Hardness 穿透（木 / 冰快、metal 慢 + 熔点相变成 molten_metal），emissive 束 overlay；连续开火 / 过热冷却（heat 累积）。
- **Excavator（挖掘）**：`Cells.Paint(empty)` 圆形擦除（保留 `MaterialBrush` 擦除路径）；采矿走 `DamageCircle` 触发 `MineYield` 采集事件；无弹药 / 半径可调。
- **Constructor（建造）**：`Cells.Paint(selectedMaterial)` 放置当前建材（stone/wood/water/sand…），受 CA 接管；建材子选择键切换。

`WeaponController : Behaviour` 管理已装备武器、切换（数字键 `1`–`6` 直选 + 滚轮循环）、弹药 / 冷却 / 过热状态、`reload`（R），按当前 `WeaponKind` 分派到对应效果例程；左键开火 / 按住连发（沿用 `PlayableProjectile` 按住边沿逻辑）、右键副武器动作（手榴弹蓄力 / 建造放置 / 激光持续）。爆炸 / 破坏经 `DamageCircle` + `IParticleSpawner` 碎屑 + `IRigidBodyApi.ApplyImpulse` 对邻近刚体施冲量，全部经既有延迟命令队列落到单线程输入 / 安全相位。

武器 HUD `WeaponHud : Behaviour.OnGui`（或并入 `PlayableHud`）经既有 `IGuiContext`（与 `DemoHud`/`PauseMenu` 同路径、由 `PixelEngine.Gui` 中性 host 承载）显示当前武器名 + `hudColor` 图标、弹药 X/Max、冷却 / 过热条、准星（`IOverlayApi` 屏幕空间）。`MaterialLegendHud : Behaviour.OnGui` 经 `IGuiContext` + `MaterialSwatchProvider` 展示当前关卡 / 玩家周围 `LegendVisible=true` 材质的 swatch + `DisplayName` + `LegendCategory` 分组，让玩家一眼识别各物质（与 plan/12 编辑器材质面板 / 调试叠层是两套 UI，互不复用输入路径）。两者均走既有 `IGuiContext` + `IOverlayApi` 保「只用公开 API」；**升级到需求 5 的 HTML UI（plan/20 `PixelEngine.UI`）为可选路线，非本文件强依赖**。

### 3.15 熔岩矿洞逃生可玩循环（MissionDirector / ObjectiveCrystal×3 / RisingHazardDirector / ExtractionTrigger）

把材质 / 反应 / 刚体 / 粒子 / 光照 / 音频 + 武器串成有目标感、胜负明确的可玩循环「熔岩矿洞逃生」——本轮**新增内容**，非默认场景（默认仍为 `playable-world.scene`）；经 `--scene content/scenes/lava-mine.scene` 或关卡选择进入，`LevelDirector`（§3.11）在此基础上装配任务组件：
...
- [x] 默认 2x 可玩相机的 render buffer 快速路径补强：`RenderBufferBuilder` 在 palette zoom fast path 内对同一 worker range 的重复屏幕行直接复制上一行 `scene/emissive/occluder`，避免 720×480 / 2x 相机移动时对同一 world row 重复访问 chunk；`BuildPaletteZoomFastPathCopiesRepeatedScreenRows` 已用计数型 `IChunkSource` 锁定 2x 缩放只采样真实 world row；真实窗口短跑与脚本化预检要求输出 `render_camera=(...,720x480)`、`fps=`、`frame_p99_ms=`、`playable_shots=`、`active_bodies=`、`player_left_ground=True`，截图未见持久黄线、圆形遮罩、残影或大块刚体化；`dotnet test tests\PixelEngine.Rendering.Tests\PixelEngine.Rendering.Tests.csproj -c Release --no-restore` 通过 168 项，Hosting 渲染相位/线程纪律筛选测试通过 33 项。〔plan/08、plan/16；§3.4、§3.12〕
...
- [x] `content/weapons.json` + `WeaponCatalog`：经引擎公开 Content/Config API（`Engine.LoadConfig<WeaponCatalog>(..., JsonTypeInfo)`）加载，**Demo 不直接 `System.Text.Json` 解析**；`WeaponKind` / `WeaponFalloff` 枚举与全字段（damage/radius/falloff/impulse/fuse/throwSpeed/gravity/bounce/cooldown/ammoMax/reload/heatPerCell/beamDps/spawnMaterial/spread/recoil/screenShake/tracerDuration/cue/hudColor）已落地，`DemoStartupOptionsTests.WeaponCatalogLoadsThroughEngineConfigApi` 覆盖真实 `content/weapons.json` 六类武器顺序、laser 热/光束字段与 builder 材质引用。〔plan/04、plan/00 §4；§3.14〕
- [x] `WeaponController : Behaviour`：已接入脚本 `Context.Config` 读取 `weapons.json`，支持切换（`1`–`6` + 滚轮）/ 弹药 / 冷却 / 过热锁存 / `R` reload / 左右键分派；默认 `PlayableWorldDirector` 由 `WeaponController` 接管输入，SingleShot 复用 `PlayableProjectileTool.FireOnceFromCurrentInput()` 保留弹道、音效与坍塌扫描后端，其它类型经公开 `IWorldEffects`/`Cells.Paint` 分派，写操作仍在脚本输入相位进入延迟命令队列，不驱动额外 sim step；`PlayerControllerIntegrationTests.WeaponControllerSwitchesAndRoutesHitscanThroughProjectileBackend` 与 `WeaponControllerReloadsAndLocksOutOverheatedWeapon` 覆盖真实目录切换/冷却/后端路由、临时目录换弹/过热锁定。〔plan/11；§3.14〕
- [x] 六类武器：HitscanBurst 已由 `WeaponController` 复用 `PlayableProjectileTool` 后端并保持 `DamageCircle`/坍塌扫描，PlacedExplosive 走 `World.Explode`，ThrownExplosive 已新增 `GrenadeProjectile` 脚本延时/反弹/爆炸，LaserBeam 走 `DamageBeam` + 新增公开 `IWorldEffects.AddHeat` + 束 overlay，Excavator/Constructor 走 `Cells.Paint` 清空/放置材质；`WeaponControllerDispatchesSixWeaponKindsIntoDistinctEffects` 覆盖六类武器 headless 触发与 cell/温度/粒子/实体差异化效果。〔plan/05、plan/04、plan/11；§3.14〕
...
- [x] `MissionDirector : Behaviour`：状态机（Playing/Won/Lost）+ 目标进度（`crystalsCollected/3`）+ 时限 / 熔岩水位 + 计分（用时 + 剩余弹药 + 未受伤加成）+ 胜负判定；订阅 `MineYield` 采集事件（Core 事件总线，不进 sim 热循环）。`MissionDirectorCollectsMineYieldAndScoresExtraction`、`MissionDirectorLosesWhenTimeLimitExpires` 与 `WeaponControllerExcavatorPublishesMineYieldEvent` 已覆盖 Core 事件订阅、时限判负、胜利计分和 Excavator 真实清除 mineYield 材质后发布采矿事件。〔plan/11、plan/02；§3.15〕
- [x] `RisingHazardDirector : Behaviour`（经 `MaterialEmitter`，数据驱动上涨速率）+ `ObjectiveCrystal×3`（crystal 矿脉布置，需不同武器打通）+ `ExtractionTrigger`（`GoalTrigger` 升级为「集齐 3 晶 + 进入 + 限时才开门」）。已新增 `RisingHazardDirector` 通过一排 `MaterialEmitter` 数据驱动注入上升熔岩并同步 `MissionDirector` 水位，`ObjectiveCrystal` 可铺设 crystal 矿簇并在 cell 被破坏后发布 `MineYieldEvent`，`ExtractionTrigger` 在任务集齐水晶且未失败时才调用撤离胜利；`ObjectiveCrystalPublishesMineYieldWhenCellsAreDestroyed`、`ExtractionTriggerRequiresCrystalsBeforeWinningMission` 与 `RisingHazardDirectorDrivesEmittersAndMissionLoss` 已覆盖。正式 `crystal` 内容包字段与 `lava-mine.scene` 接入按后续相邻条目落地。〔plan/11；§3.15〕
...
- [x] `content/scenes/lava-mine.scene` 扩展为熔岩矿洞逃生可玩循环场景（`LevelDirector` + `MissionDirector`/`RisingHazardDirector`/`ObjectiveCrystal`×3/`ExtractionTrigger`），经 `--scene` 或关卡选择进入；默认启动仍为 `playable-world.scene`。场景已关闭旧 `GoalTrigger`，由 `ExtractionTrigger` 执行集齐 3 晶 + 进入 + 限时通关；`LevelDirector` 已给玩家挂载 `WeaponController` 并禁用旧 projectile 直接输入；`LavaMineSceneRegistersDestructibleWoodAndMetalStructures` 与 `DemoLavaMineSceneFileUsesLevelDirectorBehaviour` 已验证正式场景物化任务循环组件、3 个 ObjectiveCrystal、RisingHazard MaterialEmitter 与旧 GoalTrigger 关闭。〔plan/07；§3.15〕
...
- [!] HUD 正确显示当前材质 / 笔刷 / 玩家状态 / 操作提示 / 性能行；暂停菜单可继续 / 重开 / 切叠层 / 退出；HUD 经 `IGuiContext` 而非直接 ImGui：`PlayerControllerIntegrationTests.DemoHudAndPauseMenuDrawExpectedGuiThroughScriptContext` 已通过 `IScriptRuntime.DrawGui(IGuiContext)` + recording GUI 验证 `DemoHud` 输出生命值、材质色块 / 名称 / 半径、目标状态、爆破次数、FPS / frame ms / chunk / particle / body 性能行，`PauseMenu` 经 Escape 暂停并绘制继续 / 重开 / 退出按钮、默认折叠的调试叠层入口，并断言不再出现 `打开 Editor` 按钮，点击继续后恢复 Play；`EngineOverloadControllerTests.ScriptDiagnosticsApiCapturesWindowedFrameRateStatistics` 已验证平均 FPS、平均帧耗、last、p99、1% low、jitter 和样本数来自多帧窗口统计；`PlayerControllerIntegrationTests.PauseMenuButtonsDriveRestartAndDiagnosticsFacades` 已验证暂停菜单按钮经公开 Runtime/Diagnostics facade 触发重开关卡与 dirty-rect 调试叠层切换；`PlayerControllerIntegrationTests.GoalTriggerMarksReachedAndQueuesCelebrationFeedback` 已验证通关菜单和退出按钮；`SceneAndHeadlessTests.RuntimeRestartRestoresCapturedWorldAndScriptBaseline` 已验证重开恢复 world/script 基线；`LavaMineSceneTests` 已验证默认关卡挂载 `DemoHud` 与 `PauseMenu`；脚本化窗口短跑已在真实窗口链路中证明 HUD 组件已绑定，`EngineCounters.RenderFramesPerSecond/RenderFrameMilliseconds` 已把窗口墙钟帧间隔发布到脚本诊断，`artifacts/demo-fix-check/playable-demo-current.bmp` 截图核对 HUD 可见 `Render FPS 69.2 avg`、`1% low 40.2`、`p99 24.9 ms`、`jitter 2.9`、帧耗时波动条、`Sim 30Hz`、`Bodies 0`、`Particles 73` 与 `Collapse 0 scan_border_Left, Right, Bottom` 状态；阻塞：仍缺真实窗口中 GUI 布局、鼠标点击、EditorShell dockspace 打开、重开点击链路与退出请求的人工验收。〔§3.12〕
...
- [ ] 熔岩矿洞逃生循环：从出生点用至少两种解法（引水成石桥 / 炸塌木桥垫脚 / 激光开金属道）采集 3 晶抵达充能出口通关，全程贯穿材质 / 反应 / 刚体 / 粒子 / 光照 / 音频 + 武器；熔岩上涨 / 玩家死亡 / 通路淹没触发失败并可重开；采集进度、时限压力、武器权衡与计分正确。〔§3.15〕
