...
- [ ] `content/weapons.json` + `WeaponCatalog`：经引擎公开 Content/Config API（`Content.LoadConfig<WeaponCatalog>`）加载，**Demo 不直接 `System.Text.Json`**；`WeaponKind` 枚举与全字段（damage/radius/falloff/impulse/fuse/throwSpeed/gravity/bounce/cooldown/ammoMax/reload/heatPerCell/beamDps/spawnMaterial/spread/recoil/screenShake/tracerDuration/cue/hudColor）。〔plan/04、plan/00 §4；§3.14〕
