...
- [!] 火花 / 血 / 碎屑发射：`PlayerHealth` 已用 `Particles.Burst` 喷血，`MaterialEmitter` 已用 `Particles.Spawn` 做喷口粒子，`SparkEmitter` 已接入 lava 区域 fire 火花，刚体小碎片可由 Physics damage 重建写入自由粒子；`World.Explode` 已可把 cell 抛射为粒子并推动邻近刚体；可玩破坏弹已移除持久 fire 弹道粒子，改由 overlay 弹道显示，避免窗口上方残留未知点；`PlayableProjectileConvertsDetachedSolidIslandIntoRigidBody`、`PlayableProjectileConvertsTopBorderDetachedIslandIntoRigidBody`、`PlayableProjectileContinuesCollapseScanAfterFirstConvertedIsland` 与 `PlayableProjectileConvertsImpactFracturedShelfIntoRigidBody` 已验证爆破后局部悬空 solid 岛会经公开 `IRigidBodyApi.CreateFromRegion` 转成刚体、扫描框上边界接触不会再误跳过上方悬空石块、首个碎块转换后会分帧继续扫描后续悬空块，且弹坑上沿局部悬空石块不再长期静态浮空；可玩破坏弹命中时还会立即按弹坑附近局部固体包围盒排队 `Bodies.CreateFromRegion`，避免连续山体边缘被击穿后短期静态浮空；`ScriptedPlayableDemoFirstLeftClickTargetsCollapseTerrain` 已锁定普通 scripted demo 第一枪命中右侧坍塌目标而非起点刷子输入；`PlayableWorldDirectorDisablesDebugOverlaysOnSceneStart` 已验证真实可玩场景启动时显式关闭 dirty rect / 粒子轨迹等调试叠层，避免诊断点污染画面；`artifacts/demo-regression/fixed-playable-demo.bmp` 与脚本化窗口摘要已核对破坏弹不再产生上方持久未知点，短跑尾帧 `particles=0`，并输出 `playable_collapsed_islands=1`、`active_bodies=1`、`max_active_bodies=1`；`lava-mine-particle-light-probe.scene` 已在真实窗口相位中输出 `particle_light_probe_spawned=96`、`particle_light_probe_max_active=96`、`particle_light_probe_tail_max=0`、`particle_light_probe_last_active=0` 与 `particle_light_probe_depleted=True`，证明短寿命火花粒子可进入系统并按 lifetime 退场；阻塞：缺少真实窗口爆炸粒子视觉、刚体推动画面与长跑无粒子泄漏视觉/玩法验收。〔plan/05;§3.8〕
...
- [!] 玩家挖断木栈桥 → 连通块掉落为 Box2D 刚体：`LavaMineSceneTests.LavaMineSceneRegistersDestructibleWoodAndMetalStructures` 已验证默认关卡经公开脚本 API 注册 6 个 wood/metal 动态刚体、Physics stamp 产生 owned cell，手动竖切后 1 个刚体销毁并拆成 2 个新刚体；`MaterialBrushCanDigRigidBridgeThroughPublicInput` 已验证玩家侧公开鼠标输入经 `MaterialBrush` 写入 cell 后会清除 `RigidOwned` ownership，并通过 Hosting 共享 damage queue 触发刚体销毁、新刚体创建和 active body 增长；`PlayableProjectileConvertsDetachedSolidIslandIntoRigidBody`、`PlayableProjectileConvertsTopBorderDetachedIslandIntoRigidBody`、`PlayableProjectileContinuesCollapseScanAfterFirstConvertedIsland` 与 `PlayableProjectileConvertsImpactFracturedShelfIntoRigidBody` 已覆盖破坏弹导致的脱离岛、扫描框上沿岛、多岛分帧扫描以及弹坑上沿局部悬空石块刚体化；普通 playable scripted 短跑已在 `artifacts/demo-regression/fixed-playable-demo.bmp` 对应摘要中观测 `playable_collapsed_islands=1`、`active_bodies=1`、`max_active_bodies=1` 与 HUD `Bodies 1`，证明真实窗口破坏弹会把右侧山体局部块转换为刚体；脚本化窗口路线短跑已在真实窗口链路中观测 `max_physics_destroyed=2`、`max_physics_created=2`；阻塞：仍缺真实窗口中推动/被砸、继续挖 / 烧 / 酸蚀破碎，以及 metal 近熔岩熔化导致结构坍塌的窗口玩法验收。〔§3.7、架构 §8.3〕
