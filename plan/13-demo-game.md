...
- [!] `DemoHud : Behaviour.OnGui` / `PlayableHud : Behaviour.OnGui`：源码已落地，经 `IGuiContext` 显示玩家生命、当前材质色块、笔刷半径、爆破次数、目标状态与 FPS/FrameMs/SimHz/活跃 chunk/自由粒子/刚体数；窗口 runtime 已在普通 playable 窗口中为脚本 GUI 挂载 ImGui host，不再要求 `--editor` 才能绘制脚本 HUD；Playable HUD 已直接显示 `Render FPS`、帧耗时、`Bodies`、`Particles` 与 `Collapse` 状态，避免需要滚动或猜测刷新率 / 刚体转换；`EngineCounters.RenderFramesPerSecond/RenderFrameMilliseconds` 已发布真实窗口墙钟帧率而非固定 dt 估算，`artifacts/demo-fix-check/playable-demo-render-fastpath.bmp` 已截图确认 HUD 可见 `Render FPS 74.2 13.5 ms`、`Sim 30Hz`、`Bodies 0`、`Particles 73` 与 `Collapse 0 too_few_pixels ...` 状态，画面按 720x480 内部画布等比铺满 1080x720 framebuffer，未再出现角落小画面；阻塞：缺真实窗口 HUD 像素布局与可读性验收。〔plan/11、plan/12、plan/02;§3.12〕
