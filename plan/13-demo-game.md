...
玩家与相机
- [!] `PlayerController : Behaviour`：源码已落地，经 `ICharacterController.Create/SetPosition/MoveNow/GetState` 创建、传送并移动 AABB，实现跑 / 跳 / 空中水平控制 / 贴墙滑落 / 蹬墙、重力、coyote-time 与 jump-buffer；headless 路径已能由 Hosting 自动注入脚本后端并驱动场景 Behaviour，`AttachWindowRuntime()` 已装配 `SilkInputPhaseDriver` 将窗口键盘快照写入 `ScriptInputApi`，脚本化窗口短跑已注入 `D`/`Space` 并保持真实窗口相位链路；`PlayerVisual` 已改为纯 overlay/灯光绘制玩家、准星和弹道，不再向真实粒子系统写入玩家发光标记，测试断言玩家可见层不会生成自由粒子；`playable-world` 430 帧真实窗口探针已输出 `player_left_ground=True`、`player_air_control=True`、`player_air_x_range=171.89`、`player_y_range=68.00`、`player_visual=present`、`player_visual_overlays=8`、`render_overlays=8`、`particles=0`，并产出 `artifacts/debug-user-demo-route-430-fixed.bmp` 截图用于核对单一玩家、地形、准星与 HUD；阻塞：缺少真实设备输入手感与可玩控制人工验收。〔plan/06、plan/08 输入；§3.3〕
