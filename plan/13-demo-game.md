...
- [ ] `RisingHazardDirector : Behaviour`（经 `MaterialEmitter`，数据驱动上涨速率）+ `ObjectiveCrystal×3`（crystal 矿脉布置，需不同武器打通）+ `ExtractionTrigger`（`GoalTrigger` 升级为「集齐 3 晶 + 进入 + 限时才开门」）。〔plan/11；§3.15〕
