...
玩家与相机
- [!] `PlayerController : Behaviour`：源码已落地，经 `ICharacterController.Create/SetPosition/Move/GetState` 创建、传送并移动 AABB，实现跑 / 跳 / 贴墙滑落 / 蹬墙、重力、coyote-time 与 jump-buffer；headless 路径已能由 Hosting 自动注入脚本后端并驱动场景 Behaviour，`AttachWindowRuntime()` 已装配 `SilkInputPhaseDriver` 将窗口键盘快照写入 `ScriptInputApi`，脚本化窗口短跑已注入 `D`/`Space` 并保持真实窗口相位链路；阻塞：缺少真实设备输入手感与可玩控制人工验收。〔plan/06、plan/08 输入；§3.3〕
...
- [!] 火花 / 血 / 碎屑发射：`PlayerHealth` 已用 `Particles.Burst` 喷血，`MaterialEmitter` 已用 `Particles.Spawn` 做喷口粒子，`SparkEmitter` 已接入 lava 区域 fire 火花，刚体小碎片可由 Physics damage 重建写入自由粒子；`World.Explode` 已可把 cell 抛射为粒子并推动邻近刚体；脚本化窗口短跑已观测 `max_particles=61`；阻塞：缺少真实窗口爆炸粒子视觉、刚体推动画面与长跑无粒子泄漏端到端验收。〔plan/05;§3.8〕
- [!] emissive 材质标注正确（lava/molten_metal/fire/火花），Scripting 已有 `Lighting.RevealAround` + `AddPointLight` 请求 API，Hosting 已有 `ScriptLightingSynchronizer` 将脚本请求同步为 Rendering `LightSource` 与 `FogOfWarBuffer`，`RenderPhaseDriver` 已把 fog-of-war 与点光源传入 `RenderPipeline` 并 stamp 自由粒子到 emissive buffer；`RenderPipeline` 已把点光源合成进 visibility mask；脚本化窗口短跑已观测 `max_lights=4`；阻塞：缺少真实窗口光照、fog 与 bloom 视觉验收。〔plan/08;§3.9〕
- [!] `materials.json` 的 `AudioCues` 已覆盖 impact/fire/splash/ambient/explosion/shatter，玩法脚本可经 `Audio.PlayOneShot`/`PlayAt` 请求音效；Hosting 已能从 `content/audio` 预加载 19 个 wav clip 并注入脚本上下文，`audio/cues.json` 已把材质事件 cue 句柄接到 `MaterialAudioPlayer`/已加载 clip buffer，粒子事件也接入 `Context.Events`，`World.Explode` 已可触发 explosion cue；脚本化窗口短跑已观测 `audio_loaded=19`、`max_audio_drained=20`，但 `max_audio_played=0`，只能证明事件进入音频相位并被抽取；阻塞：缺少窗口态定位音频/ambient/sizzle/corrosion 满屏限频与真实听感验收。〔plan/04、plan/10;§3.10〕
...
- [x] `content/scenes/lava-mine.scene`：已按 `.scene` 文档格式序列化 `LevelDirector` 入口与关卡参数，默认启动路径可加载并物化为等价脚本场景；`content/scenes/lava-mine-goal-probe.scene` 已作为窗口通关触发探针场景，专门验证真实窗口相位中的 `GoalTrigger` 链路；`content/scenes/lava-mine-health-probe.scene` 已作为窗口生命伤害探针场景，专门验证 `PlayerHealth` 伤害与反馈链路；两者均不替代完整路线验收。〔plan/12、plan/07;§3.2、§3.11〕
- [!] `DemoHud : Behaviour.OnGui`：源码已落地，经 `IGuiContext` 显示玩家生命、当前材质色块、笔刷半径、爆破次数、目标状态与 FPS/SimHz/Frame/活跃 chunk/自由粒子/刚体数；脚本化窗口短跑已输出 `hud_blocked=none`，证明 HUD 绑定到玩家脚本组件；阻塞：缺真实窗口 HUD 像素布局与可读性验收。〔plan/11、plan/12、plan/02;§3.12〕
- [x] `PauseMenu : Behaviour.OnGui`：源码已落地，经 `IGuiContext` 绘制暂停窗口，Esc 切换暂停/继续，`Context.Runtime` 请求退出、请求打开已接入的完整 Editor dockspace、经重开关卡快照后端恢复首个脚本 tick 后的 world/script 基线，`Context.Diagnostics` 切换 dirty rect / parity / KeepAlive / temperature / owned body / particle / CCL 调试叠层；脚本化窗口短跑已输出 `pause_open=True`，证明真实窗口链路中 Escape 可打开暂停菜单状态。〔plan/12、架构 §17.2;§3.12〕
...
- [!] HUD 正确显示当前材质 / 笔刷 / 玩家状态 / 操作提示 / 性能行；暂停菜单可继续 / 重开 / 切叠层 / 开编辑器 / 退出；HUD 经 `IGuiContext` 而非直接 ImGui：`PlayerControllerIntegrationTests.DemoHudAndPauseMenuDrawExpectedGuiThroughScriptContext` 已通过 `IScriptRuntime.DrawGui(IGuiContext)` + recording GUI 验证 `DemoHud` 输出生命值、材质色块 / 名称 / 半径、目标状态、爆破次数、FPS / chunk / particle / body 性能行，`PauseMenu` 经 Escape 暂停并绘制继续 / 打开 Editor / 重开 / 退出按钮、调试叠层 checkbox，点击继续后恢复 Play；`PlayerControllerIntegrationTests.GoalTriggerMarksReachedAndQueuesCelebrationFeedback` 已验证通关菜单和退出按钮；`SceneAndHeadlessTests.RuntimeRestartRestoresCapturedWorldAndScriptBaseline` 已验证重开恢复 world/script 基线；`LavaMineSceneTests` 已验证默认关卡挂载 `DemoHud` 与 `PauseMenu`；脚本化窗口短跑已在真实窗口链路中证明 HUD 组件已绑定且 Escape 打开暂停状态；阻塞：仍缺真实窗口中 GUI 布局、鼠标点击、Editor dockspace 打开、重开点击链路与退出请求的人工验收。〔§3.12〕
