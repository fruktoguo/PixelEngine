...
| 按材质抗性的区域伤害（破坏工具 / 炸弹 / 单点射击） | `IWorldEffects.DamageCircle(center,radius,damage,falloff,DamageKind)`（消费 Integrity/Durability + per-cell Damage lane；`Explode` 内部改组合此入口） | plan/05 + plan/11 | 已接入 Scripting cell 安全相位；MineYield 采集事件与更丰富碎屑数仍属后续武器/采集条目 |
| 光束逐 cell 钻孔伤害（激光枪） | `IWorldEffects.DamageBeam(x,y,dx,dy,length,damagePerCell,DamageKind)`（沿束逐 cell 伤害，按 Hardness 穿透） | plan/05 + plan/11 | 已接入 Scripting cell 安全相位；注热 `AddHeat` 仍属后续 API 缺口 |
| 脚本注热（激光 / 燃烧武器） | `IWorldEffects.AddHeat(x,y,radius,deltaC)`（映射 `TemperatureField.AddHeat`） | plan/04 + plan/11 | 需引擎补 API |
| 富速度分布粒子发射（枪口火花 / 碎屑锥） | `IParticleSpawner.Emit(origin,count,velCone,materialId,life)` | plan/05 + plan/11 | 需引擎补 API |
| 材质可玩性信息门面（武器 / 图例 / 危险采样） | `MaterialInfo` 扩展：`DisplayName/BaseColorBgra/CellType/Category/Emissive/Hardness/MaxIntegrity/IsDestructible/FlowRate` | plan/04 + plan/11 | 需引擎补 API |
...
- [ ] §3.13 增 5 条 API 缺口并上报接纳：`DamageCircle`/`DamageBeam`/`AddHeat`/`Emit` + `MaterialInfo` 可玩性扩展。〔AGENTS.md §0；§3.13、§3.14〕
