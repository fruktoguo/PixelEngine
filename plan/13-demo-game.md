...
- [ ] `Program.cs` 移除 `--editor` in-demo 内嵌编辑器启动分支与编辑器装配；Demo 收敛为纯玩家运行时，GUI 改经 `PixelEngine.Gui` 中性 host 承载 `DemoHud`/`PauseMenu`（不经 `PixelEngine.Editor`）。〔plan/12 GUI 中性化重构、plan/19；§3.1、§2〕
- [ ] 依赖修正：`PixelEngine.Hosting` 删除对 `PixelEngine.Editor` 的 `ProjectReference`，Demo 侧 `IGuiContext` host 由 `PixelEngine.Gui` 提供；CI 依赖方向断言更新为 Demo 与 Hosting 均不引用 Editor（玩家运行时闭包不含 `PixelEngine.Editor.dll`）。〔plan/14、plan/12、plan/15；§2〕
- [ ] 编辑器窗口证据迁移登记：plan/18 §5 editor-window scripted probe、`tools/demo-manual-acceptance-preflight.ps1` 的 `hudMenuEditorVideo` scope、`PerformanceHardeningToolingDisciplineTests` 断言 `editor_enabled`/`editor_running`/`editor_panels`/`editor_bridge_frames` 的锁定测试，等价迁移到 `apps/PixelEngine.Editor.Shell`（plan/19）编辑器进程证据链，Demo 侧编辑器探针退场。〔plan/18、plan/19；§3.1〕
...
- [ ] Demo 为纯玩家运行时：进程无 `--editor` 路径、玩家运行时闭包不含 `PixelEngine.Editor.dll`，`DemoHud`/`PauseMenu` 经 `PixelEngine.Gui` 中性 host 绘制；CI 依赖方向断言 Demo 与 Hosting 均不引用 Editor。〔§2、§3.1〕
