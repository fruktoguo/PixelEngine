...
- [x] `RigidOwnedDamageRoutingTests`（Physics.Tests / Simulation.Tests）：`CellFlags.RigidOwned` 像素破坏经 `IRigidDamageSink.OnOwnedCellDamaged` 路由触发形状重建、绝不在刚体像素累加 Damage（守 #5，联动 §4.3 拆分守恒）。证据：`RigidOwnedDamageRoutingTests` 覆盖 `DamageCircle`/`DamageBeam` 命中 RigidOwned 只入 `RigidDamageQueue`、不累加 Damage、不改材质；`PhysicsSyncTests` 覆盖 `PhysicsSystem.SyncStep` 排空队列并进入 `PendingDamage`，`PhysicsCouplingAcceptanceTests.ReactionsDamageRigidOwnedCells` 覆盖反应路径同样路由。
