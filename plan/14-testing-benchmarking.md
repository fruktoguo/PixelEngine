...
- [x] `BackendConformanceTests`：补齐 `IGameUiBackend` headless 共享契约类，覆盖 Initialize/Resize/LoadDocument/SetScreenStack 生命周期、HitTest→`UiInputCapture` 世界输入仲裁语义、FeedText 与 FeedTextComposition 分离、CopyModelPaths 缓冲不足稳定截断、DrainEvents destination capacity / ring overflow 边界；该切片不声明三后端真实一致性、RmlUi ANGLE/GLES、UltralightBackend 或真实平台 IME 完成。
