...
- [x] `EditorConsoleStoreTests` / `DemoRuntimeScriptingTests` / `HostingProjectDisciplineTests`（Hosting.Tests / 壳测试）：Console 产品面自动化切片闭合，覆盖统一 `EditorConsoleStore` / `IEditorConsoleSink` / `EditorConsoleFilter` 的 category / severity / source / text / timestamp 过滤、capacity eviction、timestamp+sequence 稳定排序、newest-first 排序、build-player preflight / NDJSON / stderr / result / warning sink、asset opener 结果归类、UI backend fallback 归类、Hosting-owned `ScriptRuntime` 经 `IScriptHotReloadDiagnosticSink` 汇入 Console，以及 watcher 热重载编译失败不静默。证据：`SnapshotFiltersByCategorySeveritySourceTextAndTimestamp`、`SnapshotAppliesCapacityEvictionAndStableTimestampOrdering`、`ConsoleSinkHelpersClassifyBuildAssetUiAndScriptDiagnostics`、`BuildSettingsPanelSinksPreflightProgressAndResultIntoConsole`、`AttachScriptingFromServicesReportsWatchedHotReloadCompileDiagnosticsToConsoleSink`、`EditorShellConsumesHostingOwnedScriptingRuntimeAndConsoleDiagnostics`、`EditorShellDeclaresBuildSettingsPanelAndPlayerBuildService`。真实窗口 Console 可读性与人工 UX 仍归 plan/19 M15 `[!]`。
...
- [x] standalone-editor 测试全绿：`.scene` v2（ParentId/Transform/Vector2）往返逐字段等价、v1 兼容升级、authoring→运行时物化（世界 TRS 烘焙 + 扁平 DOD）、prefab 嵌套 / override 传播、Project Window stable asset id / manifest / logical path / asset type 自动化切片、资产拖拽语义自动化切片、Console 产品面自动化切片、shell `--window-ticks` 证据等价迁移（plan/19/18）。证据：2026-07-07 本地 Release 验证 `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorAssetDropServiceTests|FullyQualifiedName~EditorProjectAssetModelTests"` 通过 10/10，`dotnet test tests/PixelEngine.Editor.Tests/PixelEngine.Editor.Tests.csproj -c Release --filter FullyQualifiedName~AssetBrowserPanelTests` 通过 3/3，`dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorProjectAssetModelTests|FullyQualifiedName~EditorShellSceneMaterializationTests"` 通过 7/7；Console 证据为 `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorConsoleStoreTests|FullyQualifiedName~AttachScriptingFromServicesReportsWatchedHotReloadCompileDiagnosticsToConsoleSink|FullyQualifiedName~EditorShellConsumesHostingOwnedScriptingRuntimeAndConsoleDiagnostics|FullyQualifiedName~EditorShellDeclaresBuildSettingsPanelAndPlayerBuildService"` 通过；既有 2026-07-06 `PixelEngine.Hosting.Tests` 全项目验证覆盖 scene document、EditorShellSceneMaterialization、prefab 与 shell evidence preflight 相关测试。
