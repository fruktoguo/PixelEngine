...
**BuildProfileDto 校验**：`Normalize()` 断言唯一启动场景、启动∈入包、至少一入包、场景路径逃逸拒绝、路径 / 产物名非空、非法组合被拒并给红字提示；`EngineProjectSettingsStore` 断言 `ProjectSettingsDto` / `PlayerSettingsDto` / `BuildProfileDto` 默认值、schema version、JSON 往返和相对路径校验；`PreflightAsync` 缺 .NET SDK / pwsh 时给明确可执行诊断（绝不静默）。
...
- [x] `BuildProfileDtoValidationTests`（壳构建测试）：`Normalize()` 唯一启动场景 / 启动∈入包 / 至少一入包 / 场景相对路径 / 产物名非空 / 非法组合被拒；`EngineProjectSettingsStore` 覆盖 Project / Player / Build settings 默认值、schema version、JSON 往返与路径逃逸拒绝；`PreflightAsync` 缺 SDK/pwsh 诊断。证据：`EditorShellBuildTests.BuildProfileDtoValidationAndPreflightReportActionableErrors`、`EditorShellBuildTests.EngineProjectSettingsStoreRoundTripsHostingDtosAndRejectsPathEscape`。
...
- [x] in-editor-build 测试全绿：`PlayerBuildService` NDJSON 解析 / `build-result` 合成 / 取消杀子树无残留、`BuildProfileDto` 校验、Hosting settings DTO 读写/校验、`Preflight` 诊断；audit「player 包 `app/` 无 `PixelEngine.Editor.dll` 与编辑器专属 ImGuizmo/ImPlot」断言（允许玩家 HUD 的 `Hexa.NET.ImGui`）。证据：`dotnet test tests\PixelEngine.Hosting.Tests\PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorShellBuildTests|FullyQualifiedName~HostingProjectDisciplineTests"` 通过 32/32。
...
- plan 19 → §4.9 standalone-editor（`.scene` v1/v2 往返、物化、prefab、shell `--window-ticks` 证据）+ §4.10 in-editor-build（`PlayerBuildService` NDJSON / 取消、`BuildProfileDto` 校验、Hosting settings DTO JSON 往返、player-only audit）。
