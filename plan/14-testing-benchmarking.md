...
- [x] `GameDirectorOutcomeTests`（Demo.Tests）：`ObjectiveCrystal×3` / `crystalsCollected/3` + 抵达出口 → Won、死亡 / 熔岩淹没 → Lost、分数计算、`RisingLavaHazard` 上涨速率数据驱动（plan/13 §3.17 功能完整 showcase / 熔岩矿洞逃生 evidence）。`GameDirectorOutcomeTests` 覆盖 3 水晶撤离胜利、确定性分数、玩家死亡失败与上涨速率/水位同步；`RisingHazardDirectorDrivesEmittersAndMissionLoss` 覆盖熔岩淹没判负。
