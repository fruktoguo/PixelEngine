...
- [x] `BackendConformanceTests`：补齐 `IGameUiBackend` headless 共享契约类，覆盖 Initialize/Resize/LoadDocument/SetScreenStack 生命周期、ManagedFallback 非交互透明区 pass-through / 交互控件 capture、HitTest→`UiInputCapture` 世界输入仲裁语义、FeedText 与 FeedTextComposition 分离、CopyModelPaths 缓冲不足稳定截断、DrainEvents destination capacity / ring overflow 边界与临时 XHTML 清理；该切片不声明三后端真实一致性、RmlUi ANGLE/GLES、UltralightBackend 或真实平台 IME 完成。
...
- [x] standalone-editor 测试全绿：`.scene` v2（ParentId/Transform/Vector2）往返逐字段等价、v1 兼容升级、authoring→运行时物化（世界 TRS 烘焙 + 扁平 DOD）、prefab 嵌套 / override 传播、Project Window stable asset id / manifest / logical path / asset type 自动化切片、资产拖拽语义自动化切片、Console 产品面自动化切片、默认工作台 scripted route 自动化切片、shell `--window-ticks` 证据等价迁移（plan/19/18）。证据：2026-07-07 本地 Release 验证 `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorAssetDropServiceTests|FullyQualifiedName~EditorProjectAssetModelTests"` 通过 10/10，`dotnet test tests/PixelEngine.Editor.Tests/PixelEngine.Editor.Tests.csproj -c Release --filter FullyQualifiedName~AssetBrowserPanelTests` 通过 3/3，`dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorProjectAssetModelTests|FullyQualifiedName~EditorShellSceneMaterializationTests"` 通过 7/7；Console / watcher / 默认工作台回归证据为 `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~DemoRuntimeScriptingTests|FullyQualifiedName~EditorShellProjectTests|FullyQualifiedName~EditorConsoleStoreTests|FullyQualifiedName~HostingProjectDisciplineTests"` 通过 45/45，覆盖 watcher start failure 降级、Console 诊断与致命装配错误不吞，并锁定 `--scripted-default-workbench-probe` 的新建工程、核心面板、GameObject、脚本源、保存、Play/Exit 与 Build Settings 入口；`$env:PIXELENGINE_RENDERING_GL_SMOKE = '1'; dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~OpenAllowsMissingScriptSourceDirectoryAndReportsWatcherStartFailedToConsole"` 通过 1/1，单独覆盖 `EditorProjectSession.Open` 在 GL smoke 环境下对缺失 `ScriptSourceDir` 的非阻断行为；既有 2026-07-06 `PixelEngine.Hosting.Tests` 全项目验证覆盖 scene document、EditorShellSceneMaterialization、prefab 与 shell evidence preflight 相关测试。
