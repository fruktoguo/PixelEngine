...
- [x] html-ui 测试全绿（headless 部分）：布局 / data-model 绑定 / 三级输入仲裁 / IME composition 能力诊断边界 / 脏矩形合并正确、UI 逻辑相位零分配基准；GL 上传 / 光栅化不在自动化断言内，RmlUi/Ultralight native 专属上传与真实平台 IME composition 事件仍归 plan/20 后续切片 / 阻塞。证据：2026-07-07 本地 Release 验证 `dotnet test tests/PixelEngine.UI.Tests/PixelEngine.UI.Tests.csproj -c Release --filter "FullyQualifiedName~UiInputRouterTests" --no-restore` 通过 11/11，Hosting IME capability source contract 与 discipline 定向验证通过；`GameUiAllocationBenchmarks` ShortRun 4 项 `Allocated == 0 B`。
