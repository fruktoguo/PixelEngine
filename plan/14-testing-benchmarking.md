...
- [ ] `DeterministicSimFixture` + 规范化快照序列化器（material 用稳定 name，剔除瞬时位）+ golden 文件目录 `__golden__/`（被测子系统：CA 内核 §6.2/§11.3）。
...
- [ ] `ReactionConservationTests`：双输出 / 定向反应在 chunk 边界产物计数严格守恒、不翻倍 / 不丢失；覆盖同 chunk / 水平 / 垂直 / 四角边界、p=255 与 0<p<255、§7.4 datamined 例子账本（CA 内核 §7.4、不变式 #4）。
...
- [ ] `HotReloadTests`：改源 → Roslyn 重编译 → 卸旧 ALC / 加新 ALC，新行为生效、旧实例替换、世界状态一致（脚本 §17.2）。
- [ ] `ScriptExceptionIsolationTests`：脚本回调抛异常不崩主循环、被捕获上报诊断、其余子系统继续（脚本，`AGENTS.md §4`）。
- [ ] `AlcCollectibilityTests`：多轮 load→unload 后旧 ALC 的 `WeakReference` 经 GC 不再存活、托管堆稳态不增长（脚本 §17.4，无内存泄漏）。
...
- [ ] `HotReloadTests`、`ScriptExceptionIsolationTests`、`AlcCollectibilityTests` 全绿：热重载行为正确、异常隔离不崩、ALC 经 GC 可回收且无泄漏（架构 §17.2/§17.4）。
...
- [ ] `test(script): 热重载/异常隔离/ALC 可回收测试`（对应 §4.5）。
