...
- [x] `AssetBrowserPanelTests` / `EditorAssetDropServiceTests`（Editor.Tests / Hosting.Tests / 壳测试）：Project Window typed drag payload 与 Shell 资产拖拽语义自动化闭合，覆盖 stable asset id + logical path + asset type payload、无 stable id 资产拒绝拖拽、prefab→Hierarchy 可撤销实例化并保留 parent、prefab→Scene View 按 world transform 放置、非 prefab 资产 drop 到 Hierarchy/Scene View 返回诊断且无副作用、Inspector typed asset field 编码 prefab/scene/material/script/texture/audio stable reference、script→组件列表解析 Behaviour 并 Undo、资产 move/rename 重写活动 authoring 模型与 `.scene/.prefab` 文档中的 Inspector asset reference。证据：`AssetBrowserPanelCreatesTypedDragPayloadOnlyForStableAssets` 与 `EditorAssetDropServiceTests`。
