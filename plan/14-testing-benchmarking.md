...
- [~] `CellDamageRubbleHandshakeTests`（Simulation.Tests）：已新增内核 destruction sink 覆盖破坏转 `RubbleTarget`/Empty + Damage 清 0 + parity + dirty + 跨界 KeepAlive + `DebrisCount`/`MineYield` 事件载荷，且验证 RigidOwned/未破坏/Indestructible 不发布普通 cell destruction 事件；`dotnet test tests/PixelEngine.Simulation.Tests/PixelEngine.Simulation.Tests.csproj -c Release` 167/167 通过。待 plan/05 `RequestDebris` 接入 ParticleSystem 后再把“碎屑抛射请求消费”闭合为 [x]。
