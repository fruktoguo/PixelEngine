...
- [x] standalone-editor 测试全绿：`.scene` v2（ParentId/Transform/Vector2）往返逐字段等价、v1 兼容升级、authoring→运行时物化（世界 TRS 烘焙 + 扁平 DOD）、prefab 嵌套 / override 传播、Project Window stable asset id / manifest / logical path / asset type 自动化切片、shell `--window-ticks` 证据等价迁移（plan/19/18）。证据：2026-07-07 本地 Release 验证 `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorProjectAssetModelTests|FullyQualifiedName~EditorShellSceneMaterializationTests"` 通过 7/7；既有 2026-07-06 `PixelEngine.Hosting.Tests` 全项目验证覆盖 scene document、EditorShellSceneMaterialization、prefab 与 shell evidence preflight 相关测试。
...
- plan 19 → §4.9 standalone-editor（`.scene` v1/v2 往返、物化、prefab、Project Window stable asset id/manifest/logical path/type 自动化切片、shell `--window-ticks` 证据）+ §4.10 in-editor-build/settings（`PlayerBuildService` NDJSON / 取消、`BuildProfileDto` 校验、Project/Player/Build settings DTO JSON 往返、ProjectSettings → EditorProject/EngineProject、Project/Player Settings panel scripted probe、player-only audit）。
