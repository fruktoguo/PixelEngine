...
- [x] `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorConsoleStoreTests|FullyQualifiedName~AttachScriptingFromServicesReportsWatchedHotReloadCompileDiagnosticsToConsoleSink|FullyQualifiedName~EditorShellConsumesHostingOwnedScriptingRuntimeAndConsoleDiagnostics|FullyQualifiedName~EditorShellDeclaresBuildSettingsPanelAndPlayerBuildService"` 覆盖 Console store/filter/capacity/sort、build-player/preflight/result sink、asset opener / UI backend fallback / script diagnostics 归类、Hosting-owned runtime 诊断 sink 装配边界，以及 watcher 热重载编译失败不静默，当前通过。
