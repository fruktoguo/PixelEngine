...
- [x] **Game View 自动化契约底座**：已新增 Shell 层 `EditorGameViewContract` / `GameViewPanel`，默认 dock 与 Window 菜单将 Scene View、Game View 分成两个独立面板；Scene View 明确保持 authoring camera、gizmo/画刷/点选等工具输入且不冒充玩家视角，Scene View 与 Game View 均消费 `RenderPipeline.CurrentViewportTexture` 的当前自动化底座事实，Game View 在 Play 模式声明 Game UI → gameplay 输入让位策略，并锁定 `UiPresentLayerOrders.Game` 在 editor overlay 之前；Scene View 聚焦时强制 authoring 工具捕获，Game View 未聚焦时不向 gameplay 透传，避免非目标面板输入同帧进入世界。`EditorShellGameViewContractTests` 覆盖 Scene/Game View 标题、相机 ownership、Play 输入策略、runtime viewport texture、overlay order、Game View 未聚焦阻断与 Scene View 输入阻断。
- [ ] **Game View 产品剩余项**：Play 模式 Game View 内嵌 / 裁剪 Web-first UI、真实窗口点击链路、手感与 M15 人工 UX 证据仍未闭合，继续归 §5 `[!]` 与后续真实窗口验收。
...
- [x] 本轮闭合节点：Game View 自动化契约底座已落地，`EditorGameViewContract` 明确 Scene View / Game View 的相机 ownership、Play 输入策略、runtime viewport texture 和 game/editor overlay 顺序，`GameViewPanel` 纳入默认 dock 与 Window 菜单；Play 模式 Game View 内嵌 / 裁剪 Web-first UI、真实窗口点击链路、完整人工走查与 UX 材料仍归 M15 `[!]`。
