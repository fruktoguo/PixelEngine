...
- [x] **M14 Unity-like UX Contract Settings 同源切片已产品化（自动化部分）**：Project / Player / Build Settings 均绑定 plan/18 的 Hosting DTO/store；Project / Player Settings 支持读写、校验错误提示与 scripted probe，Project Settings 已驱动 EditorProject/EngineProject 入口，Player Settings 已同源投影到 headless/runtime options、build-player 参数、`build-result.json` 与 packaged `startup.json`。Project Window 资产语义、Console/Game View 产品面与真实 UX 证据仍保留为后续条目。
...
- [x] `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorAssetDropServiceTests|FullyQualifiedName~EditorProjectAssetModelTests"` 覆盖 Project Window stable asset id/manifest/logical path/type、常见资产创建、Project Window 数据源消费、prefab move/rename 引用保持、Hierarchy / Scene View / Inspector typed asset field / script component list 资产拖拽语义、无效 drop 诊断与资产移动后 Inspector asset reference 重写，当前通过 10/10。
