...
- [x] **M14 Unity-like UX Contract Settings 同源切片已产品化（自动化部分）**：Project / Player / Build Settings 均绑定 plan/18 的 Hosting DTO/store；Project / Player Settings 支持读写、校验错误提示与 scripted probe，Project Settings 已驱动 EditorProject/EngineProject 入口，Player Settings 已同源投影到 headless/runtime options、build-player 参数、`build-result.json` 与 packaged `startup.json`。Project Window 资产语义、资产拖拽语义、脚本外部编辑器、Console 与 Game View 自动化契约底座已闭合；真实窗口人工 UX 证据仍保留为 M15 `[!]`。
