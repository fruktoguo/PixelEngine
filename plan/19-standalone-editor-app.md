...
- [x] `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorConsoleStoreTests|FullyQualifiedName~EditorConsoleScriptRuntimeReportsWatchedHotReloadCompileDiagnostics|FullyQualifiedName~EditorShellDeclaresBuildSettingsPanelAndPlayerBuildService"` 覆盖 Console store/filter/capacity/sort、build-player/preflight/result sink、asset opener / UI backend fallback / script diagnostics 归类，以及 watcher 热重载编译失败不静默，当前通过 6/6。
...
- [x] 本轮闭合节点：Console 产品面自动化切片已落地，统一 Console store/sink/filter model 汇总 build-player NDJSON/stderr/result、脚本热重载诊断、UI backend fallback、asset opener 与 project/build path 错误；`EditorConsolePanel` 提供 category / min severity / source / text 筛选、newest-first 排序与清空入口；真实窗口 Console 可读性、布局与人工 UX 仍归 M15 `[!]`。
