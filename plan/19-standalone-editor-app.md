...
- [x] **Project Window 删除确认 + 引用安全预检（自动化切片）**：Project Window 删除请求必须携带 stable asset id / logical path / asset type，Shell 数据源会回查 manifest 防止旧项或类型错配；删除前扫描活动 authoring scene 与 `.scene/.prefab` 文档中的 prefab link / typed asset reference，存在引用时拒绝删除并返回引用位置，无引用资产需二次确认后才删除磁盘文件与 manifest 记录。证据：`EditorProjectAssetModelTests.DeleteAssetPreflightBlocksReferencedAssetsAndReportsLocations`、`EditorProjectAssetModelTests.DeleteAssetRequiresConfirmationAndRemovesUnreferencedManifestRecord`、`AssetBrowserPanelTests.AssetBrowserPanelDeletesOnlyStableAssetsAfterConfirmation`。
...
- [x] `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorProjectAssetModelTests|FullyQualifiedName~EditorAssetDropServiceTests"` 覆盖 Project Window 删除确认、引用安全预检、活动 scene / `.scene` 文档引用阻塞、未引用资产确认后删除 manifest 与磁盘文件，以及既有资产拖拽引用重写回归，2026-07-07 本地通过 12/12；`dotnet test tests/PixelEngine.Editor.Tests/PixelEngine.Editor.Tests.csproj -c Release --filter "FullyQualifiedName~AssetBrowserPanelTests"` 覆盖 Project Window 面板 stable id 删除请求、二次确认回调与旧数据源项拒绝删除，2026-07-07 本地通过 5/5。
...
- [x] 本轮闭合节点：Project Window stable asset id/manifest、logical path、asset type、Project Window 数据源消费、prefab 移动/重命名引用保持、删除确认与引用安全预检自动化切片已落地；真实窗口拖拽、真实窗口删除确认 UX、脚本外部编辑器人工打开与人工 UX 证据仍保留为后续节点。
