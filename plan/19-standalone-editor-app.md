...
- [x] **广义 Inspector 资产字段类型（自动化切片）**：脚本字段可通过 `[AssetField(ScriptAssetKind.*)]` 显式声明 Texture / Audio / Prefab / Scene / Script / Material typed asset reference，Inspector 字段描述携带 `ScriptFieldKind.AssetReference` 与目标资产类型，GameObject Inspector 可从字段描述生成 drop target 并写入 stable asset id + logical path + asset type；强类型 `ScriptAssetReference` 已接入 EditorShell runtime projection 与 `.scene` loader 字段绑定。证据：`ScriptInspectorTests.InspectFieldsClassifiesTypedAssetReferencesAndSetsCompatibleValues`、`EditorAssetDropServiceTests.InspectorPanelAppliesDescriptorDeclaredTypedAssetFieldDrop`、`EditorShellSceneMaterializationTests.RuntimeProjectionBakesHierarchyAndBindsStableIdsVector2AndMaterialId`。
...
- [x] **Game View viewport/input 自动化契约底座**：已新增 Shell 层 `EditorGameViewContract` / `GameViewPanel` / `GameViewViewportSnapshot`，默认 dock 与 Window 菜单将 Scene View、Game View 分成两个独立面板；Scene View 明确保持 authoring camera、gizmo/画刷/点选等工具输入且不冒充玩家视角，Scene View 与 Game View 均消费 `RenderPipeline.CurrentViewportTexture` 的当前自动化底座事实；Game View 记录 image rect、texture size、fit scale、visible viewport rect 与 panel-local → viewport texture 坐标映射，Play 模式声明 Game UI → gameplay 输入让位策略、UI clip rect、Web-first UI hit-test 坐标来源与 editor overlay 优先级，并将 `InputFocused` / `InputHovered` 从整窗口收紧到实际图像区域。`EditorShellGameViewContractTests` 覆盖 Scene/Game View 标题、相机 ownership、Play 输入策略、runtime viewport texture、overlay order、Game View 未聚焦阻断、Scene View 输入阻断、面板空白区阻断、图像内透明区 pass-through 与交互 UI capture 契约入口。
- [ ] **Game View 产品剩余项**：Play 模式 Game View 内嵌 / 裁剪 Web-first UI 的真实合成路径、真实窗口点击链路、手感与 M15 人工 UX 证据仍未闭合；本轮只闭合 viewport geometry + input mapping 自动化底座，继续归 §5 `[!]` 与后续真实窗口验收。
- [x] **默认工作台体验（自动化路线底座）**：已新增 `--scripted-default-workbench-probe`，在无 `--project` 时自动新建临时工程，显示默认核心面板，创建 GameObject，生成默认脚本源文件，保存场景，进入/退出 Play，并打开 Build Settings/准备 build 输出目录，输出 `editor_default_workbench_probe` 结构化摘要；该切片只闭合 headless/scripted 连续路线底座，不冒充真实窗口人工 UX 或真实玩家包构建完成。
- [ ] **默认工作台产品剩余项**：真实窗口首次启动手感、脚本热重载后可直接挂载 Behaviour、完整 Build/Build And Run 玩家包产出、截图/视频与人工 UX 复核仍未闭合。
...
- [!] Project Window 引用稳定性：需要人工或端到端证据证明资产移动/重命名/删除确认后 Prefab、Scene、Inspector 字段与 Build 入包不丢引用；本轮删除安全预检与引用阻塞单元测试只能作为辅助，不能替代真实窗口/端到端证据。
...
- [x] `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorAssetDropServiceTests|FullyQualifiedName~EditorProjectAssetModelTests|FullyQualifiedName~EditorScriptAssetOpenServiceTests"` 覆盖 Project Window stable asset id/manifest/logical path/type、常见资产创建、Project Window 数据源消费与只读预览摘要、prefab move/rename 引用保持、Hierarchy / Scene View / Inspector typed asset field / script component list 资产拖拽语义、无效 drop 诊断、资产移动后 Inspector asset reference 重写，以及脚本外部编辑器 configured/fallback/失败/拒绝/路径安全自动化切片；预览摘要新增定向验证见下条。
- [x] `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --no-restore --filter "FullyQualifiedName~EditorProjectAssetModelTests"` 覆盖 Project Window stable asset id、manifest、logical path、prefab move/rename 引用保持、删除预检 / 二次确认，以及纹理、音频、材质 JSON、场景、Prefab、脚本的只读预览摘要，2026-07-07 本地通过 6/6；`dotnet test tests/PixelEngine.Editor.Tests/PixelEngine.Editor.Tests.csproj -c Release --no-restore --filter "FullyQualifiedName~AssetBrowserPanelTests"` 覆盖 Project Window 面板筛选、选择、音频试听、typed drag payload、脚本外部打开、stable id 删除请求、二次确认回调、旧数据源项拒绝删除，以及同路径资产刷新为新 id 后旧删除确认失效，2026-07-07 本地通过 6/6。
- [x] `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~DemoRuntimeScriptingTests|FullyQualifiedName~EditorShellProjectTests|FullyQualifiedName~EditorConsoleStoreTests|FullyQualifiedName~HostingProjectDisciplineTests"` 覆盖 Console store/filter/capacity/sort、build-player/preflight/result sink、asset opener / UI backend fallback / script diagnostics 归类、Hosting-owned runtime 诊断 sink 装配边界、watcher 热重载编译失败不静默、合法但不存在的 `ScriptSourceDir` 降级为 `WatcherStartFailed` 且不阻塞 `AttachScriptingFromServices`、真正致命热重载装配错误仍抛出；本轮同一过滤器新增覆盖 `--scripted-default-workbench-probe` 参数解析与默认工作台 scripted route 源码契约（新建工程、核心面板、GameObject、脚本源、保存、Play/Exit、Build Settings 入口），2026-07-07 本地通过 45/45；其中 `EditorProjectSession.Open` 真实窗口依赖路径需配合单测环境变量单独复验。
...
- [x] `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorShellGameViewContractTests|FullyQualifiedName~InputArbitratorTests"` 覆盖 Scene View / Game View 独立产品契约、Play 模式 Game UI → gameplay 输入让位、Game View 未聚焦阻断、Scene View authoring 输入阻断、editor overlay 捕获优先、Game View image rect / fit scale / panel-local→viewport 映射、面板空白区阻断、图像内透明区 pass-through、交互 UI capture 与输入仲裁回归，2026-07-07 本地通过 15/15；`dotnet test tests/PixelEngine.Editor.Tests/PixelEngine.Editor.Tests.csproj -c Release --filter "FullyQualifiedName~EditorAppTests"` 覆盖默认 dock/window title 回归，2026-07-07 本地通过 10/10。
...
- [x] 本轮闭合节点：Project Window stable asset id/manifest、logical path、asset type、Project Window 数据源消费、只读预览摘要、prefab 移动/重命名引用保持、删除确认与引用安全预检自动化切片已落地；删除二次确认已绑定原始 stable asset id / logical path / asset type，同路径资产刷新为新 id 后旧确认失效；真实窗口拖拽、真实窗口删除确认 UX、脚本外部编辑器人工打开、真实窗口预览布局与人工 UX 证据仍保留为后续节点。
...
- [x] 本轮闭合节点：Game View viewport/input 自动化契约底座已落地，`EditorGameViewContract` 明确 Scene View / Game View 的相机 ownership、Play 输入策略、runtime viewport texture、UI clip rect、viewport coordinate space、Web-first UI hit-test 坐标来源和 game/editor overlay 顺序，`GameViewPanel` 记录 `GameViewViewportSnapshot` 并将输入焦点/hover 收紧到实际图像区域；Play 模式 Game View 内嵌 / 裁剪 Web-first UI 的真实合成路径、真实窗口点击链路、完整人工走查与 UX 材料仍归 M15 `[!]`。
- [x] 本轮闭合节点：默认工作台 scripted route 底座已落地，`--scripted-default-workbench-probe` 串联无工程首次启动自动新建工程、默认核心面板显示、GameObject 创建、默认脚本源生成、场景保存、Play/Exit 与 Build Settings 入口，并输出 `editor_default_workbench_probe`；真实窗口首次启动手感、脚本热重载后直接挂载 Behaviour、完整 Build/Build And Run 玩家包产出与人工 UX 证据仍归后续节点。
