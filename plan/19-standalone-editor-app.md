...
- [x] **M14 Unity-like UX Contract Settings 同源切片已产品化（自动化部分）**：Project / Player / Build Settings 均绑定 plan/18 的 Hosting DTO/store；Project / Player Settings 支持读写、校验错误提示与 scripted probe，Project Settings 已驱动 EditorProject/EngineProject 入口，Player Settings 已同源投影到 headless/runtime options、build-player 参数、`build-result.json` 与 packaged `startup.json`。Project Window 资产语义、删除确认/引用安全预检、资产拖拽语义、脚本外部编辑器、Console 与 Game View 自动化契约底座已闭合；真实窗口人工 UX 证据仍保留为 M15 `[!]`。
...
- [ ] **Project Window 产品剩余项**：预览打磨、真实窗口删除确认 UX、广义 Inspector 资产字段类型、真实窗口拖拽 / 脚本外部编辑器人工打开证据仍未闭合；不得把本自动化切片视为 M15 人工 UX 证据。
...
- [x] `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorProjectAssetModelTests|FullyQualifiedName~EditorAssetDropServiceTests"` 覆盖 Project Window 删除确认、引用安全预检、活动 scene / `.scene` 文档引用阻塞、未引用资产确认后删除 manifest 与磁盘文件，以及既有资产拖拽引用重写回归，2026-07-07 本地通过 12/12；`dotnet test tests/PixelEngine.Editor.Tests/PixelEngine.Editor.Tests.csproj -c Release --filter "FullyQualifiedName~AssetBrowserPanelTests"` 覆盖 Project Window 面板 stable id 删除请求、二次确认回调、旧数据源项拒绝删除，以及同路径资产刷新为新 id 后旧删除确认失效，2026-07-07 本地通过 6/6。
...
- [x] 本轮闭合节点：Project Window stable asset id/manifest、logical path、asset type、Project Window 数据源消费、prefab 移动/重命名引用保持、删除确认与引用安全预检自动化切片已落地；删除二次确认已绑定原始 stable asset id / logical path / asset type，同路径资产刷新为新 id 后旧确认失效；真实窗口拖拽、真实窗口删除确认 UX、脚本外部编辑器人工打开与人工 UX 证据仍保留为后续节点。
