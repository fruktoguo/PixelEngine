...
- [!] 阻塞：当前 bloom/light/air compute pass 与 `RenderPipeline` 资源契约均绑定 OpenGL texture/image/SSBO 句柄；`ComputeSharpBackend` 仍是隔离 stub，Rendering 项目默认不引用 `ComputeSharp`，仅在 `EnableComputeSharpBackend=true` 时条件引用包并编译 `ComputeSharpSupport` 的 DX12 device 探测分支。ComputeSharp/DX12 不能直接消费现有 GL texture handle；`docs/rendering-computesharp-resource-contract.md` 已记录可接受路线（D3D 渲染后端或显式 GL-DX12 shared resource/fence 层）与禁止路线（把 GL texture name 解释为 DX12 resource）。代码层面已新增 `GpuResourceContractKind` 与 `ComputeSharpResourceContract`：当前 `GpuComputeResources` 显式声明 `OpenGlTextureNames` 且 `CanBeConsumedByComputeSharp=false`，ComputeSharp 合法契约必须是 `D3D12RenderGraph` 或 `GlDx12SharedResources` 并带完整非零 device/queue/resource/fence 句柄，未知枚举值也会被拒绝；`GpuCapabilities`/`ComputeCapabilityGate` 也要求 `HasComputeSharpResourceContract=true` 时必须带非 OpenGL 的合法 `ComputeSharpResourceContractKind`，仅伪造 bool 不会命中 G2。门控已要求 Windows/DX12、`IsComputeSharpCompiled`、合法资源契约与 `ComputeSharpBackend.IsExecutable` 同时满足；当前可执行后端仍为 false，因此即使显式启用构建能探测 ComputeSharp/DX12 device，也不会选择 ComputeSharp。当前新增 ComputeSharp stub 全接口不可用契约自动化切片：`ComputeSharpBackendRejectsEveryExecutionEntryPointWhenInstantiatedDirectly` 覆盖 `LoadKernel`、`BindStorageBuffer`/`BindTexture`/`BindImage`、uniform、`Dispatch`、`MemoryBarrier` 与 timer query 全入口一致抛出资源契约异常，防止 G2 stub 被局部误接通；验证命令为 `dotnet test tests/PixelEngine.Rendering.Tests/PixelEngine.Rendering.Tests.csproj -c Release --no-restore --filter "FullyQualifiedName~ComputeSharpBackend"`。当前新增 ComputeSharp 偏好接线纪律切片：`RenderPipelineSettings.PreferComputeSharpBackend`、`EngineBuilder.PreferComputeSharpBackend()` 与 `EngineOptions.PreferComputeSharpBackend` 会把显式用户偏好传入 `RenderPipeline` 创建时的 `ComputeCapabilityGate.Evaluate(...)`，但 headless/GPU 禁用会清除该偏好，且 G2 仍受资源契约与 stub 后端阻止；验证命令为 `dotnet test tests/PixelEngine.Rendering.Tests/PixelEngine.Rendering.Tests.csproj -c Release --no-restore --filter "FullyQualifiedName~RenderPipelineContractTests|FullyQualifiedName~ComputeCapabilityGateTests|FullyQualifiedName~GpuComputeCapabilityTests"` 与 `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --no-restore --filter "FullyQualifiedName~EngineBuilderTests"`。真实执行现有 pass 仍需实现该资源契约背后的 D3D/共享资源同步层或 D3D 后端；当前不能把该项勾选，也不能用 GL 句柄模拟 DX12 resource。
- [x] 实现 `NullComputeBackend`（空实现，所有入口委派 `plan/08` fragment/CPU 路径）。
- [x] 实现 `GpuCapabilities` 探测（GL 版本、compute/SSBO/image 扩展、`GL_MAX_COMPUTE_WORK_GROUP_*`、ANGLE/ES3、DX12），启动期探测一次并缓存；当前新增生产调用纪律契约：`FromGlCapabilities(...)` 只构造基础快照，work group limit 为 0 时不得命中 G1，生产路径必须经 `GpuCapabilities.Query(...)` 补齐真实 `GL_MAX_COMPUTE_WORK_GROUP_*` 后再 `ComputeCapabilityGate.Evaluate(...)`，防止 GL 4.3+ 机器因未查询 device limit 被误判或绕过 local-size 校验。
...
- [x] 每 pass GL timer query（异步取上一帧结果，不 stall），注册 Core 诊断分项；`FrameProfiler.LastSubFrame` 供 plan/12 HUD 消费；`GpuComputeProfilerTests` 已锁定 completed query 显式 delete、pending query 非阻塞保留、容量覆写时退休最旧 query，避免 timer query 生命周期泄漏或无限积累。
...
- [!] 阻塞：高密度（≥10 万）加色火花场景帧时间优于 CPU stamp 的 BenchmarkDotNet/帧计时实测仍缺目标 GPU 硬件基准，不能仅凭合同测试勾选；当前已新增 `--particle-frame-probe` 真实窗口入口并在本机 win-x64 以 10 万粒子跑通 CPU/GPU 短样本（见 `docs/runtime-reports/2026-07-02-particle-frame-probe.md`），并提供 `tools/gpu-particle-benchmark-preflight.ps1` 收集本机 probe 与校验目标硬件 evidence manifest。该脚本无目标硬件 manifest 时为 `blocked_missing_target_gpu_evidence`，本机短 probe 为 `local_probe_only` 并额外写出 `local-comparison.md/json`（显式 `local_only: true`、`target_gpu_evidence: false`，不提供 `gpuFasterThanCpu` 目标字段），本机 probe 子进程失败或缺 summary 为 `blocked_invalid_local_probe`，manifest 缺 scope/hash 为 `blocked_missing_target_gpu_scope_evidence`，schema、未知/重复 scope、缺文件、sha256 不匹配、硬件报告缺少 `targetGpuName`/`targetGpuDriver`/`gpuBackend`/`particleCount`/`benchmarkRunId` 等机器字段，或 `cpuProbeReport`/`gpuProbeReport` 的 `particle_frame_probe` summary 缺少 `source=PixelEngineParticleFrameProbe`、`benchmark_run_id`、`sample_seconds`，或 probe/comparison 的 run id、`gitCommit` 与 `targetHardwareReport` 不一致，都会被视为拼接不同 run/不同提交的无效证据；`targetHardwareReport.particleCount` 与 CPU/GPU probe `requested_count` 不一致、CPU/GPU probe summary 模式或 `measured_frames>=300`/`requested_count>=100000` 不达标、`comparisonReport` 缺少 `gpuFasterThanCpu: true`、`cpuWallAvgMs`/`gpuWallAvgMs` 不匹配 probe `wall_avg_ms`、`speedupRatio` 不匹配 `cpuWallAvgMs/gpuWallAvgMs` 重算值或不大于 1、`sampleSeconds>=10` 不达标或大于 CPU/GPU probe summary 的 `sample_seconds` 等语义均为 `blocked_invalid_target_gpu_evidence`（由 `PerformanceHardeningToolingDisciplineTests` 的 GPU preflight 行为测试锁定本机 fake `DotNetPath` 正向 probe 的默认非零 / `-AllowBlocked` 退出纪律、local-comparison `local_only: true` / `target_gpu_evidence: false` 输出、scope 污染、硬件字段、同源身份、粒子数一致性、probe 长样本和比较语义边界），证据齐全为 `target_gpu_evidence_attached_pending_review`；这些状态均不是验收通过，短样本与待审证据不能替代目标 GPU 长基准结论。
...
- [x] **profiling/降级**：各 pass GPU 计时注册 Core 诊断并在 HUD 可见；§4.3 第二级自动降级按序触发（RC→bloom→fog-of-war），切换无缝不重建上下文，GPU 计时不回读 sim。
- [x] **不变式与技术栈无冲突**：全文档不违背 `AGENTS.md §1`（尤其 #9 CPU sim 权威）与 `plan/00 §4` 技术栈表（ComputeSharp 作门控可选增项、已在 CPM 登记，默认 GL compute）。
