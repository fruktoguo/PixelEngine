...
- [!] 阻塞：当前 bloom/light/air compute pass 与 `RenderPipeline` 资源契约均绑定 OpenGL texture/image/SSBO 句柄；`ComputeSharpBackend` 只是隔离 stub，Rendering 项目未引用 `ComputeSharp`。ComputeSharp/DX12 不能直接消费现有 GL texture handle；`docs/rendering-computesharp-resource-contract.md` 已记录可接受路线（D3D 渲染后端或显式 GL-DX12 shared resource/fence 层）与禁止路线（把 GL texture name 解释为 DX12 resource）。代码层面已新增 `GpuResourceContractKind` 与 `ComputeSharpResourceContract`：当前 `GpuComputeResources` 显式声明 `OpenGlTextureNames` 且 `CanBeConsumedByComputeSharp=false`，ComputeSharp 合法契约必须是 `D3D12RenderGraph` 或 `GlDx12SharedResources` 并带完整非零 device/queue/resource/fence 句柄。门控已要求 Windows/DX12、`IsComputeSharpCompiled`、`HasComputeSharpResourceContract` 与 `ComputeSharpBackend.IsExecutable` 同时满足；当前可执行后端仍为 false，因此即使测试人工构造 compiled/contract 标志也不会选择 ComputeSharp。真实执行现有 pass 仍需实现该资源契约背后的 D3D/共享资源同步层或 D3D 后端；当前不能把该项勾选，也不能用 GL 句柄模拟 DX12 resource。
