...
- [!] 阻塞：高密度（≥10 万）加色火花场景帧时间优于 CPU stamp 的 BenchmarkDotNet/帧计时实测仍缺目标 GPU 硬件基准，不能仅凭合同测试勾选；当前已新增 `--particle-frame-probe` 真实窗口入口并在本机 win-x64 以 10 万粒子跑通 CPU/GPU 短样本（见 `docs/runtime-reports/2026-07-02-particle-frame-probe.md`），并提供 `tools/gpu-particle-benchmark-preflight.ps1` 收集本机 probe 与校验目标硬件 evidence manifest。该脚本无目标硬件 manifest 时为 `blocked_missing_target_gpu_evidence`，本机短 probe 为 `local_probe_only` 并额外写出 `local-comparison.md/json`（显式 `local_only: true`、`target_gpu_evidence: false`，不提供 `gpuFasterThanCpu` 目标字段），本机 probe 子进程失败或缺 summary 为 `blocked_invalid_local_probe`，manifest 缺 scope/hash 为 `blocked_missing_target_gpu_scope_evidence`，schema、未知/重复 scope、缺文件、sha256 不匹配或 `comparisonReport` 缺少 `gpuFasterThanCpu: true` 为 `blocked_invalid_target_gpu_evidence`（由 `PerformanceHardeningToolingDisciplineTests.GpuParticleBenchmarkPreflightRejectsUnknownAndDuplicateScopes` 与 `GpuParticleBenchmarkPreflightRejectsComparisonReportWithoutGpuFasterThanCpu` 锁定 scope 污染和比较语义边界），证据齐全为 `target_gpu_evidence_attached_pending_review`；这些状态均不是验收通过，短样本与待审证据不能替代目标 GPU 长基准结论。
