...
- [!] 阻塞：高密度（≥10 万）加色火花场景帧时间优于 CPU stamp 的 BenchmarkDotNet/帧计时实测仍缺目标 GPU 硬件基准，不能仅凭合同测试勾选；当前已新增 `--particle-frame-probe` 真实窗口入口并在本机 win-x64 以 10 万粒子跑通 CPU/GPU 短样本（见 `docs/runtime-reports/2026-07-02-particle-frame-probe.md`），但该短样本只能证明入口与子相位采样可用，不能替代目标 GPU 长基准。
