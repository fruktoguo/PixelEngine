...
**实现补充**：当相机为整数放大档（`CellsPerPixel=1/2/4/8` 的倒数）且 Full 样式档开启时，`BuildRowsStyledZoom` 对每个世界 cell 只执行一次完整样式/温度采样，再填充其对应的屏幕像素重复区间；动态样式、温度 glow、纹理提供器与 emissive/occluder 语义均保留。1:1 样式段对同一 chunk 内的左右/上邻居直接读取 SoA，仅在 chunk 边界查表；无纹理提供器时带 `TextureId` 的静态材质仍按 `BaseColorBGRA` 进入 palette 快路径。
...
当前 Hosting 通过 `RenderFrameContext.ForceRebuild` 标记相机移动和 CPU 粒子擦除等未由 chunk dirty rect 表达的视觉变化；其余稳定帧由 `RenderBufferBuilder` 按 resident chunk 引用和 dirty 元数据复用 render/aux buffer，避免错误复用动态温度场。
...
- [x] （可选）UI overlay 纹理上传 API：为 Ultralight 等 CPU 位图上屏路径复用 §3.3 dirty-rect `glTexSubImage2D` 子上传，暴露 `UiOverlayTexture` / `UiPresentContext.UploadOverlayTexture` 脏矩形上传入口，记录 `FrameSubPhase.UiUpload`，并恢复 `PIXEL_UNPACK_BUFFER`、`UNPACK_ROW_LENGTH`、`UNPACK_SKIP_*` 等 GL 状态，避免每帧全量重传和 UI layer 状态污染；`RenderWindowIntegrationTests.UiOverlayTextureUploadsDirtyRectWhenExplicitlyEnabled` 以真实 GL 读回验证。〔synthesis〕
...
- [x] 新增分项耗时诊断：相位 9 RenderStyle 差异化着色已接入 `FrameSubPhase.RenderStyleShading` 与编辑器 HUD 采样；游戏 UI/GUI present 统一记录 `UiComposite`，离屏上传记录 `UiUpload`，并通过 `UiPresentationIntervalFrames`/`UiSkippedPresentationFrames` 暴露降级 cadence；`Present`/`PresentWait` 继续分离提交工作与 SwapBuffers 等待。Ultralight 后端本体仍归 plan/20，不阻塞 Rendering 契约闭合。〔synthesis〕
...
- [x] `UiPresentContext` 共享 GL 原语契约（带 scissor/transform 的三角形批 + 状态恢复）可被 `PixelEngine.UI` 三后端（RmlUi 子集/Ultralight/ManagedFallback）与 ImGui 共用；稳态提交路径走预分配 `UiPrimitiveRenderer`，无每次提交托管分配；`UiOffscreenSurfacePresenterSmokeTests` 已在真实窗口覆盖 `SubmitTriangles` 纯色 probe 与 textured quad 合成，并修正 `uFramebufferSize` float uniform 口径。〔synthesis〕
- [x] 新增分项耗时诊断（RenderStyle 着色、UI update/paint/upload/composite、present submit/wait）出现在性能 HUD 并可驱动降级：RenderStyle 着色已进入编辑器 HUD，且 `IRenderStyleQualityController` 已接入 Hosting 过载降级/恢复；游戏 UI present cadence 降级已接入 Hosting 计数器；Rendering 共享 `UiOverlayTexture` 脏矩形上传入口已记录 `UiUpload`，`UiComposite`/`UiPaint`/`UiUpdate` 与 cadence/skipped 计数已由 HUD 聚合显示。Ultralight 后端本体仍归 plan/20，不阻塞 Rendering 契约闭合。〔synthesis〕
