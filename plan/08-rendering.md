...
- [ ] 建立 `src/PixelEngine.Rendering/PixelEngine.Rendering.csproj`，`TargetFramework=net10.0`、`AllowUnsafeBlocks=true`、ProjectReference 至 `Core`/`Simulation`/`Content`/`World`（依赖方向遵 `00` §5，无反向）。
- [ ] `RenderWindow`：封装 `IWindow`+`IInputContext`+`GL`；请求 OpenGL 3.3 Core profile，debug 构建加 `ContextFlags.Debug`；暴露手动 pump（`DoEvents`/`SwapBuffers`），不调用 Silk.NET 内部 `Run()`（与架构 §4 帧节奏对齐）。
- [ ] `GlCapabilities`：`glGetString(GL_VERSION/RENDERER/VENDOR)`、`glGetIntegerv(GL_MAJOR_VERSION/GL_MINOR_VERSION/GL_NUM_EXTENSIONS)` + `glGetStringi(GL_EXTENSIONS,i)`；置位 `HasComputeShader`(≥4.3)、`HasBufferStorage`(≥4.4)、`IsGles`。
- [ ] `RenderBackend` 选择：默认 `DesktopGl33`；桌面 GL 创建失败或配置指定时回退 `GlEs30Angle`（ES 3.0 via ANGLE，架构 §9.5），创建期一次性决策。
- [ ] GL debug 回调：`glDebugMessageCallback`/`GL_KHR_debug`（capability-gate，debug 构建）转入 Core 诊断；release 关键调用后 `glGetError` 抽样。

### 4.2 GL 资源封装基础

- [ ] `GlTexture`：`glGenTextures`/`glBindTexture`/`glTexImage2D`/`glTexParameteri`（`GL_NEAREST` min/mag、`GL_CLAMP_TO_EDGE`），`IDisposable`。
- [ ] `GlBuffer`：VBO/PBO/UBO 统一封装；PBO 走 `GL_PIXEL_UNPACK_BUFFER`。
- [ ] `Framebuffer`：FBO + 颜色附件 + 完整性检查（`glCheckFramebufferStatus`），离屏 pass 用。
- [ ] `ShaderProgram`：`glCreateShader`/`glShaderSource`/`glCompileShader`/`glLinkProgram`，编译/链接失败抛明确异常并附 info log；缓存 uniform location。
- [ ] `FullscreenQuad`：全屏三角形 VAO/VBO，所有 post pass 复用。
- [ ] GLSL profile 管理：`#version 330 core` 基线 + `#version 300 es` 变体，宏统一差异（精度限定符、`texture()` 等）。

### 4.3 世界纹理流式（架构 §9.2，性能关键）

- [ ] `WorldTexture`：单张视口大小 `GlTexture`，`internalFormat=GL_RGBA8`、`format=GL_BGRA`、`type=GL_UNSIGNED_INT_8_8_8_8_REV`（直 memcpy，>25x，架构 §9.2）；resize 随视口重建。
- [ ] render buffer 后备存储：POH `GC.AllocateArray<uint>(w*h, pinned:true)` 或 `NativeMemory`（BGRA8，零 pin 开销、零托管分配，架构 §14.3）。
- [ ] `PboUploader`（2-PBO ping-pong）：两个 `GL_PIXEL_UNPACK_BUFFER`；每帧 orphan（`glBufferData(...,NULL,GL_STREAM_DRAW)`）→ `glMapBufferRange(GL_MAP_WRITE_BIT|GL_MAP_INVALIDATE_BUFFER_BIT|GL_MAP_UNSYNCHRONIZED_BIT)` → `memcpy` → `glUnmapBuffer` → 绑定 PBO + `glTexSubImage2D`（异步 DMA）。
- [ ] 全帧上传模式（默认）：整 render buffer → PBO → 一次 `glTexSubImage2D` 覆盖全纹理。
- [ ] dirty-rect 子上传模式：聚合 `03` 的 per-chunk dirty rect 只读视图，`glPixelStorei(GL_UNPACK_ROW_LENGTH/SKIP_ROWS/SKIP_PIXELS)` 描述子区，逐脏矩形 `glTexSubImage2D`；运行期可切换（默认关，profiling 触发）。
- [ ] persistent-mapped 快车道（可选，gate `HasBufferStorage`）：`glBufferStorage(...GL_MAP_PERSISTENT_BIT|GL_MAP_COHERENT_BIT)` + `glFenceSync`/`glClientWaitSync`，仅作 A/B 开关、非默认。
- [ ] 明确反对 per-chunk 纹理：仅单张视口纹理（代码注释引用架构 §9.2，禁止 per-chunk 上传纹理）。

### 4.4 render buffer 构建（帧相位 9，并行）

- [ ] `RenderBufferBuilder.Build(...)`：相位 9 并行（Core 持久线程池，按屏幕行带 tile 划分），输出 BGRA8 render buffer。
- [ ] 世界→屏幕映射：由 `CameraState`（来自 `07`）把屏幕像素映射到世界 cell 坐标，只读 `Material` id。
- [ ] `MaterialTextureProvider`：按 `MaterialDef.TextureId` 采样材质纹理，UV 用**世界坐标**取模（同材质大片连续）；无纹理用 `BaseColorBGRA`；可选 `colorVariant`/`ColorNoise` 世界坐标哈希噪声扰动亮度（架构 §7.1/§7.3）。
- [ ] 温度 glow：从 1/4 分辨率温度场（架构 §7.5）采样，超发光阈值按温度加色，写入 BGRA8。
- [ ] emissive 与 solidity 副输出：builder 同时写出 emissive buffer（发光材质）与 occluder/solidity map（供 §4.6）。
- [ ] 守护不变式 #7：本 pass 只读 `Material`/`Temperature`，绝不向 sim cell 写颜色（代码注释引用架构 §7.1，CI 评审项）。
- [ ] sim 降频复用：`RenderFrameContext.SimStepped==false` 时跳过重建、复用上帧 render buffer，相机平移用整图偏移采样（架构 §4.2）。
- [ ] SIMD 转色（palette→BGRA）：标量正确实现优先；热点处 `Vector<T>`/Intrinsics + scalar fallback，反汇编/基准校验后启用（架构 §12.5）。

### 4.5 自由粒子 CPU stamp 合成（帧相位 9，架构 §9.3）

- [ ] `ParticleCompositor.Stamp(...)`：相位 9（render buffer 之后、上传之前）消费 `05` 暴露的活跃粒子 `ReadOnlySpan<Particle>` + active-count（连续数组无虚调用，架构 §7.6）。
- [ ] 屏幕映射 + `round(x),round(y)` 写材质色（+`colorVariant`）进 render buffer；视口裁剪屏外粒子。
- [ ] 发光粒子（火花/熔渣）同时写 emissive buffer（供 bloom，架构 §9.3）。
- [ ] 提供 emissive buffer 契约与 GPU point-sprite hook 点（实际 GPU 批绘归 `09`，本文档不实现）。

### 4.6 光照管线（帧相位 10，架构 §9.4）

- [ ] `EmissiveBuffer`：离屏 `Framebuffer` 颜色附件，承接发光材质 + 发光粒子（HDR 格式 `GL_R11F_G11F_B10F` 可选，gate capability）。
- [ ] `OccluderMap`/solidity map：从 cell 固体性派生（builder 副输出或独立轻 pass），作可见性遮挡输入。
- [ ] `ShadowMap1DPass`：每光源 1D shadow map 硬阴影（极角→最近遮挡距离，mattdesl 式），光源数 gate 上限；`shadow_1d.frag`。
- [ ] `FogOfWarBuffer`：粗字节数组（~1 字节/32×32 区，架构 §9.4），光源 punch 洞 reveal；与 1D shadow 可配置/并存。
- [ ] `CompositePass`：世界纹理 + emissive + 可见性 additive 合成（`glBlendFunc(GL_ONE,GL_ONE)` 或 `composite.frag` 内加色）。
- [ ] `BloomPass`：bright-pass(`bright_pass.frag`) → mip 降采样 → **dual-Kawase**(`kawase_down.frag`/`kawase_up.frag`，默认) 或 separable Gaussian(`gaussian_blur.frag` 水平/垂直，回退) → additive upsample。
- [ ] `LightingPipeline.QualityLevel` 降级档位（架构 §4.3 第二级）：关 Radiance Cascades(归 `09`)→关 bloom→fog-of-war+emissive only；响应 Core 诊断过载信号（决策不在本子系统）。
- [ ] capability hook：`HasComputeShader` 为真时把高质量光照/RC 委托 `09`，本管线只保留 fragment 路径与 hook 点。

### 4.7 后处理与合成编排（架构 §9.3）

- [ ] `RenderPipeline.RenderFrame(...)`：相位 10 主入口，固定合成顺序：世界 blit → (粒子已并入) → 角色/刚体高亮 → 光照合成 → bloom → dither → gamma → UI → present。
- [ ] `world_blit.vert/frag`：世界纹理按 `CameraState`（视口/缩放）绘到内部 framebuffer，`GL_NEAREST` 采样。
- [ ] `OverlayRenderer`：实色矩形/精灵/轮廓绘制原语，供 Demo/Editor 通过公开 API 提交角色/刚体高亮（不硬编码玩法内容）。
- [ ] `DitherPass`：有序 Bayer/蓝噪声抗 banding（gamma 前）；`post_final.frag` 内或独立 pass。
- [ ] `GammaPass`：线性→sRGB 输出校正。
- [ ] 可选 `CrtPass`/scanline：最终 pass 开关。
- [ ] UI present hook：present 前暴露「UI 绘制时机」回调 + 共用 GL 上下文/默认 framebuffer 给 `12`（ImGui 内容不在本文档）；最后 `SwapBuffers`。

### 4.8 相机/视口协作（与 `07`）

- [ ] 只读消费 `CameraState`（位置/缩放/视口矩形，权威在 `07`/`PixelEngine.World`），构造世界 blit 投影与 render buffer 世界坐标映射；本子系统不持有相机权威。
- [ ] 视口 resize：`RenderWindow.Resize` → 重建 `WorldTexture`/render buffer/FBO 链，更新内部渲染分辨率。

### 4.9 诊断、降级与零分配

- [ ] 向 Core 诊断注册分项计时器：相位 9（render buffer 构建、粒子 stamp）、相位 10（PBO 上传、光照、bloom、post、present）。
- [ ] 暴露光照/bloom 质量档位与开关给过载降级（架构 §4.3 第二级）。
- [ ] 公开 API 全带中文 XML 文档注释（脚本/编辑器 IntelliSense 依赖，`00` §7、AGENTS §4）。
- [ ] 稳态帧循环零托管堆分配：POH/`NativeMemory` 后备、无 LINQ/闭包/装箱于相位 9/10 热路径、句柄/uniform location 启动期缓存（基准校验，AGENTS §3）。

### 4.10 带宽与上传基准

- [ ] BenchmarkDotNet 基准：全帧上传 vs dirty-rect 上传的 CPU memcpy/上传耗时（1080p/720p），校验 §3.8 带宽核算结论，据此定档默认上传模式。

---

## 5. 验收标准

- [ ] OpenGL 3.3 Core 上下文在 win-x64 成功创建并出帧；`GlCapabilities` 正确报告 version/extensions 与 `HasComputeShader`/`HasBufferStorage`/`IsGles`。
- [ ] 桌面 GL 创建失败路径能回退到 OpenGL ES 3.0 via ANGLE 并正常出帧（架构 §9.5），整条管线仅用 3.3 Core/ES3 必备能力（`glTexSubImage2D`+PBO+FBO）。
- [ ] 世界纹理为单张视口大小纹理、BGRA8 + `GL_UNSIGNED_INT_8_8_8_8_REV`；CPU render buffer 到 PBO 为直 `memcpy`（无逐像素 swizzle）；代码中无 per-chunk 纹理上传路径。
- [ ] 2-PBO ping-pong 全帧上传工作：map 前 orphan、`GL_MAP_INVALIDATE_BUFFER_BIT|GL_MAP_UNSYNCHRONIZED_BIT`、异步 DMA，无每帧同步停顿（基准确认上传不阻塞）。
- [ ] dirty-rect 子上传模式：聚合 `03` 的 chunk dirty rect 正确子上传，画面与全帧上传逐像素一致（无脏区漏更新/缝）。
- [ ] 相位 9 render buffer 构建在 Core 持久线程池上并行（非 per-frame `Parallel.For`），输出 BGRA8；颜色由材质纹理(世界坐标采样)+温度 glow 生成。
- [ ] 守护不变式 #7：sim cell 不含 RGBA；渲染色在相位 9 生成（代码评审 + 无任何向 `CellGrid` 写颜色的调用）。
- [ ] 守护不变式 #9：渲染全程无 GPU→CPU readback 回到 sim 网格的路径。
- [ ] sim 降频时（架构 §4.2）渲染仍每帧出帧、复用上帧世界纹理、相机平移整图偏移而非像素插值（不变式 #6 不追帧）。
- [ ] 自由粒子在相位 9 按 `round(x,y)` stamp 进 render buffer 后飞行中可见；发光粒子同时进 emissive buffer 并在 bloom 中产生辉光（架构 §9.3）。
- [ ] 光照管线产出 emissive additive + occluder/solidity + 可见性（1D shadow map 硬阴影 与/或 fog-of-war reveal）+ additive composite + bloom(dual-Kawase 默认/Gaussian 回退) + dither + gamma，可选 CRT/scanline。
- [ ] 合成顺序严格为：世界 → 粒子 → 角色/刚体高亮 → 光照 → bloom → dither → gamma → UI（架构 §9.3）。
- [ ] 光照/bloom 可按 `QualityLevel` 降级（关 RC→关 bloom→fog-of-war+emissive only），响应 Core 诊断信号（架构 §4.3 第二级）。
- [ ] 高质量 Radiance Cascades/compute 路径仅通过 capability hook 委托 `09`，本文档不含 compute 实现。
- [ ] ImGui 通过共用 GL 上下文 + present 前 UI 回调接入（`12`），本文档不含 ImGui 控件实现。
- [ ] 相机为只读消费 `07` 的 `CameraState`，本子系统不持有相机权威；视口 resize 正确重建纹理/FBO 链。
- [ ] 带宽基准确认：1080p 全帧上传 ~480MB/s 量级、远低于 PCIe；默认上传模式据基准定档（架构 §9.2）。
- [ ] 稳态帧循环（相位 9/10）零托管堆分配（BenchmarkDotNet `MemoryDiagnoser` 确认 Gen0=0）。
- [ ] 所有公开 API 带中文 XML 文档注释。
- [ ] 本文档「技术栈与依赖」不与 `00-conventions-and-techstack.md` §4 冲突（验收时核对）。
...
- [ ] `feat(render): Silk.NET 窗口/GL3.3 上下文与能力门控(ANGLE 回退)`（§4.1+§4.2）— 对应本文档 §实现清单 4.1/4.2。
- [ ] `feat(render): 世界纹理 BGRA8 流式与 2-PBO ping-pong 上传`（§4.3）— 含全帧/dirty-rect/persistent-mapped。
- [ ] `feat(render): 相位9 并行 render buffer 构建(材质纹理+温度 glow)`（§4.4）— 守护不变式 #7。
- [ ] `feat(render): 相位9 自由粒子 CPU stamp 合成与 emissive 副输出`（§4.5）。
- [ ] `feat(render): 相位10 光照管线(emissive/occluder/可见性/composite)`（§4.6 前半）。
- [ ] `feat(render): bloom(dual-Kawase) 与 dither/gamma/CRT 后处理`（§4.6 bloom + §4.7）。
- [ ] `feat(render): 合成编排/相机协作/诊断降级与零分配加固`（§4.7/§4.8/§4.9）。
- [ ] `test(render): 上传与零分配/带宽基准并定档默认上传模式`（§4.10 + §5 基准类验收）。
