...
- [~] 合成尾部两级 UI 确定性叠放：游戏 UI（`PixelEngine.UI`）先、编辑器 UI（ImGui）后，均在 gamma 后、present 前；`RenderPipeline` 已按 order 遍历并在每层前重置默认 framebuffer/viewport/blend/depth/cull 基线，玩家包 player-only 审计与完整 GL 状态栈仍待后续。〔synthesis〕
