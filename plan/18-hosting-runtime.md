...
- [!] `Scene` 模型 + `ISceneService`:加载/卸载/切换;从存档(plan/07)或程序化生成构建起始世界。阻塞:已完成来源校验与解析,且 `AttachWorldFromSaveDirectory` 可显式从 save directory 装配 live World/Simulation/粒子后端；`.scene InitialSaveDirectory` 自动解析、程序化 world generator、Physics 刚体快照恢复、world seed/game time 恢复仍未完成。
