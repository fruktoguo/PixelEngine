...
- [!] Play/Edit/Step 三态机 + 进入 Play 前世界快照、退出回滚(plan/07 快照),脚本生命周期协调(plan/11、plan/12)。阻塞:已完成模式驱动、StepOnce、resident world 快照/恢复、Editor 临时 Play 快照后端、存活 Behaviour 字段状态恢复，以及退出 Play 时对存活 Behaviour 派发 OnDestroy、再次进入 Play 时重新 OnStart；剩余缺口为脚本 Scene 拓扑（Play 中新增/删除实体与组件）的快照或重建策略。
...
- [!] `EngineBuilder().…​.Build().Run()` 能装配全部子系统并跑稳定 60fps 空场景。阻塞:Rendering/Audio/Physics/Scripting/Editor 已有真实运行入口,稳定 60fps 空场景仍需 plan/14 运行态验证。
...
- [!] Play/Edit/Step 切换正确:进入 Play 快照、退出回滚到编辑态,脚本 OnStart/OnDestroy 正确触发。阻塞:world 快照/回滚已由 `EngineWorldSnapshotStore` 接入 Editor 临时 Play，`EngineEditorPlaySessionService.ExitPlay()` 已结束脚本 Play Session 并允许再次进入 Play 时重新 OnStart，存活 Behaviour 字段已恢复；脚本 Scene 拓扑回滚仍需 plan/11/12。
...
- [!] `feat(host): 场景/项目模型 + Play/Edit/Step 模式 + headless`。阻塞:模型/模式/headless/Physics phase 8/world 快照回滚/脚本 Play Session 生命周期/存活 Behaviour 字段回滚基础已落地,脚本 Scene 拓扑回滚仍需后续计划。
