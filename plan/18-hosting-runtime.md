...
- [!] 关闭时 native 资源与 ALC 正确释放,无泄漏(配合 plan/14 scripting 测试)。Hosting 已验证关闭时释放 `IScriptRuntime`，Scripting 已验证热重载旧 ALC 可回收；本机真实窗口非 Editor 3600 tick 与 Editor 1200 tick 进程均 exit=0，外部采样峰值工作集约 163 MB；`tools/native-leak-preflight.ps1` 可生成 process smoke 与 detector 报告索引，但无 detector 时默认失败。阻塞:仍缺跨平台 runner 与专用 GL/OpenAL/Box2D native leak detector 证据。
