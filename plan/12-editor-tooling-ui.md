...
`PixelEngine.Editor` 是服务 Unity-like 独立编辑器壳的 Editor ImGui 可视化工具层：它把 sim 的不可见态（dirty rect、parity、活跃 chunk、owned-by-body）变为可视、可交互；提供材质/反应实时编辑、世界画刷、检视器、性能 HUD、存读档与各子系统调参面板；并提供「编辑 / 运行游戏」模式切换。

锁定决策（本文档核心、不可偏离）：

- Editor ImGui 面板层 = **Dear ImGui（Hexa.NET.ImGui）停靠式面板 + 调试工具层**，**与 `plan/08`（Rendering）共享同一个 OpenGL 上下文**，ImGui 在世界渲染（架构帧相位 [10]）之后、`present` 之前绘制于顶层。**面板层 `PixelEngine.Editor` 是宿主进程内的开发工具层，不另起进程 / 不另开窗口 / 不另建 GL 上下文——该约束是相对「被编辑 / 运行的游戏世界」成立的**：面板层不为运行游戏 spawn 第二进程、第二窗口或第二 GL 上下文。承载编辑器面板层的**宿主进程由独立顶层应用 `apps/PixelEngine.Editor.Shell`（见 `plan/19`）负责**（该壳拥有唯一 `RenderWindow`+GL 上下文，`plan/18` 提供 attach/不 own 的窗口所有权解耦 API）；编辑器进程内始终是**单窗口 / 单 GL 上下文**跑 Edit/Play，本条约束在该进程内仍完全成立。
- **ImGui 恒为编辑器 / 开发（dev）向 UI**：调试叠层、检视器、调参、性能 HUD、材质编辑器一律 ImGui。**面向玩家的主菜单 / 背包 / 对话 / 设置等运行时 UI 归 `PixelEngine.UI`（`plan/20`）的 Web-first HTML 后端**，与 ImGui 是两套并存 UI，共享同一 GL 上下文但职责不重叠（合成次序 / 输入仲裁见 §3.3 与本节新增小节）。玩家 HUD 所需的 ImGui host（`PixelEngine.Gui.HexaImGuiBackend`、`IGuiContext` 运行时适配、字体栈）已下沉至中性程序集 `PixelEngine.Gui`（位于 Rendering 之上、Editor 之下）；编辑器面板层仍保留 `PixelEngine.Editor` 专用 backend / dockspace / bridge，但由独立壳注入，不再被 Hosting 或 Demo 静态引用；`plan/20` 的字体 / 回退复用 `PixelEngine.Gui` 中性栈。
...
边界说明：`AssetBrowserPanel` 是 `plan/19` Unity-like Project Window 可复用的资源浏览底座，不等于完整 Project Window 产品契约。本文件不拥有 stable asset id/manifest、资源拖拽引用、脚本双击外部编辑器、Project Settings / Player Settings 或资源重命名后的引用保持；这些 shell 级 authoring UX 归 `apps/PixelEngine.Editor.Shell` / `plan/19`。
...
### 3.16 编辑器 ImGui 与 `PixelEngine.UI`（`plan/20`）共存

编辑器 ImGui 与 `PixelEngine.UI`（`plan/20`，Web-first 透明 HTML UI Runtime）同处一个 GL 上下文、同在世界渲染（相位 [10]）之后叠加，须建立确定性协作，不靠订阅顺序隐式决定叠放：

- **合成次序（UI 先 / 编辑器后）**：经 `plan/08` 新增的**显式带序号 / 优先级的 UI 层注册接口**（替代脆弱的多播订阅顺序），叠放顺序固定为 **世界 → `PixelEngine.UI` 透明 UI（`plan/20`）→ 编辑器 ImGui（本文件）**——编辑器面板恒绘制在运行时 UI 之上，便于开发期覆盖调试。两订阅者各自 `Composite`/`Render` 后恢复 GL 状态（program/blend/scissor/vao），互不污染（呼应 §3.3 GL 状态恢复）。发行构建禁用编辑器后仅运行时 UI 存在，零编辑器开销（守 §1）。
- **输入仲裁**：沿用 §3.3 的多级优先级链 **编辑器 ImGui > 模态游戏 UI > 非模态 HUD > 世界**。
- **Play 让位**：进入 Play 模式，编辑器工具让位（仅保留调试叠层/HUD 显示），`PixelEngine.UI` 接管玩家交互（呼应 §3.15 `EditorMode`）。
...
- [x] 材质编辑器可编辑材质可玩性/视觉字段并经 `ReloadStable` 热重载生效、id 不重排、`RubbleTarget` 缺失走 fallback；`FlowRate` 超 `MoveCap` 被 clamp（#4/#7/#8、§3.8）
