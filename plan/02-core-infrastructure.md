...
- [ ] `FrameTiming.cs`：`readonly struct FrameTiming { Dt; RunSim; RunPhysics; FrameIndex; SimTickIndex; }`。
- [ ] 在实现处写明「**绝无追帧 accumulator**，每帧至多一步，过载即时间膨胀」注释，引用架构 §4.1 与不变式 #6（驱动相位 0，架构 §3.3）。
