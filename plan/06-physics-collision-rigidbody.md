...
- [x] `metal` 梁近 `lava` 相变失去 `RigidOwned` 固体性后，上方脱落连通块由 §4.7 (8a) 既有「新脱落块→建新刚体」逻辑自动坍塌成动态刚体（无 Demo 特判、无新 native）。证据：`TemperatureFieldTests.ApplyPhaseTransitionsRoutesRigidOwnedMeltToDamageSink`、`TemperatureFieldTests.ApplyPhaseTransitionsScansActiveTemperatureBlocksOutsideDirtyRect`、`LavaMineSceneTests.MetalLavaMeltsSupportAndDropsUpperWoodStructure`；其中 Demo 验收只向下方 `lava` 池注热，断言热量经温度场传导到 `metal` 梁达到熔点后再触发相变、刚体 damage 路由与 8a 重建。
...
- [x] `metal` 梁近 `lava` 熔化后其上木结构脱落成动态刚体并下落（既有 8a 脱落链路，headless tick 断言）。证据：`LavaMineSceneTests.MetalLavaMeltsSupportAndDropsUpperWoodStructure` 构造 headless metal/wood/lava 局部结构，只向下方 lava 池注热，先断言 metal 梁低于熔点，再断言温度场传导使 metal 梁达到熔点、metal→molten_metal、RigidOwned metal 支撑清除、8a 重建产生 wood 子刚体并在后续 tick 沿重力下落。
