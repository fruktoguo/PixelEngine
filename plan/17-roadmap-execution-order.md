...
- [x] M15 性能局部增量证据：CA 热路径中心 chunk movement / lifetime dirty 标记已改为直接本地写入，`CheckerboardSchedulerTests.StepCaInternalMoveMarksCenterDirtyWithoutExtraChunkLookup` 锁定内部移动不再额外回查 chunk map。该证据只关闭局部重复查表风险，不替代 full-active CA 目标硬件 cells/frame、硬件计数器、AVX-512 或帧预算验收。
