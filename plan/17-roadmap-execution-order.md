...
- [x] M14 自动化增量证据：中性 `GuiWindowInputConnector` 与 EditorShell `EditorWindowInputConnector` 已在 MouseDown/MouseUp 前同步当前 `mouse.Position`，并由 `GuiInputConnectorsRefreshPointerPositionBeforeMouseButtons` 锁定，降低高 DPI、窗口切换或缺少最新 MouseMove 时按钮命中旧坐标导致“能滚动但点不到”的风险；该证据不替代真实窗口完整点击链路、人工 UX 或 IME/剪贴板手感验收。
