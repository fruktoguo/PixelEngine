...
| **M12** | Demo 整合+测试+打包+性能收口 | 13、14、15、16 | 落沙游戏:可操作角色+全材质反应+刚体+粒子+光照+音频+关卡(仅依赖公开 API);全套测试通过;6-RID R2R+AOT 打包;玩家友好启动布局(包根 launcher、运行时依赖进 app/);目标硬件基准落实;GC 模式定档 | 满足架构 §1.4 全部成功度量;Demo 端到端可玩;反推出的引擎 API 缺口全部补齐 |
