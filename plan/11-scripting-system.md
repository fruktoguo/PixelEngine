...
**`MaterialInfo` 门面扩展（只读镜像 `MaterialDef`，plan/04）**：现 `readonly record struct MaterialInfo(Id, Name, Density, IsSolid)` 增补供武器伤害计算、材质图例（`MaterialLegendHud`）、危险采样读取的可玩性/视觉字段——`DisplayName`、`LegendCategory`、`LegendVisible`、`BaseColorBgra`、`MineYield`、`CellType`、`Category`、`Emissive`、`Hardness`、`MaxIntegrity`、`IsDestructible`、`FlowRate`、`BlocksCharacter`、`Flammability`、`AutoIgnitionTemp`、`FireHp`、`TemperatureOfFire`、`GeneratesSmoke`、`HeatConduct`、`HeatCapacity`、`RenderStyle` 与 `Properties`。**铁律（守 #7）**：所有视觉/颜色字段只从 `MaterialDef` 只读投影，`MaterialInfo` 是内存镜像，脚本**绝不**据此把颜色写回 cell；渲染 BGRA 由渲染相位 CPU 计算（plan/07），与本门面无关。字段值随材质热重载刷新，`GetInfo` 保持即时只读。

**`CellView` 门面扩展（单 cell 耐久投影）**：现 `readonly record struct CellView(Material, Flags, Lifetime)` 增 `Integrity`（`byte`）字段，语义为 `= MaxIntegrity(Material) − Damage(x,y)`，其中 per-cell `Damage` 平面是 plan/03 新增的 SoA lane（单缓冲原地，随 chunk 快照/迁移，见 plan/03/07/16）。`CellView` 是**即时只读快照**，供脚本/武器查询某 cell 剩余耐久（如激光穿透判定、挖矿进度显示）；它只投影不写回，破坏一律走下述 `IWorldEffects` 延迟路径。

**`IWorldEffects` 门面扩展（破坏/热注入，plan/05 执行、plan/04 热场）**：现 `IWorldEffects.Explode(x,y,radius,force)` 保留签名但**内部改为组合**——不再无条件抛射半径内全部 cell，而是 `DamageCircle` 破坏（Empty/RubbleTarget 化 + 碎屑请求）+ 对邻近刚体的径向 `ApplyImpulse`，使材质抗性真正生效。新增三个方法与一个枚举：

```csharp
/// <summary>脚本破坏来源；区分破坏模型与音视觉反馈，且供 sim 侧统计。</summary>
public enum DamageKind : byte
{
    Impact,      // 爆炸/弹丸冲击（半径衰减）
    Beam,        // 光束逐 cell 钻孔（激光）
    Corrosion,   // 酸蚀（反应驱动，按 Probability 与 Hardness）
    Heat,        // 热致破坏（熔化/燃烧配合相变）
}

public interface IWorldEffects
{
    void Explode(float x, float y, int radius, float force); // 保留：内部 = DamageCircle + 邻近刚体径向冲量
    /// <summary>圆形区域按材质抗性扣耐久：半径内每 cell 按 falloff 距离衰减调 ApplyDamage，消费 Hardness/MaxIntegrity，破坏归零转 RubbleTarget/Empty + 碎屑 + MineYield 采集事件。延迟入命令队列，相位安全窗口落地并标 dirty + KeepAlive。</summary>
    void DamageCircle(float x, float y, int radius, float damage, float falloff, DamageKind kind = DamageKind.Impact);
    /// <summary>沿束逐 cell 钻孔伤害：从 (x,y) 沿 (dx,dy) 推进 length，每 cell ApplyDamage(damagePerCell) + AddHeat(heatPerCell)，遇超高 Hardness 或能量耗尽停止。延迟入队，落安全相位。</summary>
    void DamageBeam(float x, float y, float dx, float dy, float length, float damagePerCell, float heatPerCell, DamageKind kind = DamageKind.Beam);
    /// <summary>圆形区域注热（映射 plan/04 §3.9 TemperatureField.AddHeat）：半径内每 cell 温度 +deltaC，供激光/燃烧武器触发熔化/相变。延迟入队，落温度场安全窗口。</summary>
    void AddHeat(float x, float y, int radius, float deltaC);
}
```

相位与不变式合规：`DamageCircle`/`DamageBeam`/`AddHeat` 全部编码为 blittable 命令压入 §3.3 的 `ScriptCommandQueue`，与现 `Explode`/`SetCell` **同构**——只在单线程输入/安全相位 flush 到真实后端，破坏后标 dirty + 跨界 KeepAlive（守 #1/#2/#3/#6）。破坏是**安全相位的离散编辑**，不受液体 32px halo 约束（**不引 #4** 去约束破坏半径；#4 只约束液体单步位移）。命中刚体像素时不在破坏路径累加 `Damage`，而由 plan/05/06 在写 cell 前查 `CellFlags.RigidOwned`、经 `IRigidDamageSink.OnOwnedCellDamaged` 路由触发形状重建（守 #5，本门面只发意图、不实现刚体逻辑）。

**`IParticleSpawner` 门面扩展（富速度分布发射，plan/05）**：现 `Spawn`/`Burst` 之外增 `Emit`，供枪口火花、碎屑锥等带**速度锥/分布**的发射：
...
`Emit` 与 `Spawn`/`Burst` 同样延迟到**相位 7**（架构 §7.6），零分配命令编码；速度锥采样在 flush 时由 plan/05 后端完成（确定性可选种子），脚本侧只提交描述。
...
- [x] 审定并标注公开 API 表面：`Behaviour`/`IComponent`/`ISystem`/`Entity`/`Scene`/`IScriptContext` 及全部子接口/值类型为 `public`；内核（CellGrid SoA、checkerboard、Box2D 桥、ALC、编译管线、命令队列）为 `internal`（§3.1，架构 §3.1/§13.1）
...
- [x] 建游戏项目 `.csproj` 模板与引用集合（`ProjectReference` for Demo / `PackageReference` for 外部 NuGet），承接引擎 XML 注释（§3.6）
...
- [x] 派发路径零分配验证：无 LINQ/闭包/装箱/`params`，缓存开放委托或虚调用（AGENTS §3）
...
- [x] Physics/角色脚本命令 flush：`CreateBodyFromRegion/ApplyImpulse/DestroyBody/MoveCharacter` 通过 `ScriptCommandQueue` 与 Hosting phase 8 step 前 `FlushPhysicsCommands()` 落到真实 `PhysicsSystem`/`CharacterController`，并维护刚体与角色状态句柄 registry。
...
- [x] 捕获后：经诊断 sink 记录（异常+脚本类型+回调名+帧号）、标 `Faulted` + `Enabled=false`、通知编辑器；绝不冒泡主循环
- [x] `ResetFault()`：热重载修复后清除 Faulted（§3.5）
- [x] 验证：单脚本异常不影响同帧其它脚本、不崩引擎（对应 §5 验收）

### 4.6 IDE 集成（轻量，§3.6）
- [x] `internal sealed class IdeLauncher`：探测 Rider/VS（vswhere）/VS Code（PATH/注册表），按优先级 `Process.Start` 打开 `.sln`
- [x] `internal sealed class ProjectGenerator`：从模板生成/刷新游戏 `.csproj`/`.sln`，开 `GenerateDocumentationFile`（§3.6）
...
- [x] `IWorldEffects` 增 `DamageBeam(x,y,dx,dy,length,damagePerCell,kind)`：沿束逐 cell `ApplyDamage`，配套注热由 `AddHeat` 单独表达；遇超高 `Hardness`/能量耗尽停止；延迟命令落安全相位（§3.9，plan/05/04）。证据：`WorldDamageCommandsFlushIntoStructuralDamage`、`DamageBeamBurnThroughTests`。
...
- [x] `Scripting.Transform` 字段与 plan/18 `.scene` v2 Transform 块逐字段对齐，`Scripting.Scene`/`Entity`/`Behaviour` 保持扁平 DOD、**不**引 `ParentId`/父子层级；组件字段绑定支持 `Vector2`（§3.11，物化由 Hosting 烘焙世界 TRS）。证据：`SceneAndHeadlessTests.SceneDocumentV2BuildsTransformHierarchyAndVector2Fields`、`SceneDocumentV2BakesParentRotationAndScaleIntoRuntimeTransform`、`EditorShellSceneMaterializationTests.RuntimeProjectionBakesHierarchyAndBindsStableIdsVector2AndMaterialId`。
...
- [x] 完整脚本 facade 能力：建/查/控刚体、驱动角色控制器、控相机、读输入已有真实后端与验收覆盖；Diagnostics/GUI 不属于 `IScriptContext` 契约，归 plan/02 诊断后端与 plan/12 Editor 面板处理（§3.3）。
...
- [x] 脚本经 `Materials.GetInfo` 读到扩展字段（`Hardness`/`MaxIntegrity`/`Emissive`/`BaseColorBgra`/`FlowRate`/`Flammability`/`HeatConduct`/`RenderStyle`/`Properties` 等），值与 `materials.json` 一致且随材质热重载刷新；`FlowRate` 已 clamp 到 `[0,MoveCap]`（§3.9，守 #4/#7/#8）。证据：`MaterialContentLoaderTests`、`MaterialTableTests`、`DispersionMoveCapTests`、`ScriptSimulationContextTests.MaterialInfoFacadeReflectsMaterialReload`。
- [x] 同一 `DamageCircle(damage=X)` 对不同材质差异化生效：sand/dirt 立即抛碎、stone 需累计或大当量、metal 小爆破近免疫（差异全来自 `Hardness`/`MaxIntegrity`）；破坏归零转 `RubbleTarget` 并抛碎屑、发 `MineYield` 采集事件；命中刚体像素经 `IRigidDamageSink` 路由而非累加 `Damage`（§3.9，守 #5）。证据：`CellDamageResistanceTests`、`CellDamageRubbleHandshakeTests`、`RigidOwnedDamageRoutingTests`。
- [x] `DamageBeam` 沿束逐 cell 按 `Hardness` 烧穿（木/冰快、metal 慢）并可配合 `AddHeat` 触发相变；`CellView.Integrity` 反映剩余耐久（§3.9）。证据：`DamageBeamBurnThroughTests`、`WorldAddHeatFlushesIntoTemperatureField`、`FacadesReadMaterialCellsAndSolidsFromSimulationBackends`。
- [x] `Explode`/`DamageCircle`/`DamageBeam`/`AddHeat`/`Emit` 写入类调用经命令队列在安全相位落地（cell 破坏标 dirty + KeepAlive、particle 落相位 7），破坏为离散编辑不受 32px halo 约束、无跨相位竞争、稳态帧派发零托管分配（§3.9，守 #1/#4/#6，AGENTS §3）。证据：`WorldDamageAndHeatCommandsDoNotAllocateAfterWarmup`、`ParticleEmitCommandsDoNotAllocateAfterWarmup`、`WorldEffectBoundaryConservationTests`。
...
- [x] authoring（plan/19）→运行时物化后 `Scripting.Scene`/`Entity`/`Behaviour` 保持扁平、无 `ParentId`/父子层级；`Scripting.Transform` 承接 plan/18 `.scene` v2 世界 TRS 烘焙值、`Vector2` 字段绑定正确还原（§3.11，物化/往返测试归 plan/14/18/19）。证据：`SceneAndHeadlessTests` 与 `EditorShellSceneMaterializationTests` 的 `.scene v2`/runtime projection 测试。
...
- [x] `feat(script): 建立 Scripting 项目与公开 API 边界(项目引用模型+XML 注释门禁)`（对应 §4.1）
- [x] `feat(script): 实现 Behaviour/Component/System 与实体组件存储(相位1生命周期)`（对应 §4.2）
- [x] `feat(script): 实现 IScriptContext 世界脚本接口 facade 与延迟命令队列`（对应 §4.3）
- [x] `feat(script): 实现 Roslyn 编译与可回收 ALC 热重载(状态保持+防泄漏)`（对应 §4.4）
- [x] `feat(script): 实现脚本回调异常隔离(记录/禁用/不崩引擎)`（对应 §4.5）
- [x] `feat(script): 实现轻量 IDE 集成与项目生成(Rider/VS/VSCode 启动)`（对应 §4.6）
- [x] `feat(script): 接通编辑器 Inspector 反射与 Hosting 主循环驱动`（对应 §4.7）
- [x] `test(script): 脚本系统验收测试(热重载无泄漏/异常隔离/相位安全/零分配)`（对应 §5）
- [x] `feat(script): MaterialInfo/CellView 玩法字段门面扩展(抗性/耐久/视觉只读投影)`（对应 §4.8，§3.9）
- [x] `feat(script): IWorldEffects DamageCircle/DamageBeam/AddHeat+DamageKind(Explode 改组合)与 IParticleSpawner.Emit`（对应 §4.8，§3.9）
- [x] `feat(script): IScriptContext 接入 IGameUiService 交互 UI 门面(相位安全事件/写入,不反向依赖 UI)`（对应 §4.8，§3.10）
- [x] `feat(script): Scripting.Scene/Transform 扁平物化目标对齐 .scene v2(Vector2 字段绑定,不引层级)`（对应 §4.8，§3.11）
