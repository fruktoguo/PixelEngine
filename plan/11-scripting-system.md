...
- [ ] `MaterialInfo` 增 `DisplayName`/`BaseColorBgra`/`CellType`/`Category`/`Emissive`/`Hardness`/`MaxIntegrity`/`IsDestructible`/`FlowRate` 只读字段，从 `MaterialDef` 只读投影，随材质热重载刷新；补全中文 XML 注释（§3.9，plan/04，守 #7 不写回颜色）
- [ ] `CellView` 增 `Integrity`（`byte`）字段 = `MaxIntegrity(Material) − Damage(x,y)`（消费 plan/03 per-cell `Damage` SoA lane），即时只读投影不写回（§3.9）
- [ ] `enum DamageKind`（`Impact`/`Beam`/`Corrosion`/`Heat`）为 `public`，含中文 XML 注释（§3.9）
- [ ] `IWorldEffects` 增 `DamageCircle(x,y,radius,damage,falloff,kind)`：按材质 `Hardness`/`MaxIntegrity` 圆形衰减扣耐久、破坏归零转 `RubbleTarget`/`Empty` + 碎屑请求 + `MineYield` 采集事件；延迟命令、安全相位落地标 dirty + KeepAlive（§3.9，plan/05，守 #1/#2/#3/#6）
- [ ] `IWorldEffects` 增 `DamageBeam(x,y,dx,dy,length,damagePerCell,heatPerCell,kind)`：沿束逐 cell `ApplyDamage` + `AddHeat`，遇超高 `Hardness`/能量耗尽停止；延迟命令落安全相位（§3.9，plan/05/04）
...
- [ ] `IGameUiService`/`UiEvent`/`UiValue` 契约置于中性契约层（同 `IAudioApi` 策略），`PixelEngine.Scripting` **不** `ProjectReference` `PixelEngine.UI`；禁用 HTML UI 时 `Ui` 注入 no-op 空对象（§3.10，plan/00 §5 依赖方向）
...
- [ ] 脚本经 `Materials.GetInfo` 读到扩展字段（`Hardness`/`MaxIntegrity`/`Emissive`/`BaseColorBgra`/`FlowRate` 等），值与 `materials.json` 一致且随材质热重载刷新；`FlowRate` 已 clamp 到 `[0,MoveCap]`（§3.9，守 #4/#7/#8）
...
- [ ] `PixelEngine.Scripting` 不引用 `PixelEngine.UI`（`IGameUiService` 契约在中性层，后端由 Hosting 注入），依赖方向合规（§3.10，plan/00 §5）
...
- [ ] `feat(script): MaterialInfo/CellView 玩法字段门面扩展(抗性/耐久/视觉只读投影)`（对应 §4.8，§3.9）
- [ ] `feat(script): IWorldEffects DamageCircle/DamageBeam/AddHeat+DamageKind(Explode 改组合)与 IParticleSpawner.Emit`（对应 §4.8，§3.9）
- [ ] `feat(script): IScriptContext 接入 IGameUiService 交互 UI 门面(相位安全事件/写入,不反向依赖 UI)`（对应 §4.8，§3.10）
- [ ] `feat(script): Scripting.Scene/Transform 扁平物化目标对齐 .scene v2(Vector2 字段绑定,不引层级)`（对应 §4.8，§3.11）
