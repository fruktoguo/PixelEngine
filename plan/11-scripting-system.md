...
- [x] 脚本经 `Materials.GetInfo` 读到扩展字段（`Hardness`/`MaxIntegrity`/`Emissive`/`BaseColorBgra`/`FlowRate` 等），值与 `materials.json` 一致且随材质热重载刷新；`FlowRate` 已 clamp 到 `[0,MoveCap]`（§3.9，守 #4/#7/#8）。证据：`MaterialContentLoaderTests`、`MaterialTableTests`、`DispersionMoveCapTests`、`ScriptSimulationContextTests.MaterialInfoFacadeReflectsMaterialReload`。
