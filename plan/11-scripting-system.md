...
- [!] Hosting 装配期构造完整 `IScriptContext` 并注入各子系统公开 API（§3.8）。进展：cell/material/particle/solid/time/audio/input/camera/lighting/physics facade 已有真实后端，Hosting 会注册 `ScriptSimulationContext` 供相位 8 flush；阻塞：Diagnostics/GUI 等编辑器向后端尚未统一聚合，不能用抛 `NotSupportedException` 的假实现冒充装配。
...
- [!] 完整脚本 facade 能力：建/查/控刚体、驱动角色控制器、控相机、读输入已有真实后端与验收覆盖；阻塞：Diagnostics/GUI 等完整上下文装配仍未统一完成（§3.3）。
