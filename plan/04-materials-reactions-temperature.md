...
- [x] 定义 `DamageKind` 与三路径差异化抗性契约：爆炸 `DamageCircle`（`Explode` 内部改组合）、激光 `DamageBeam`(+AddHeat)、酸蚀反应改走结构伤害；抗性差异全由 materials.json 表达（§3.11；API 缺口归 plan/05+plan/11）。证据：`IWorldEffects.DamageCircle/DamageBeam`、`World.Explode`→`DamageCircle`、Demo 武器 `DamageCircle`/`DamageBeam`+`AddHeat`、`DamageBeamBurnThroughTests` 与脚本 flush/零分配测试覆盖爆炸/激光路径；`ScriptCommandQueueTests.DamageCommandsPreserveDamageKind` 锁定 `DamageKind` 不被脚本命令丢弃；`ReactionEngineTests.CorrosionReactionAccumulatesStructuralDamageAndKeepsAcidSource` / `CorrosionReactionDestroysToRubbleAndPublishesCellDestruction` / `CorrosionReactionDoesNotBypassHighHardnessMetal` 覆盖 `acid + [corrodible]` 不再直接输出式替换目标，而是按 Hardness/Integrity 累积或转 `DestroyedTarget` 并发布 `CellDestructionEvent`。
...
- [x] cell 破坏归零转 `DestroyedTarget`（stone→gravel）并按 `DebrisCount` 抛碎屑；`Integrity==0` 材质即时破坏；`Diggable`&`MineYield` 材质（crystal）被武器破坏发一次采集事件；`Damage` lane 稳态帧零托管分配（守 #1，MemoryDiagnoser 确认）。证据：`CellDamageRubbleHandshakeTests` 覆盖 rubble/Empty、DebrisCount→真实粒子、Diggable MineYield→`CellDestructionEvent`；`CellDamageResistanceTests.MaxIntegrityZeroDestroysImmediatelyAfterHardness` 覆盖 `Integrity==0`；`WorldEffectBoundaryConservationTests.StructuralDamageEntriesDoNotAllocateAfterWarmup` 与 `ScriptSimulationContextTests.WorldDamageAndHeatCommandsDoNotAllocateAfterWarmup` 覆盖结构破坏入口与脚本命令稳态 0 分配；Demo `WeaponControllerExcavatorPublishesMineYieldEvent` 覆盖挖掘武器采集事件，`DamageCircleDestroysObjectiveCrystalAndMissionDirectorCollectsMineYield` 覆盖普通 `IWorldEffects.DamageCircle` 破坏 crystal 后经 `ObjectiveCrystal` 发布 `MineYieldEvent` 并被 `MissionDirector` 计入目标；BenchmarkDotNet `MemoryDiagnoser` 短基准 `SimulationAllocationBenchmarks.ApplyStructuralDamageAccumulates` / `DamageCircleAccumulates` 报告 `Allocated=-`（命令：`dotnet run --project bench\PixelEngine.Benchmarks\PixelEngine.Benchmarks.csproj -c Release -- --filter "*SimulationAllocationBenchmarks*Damage*" --job short --warmupCount 1 --iterationCount 3`，报告：`BenchmarkDotNet.Artifacts\results\PixelEngine.Benchmarks.SimulationAllocationBenchmarks-report-github.md`）。
