...
- 常量：`ChunkSize=64`、`MoveCap=32`、`DirtyRectPadding`（默认 1，可调）、`SingleThreadChunkThreshold`（活跃 chunk 回退阈值，可调）等集中在 `PixelEngine.Core` 的 `EngineConstants`（plan/00 §7），编译期常量便于 JIT 优化。
...
- **grow**：任何 `set/move/react` 触及 cell `(lx,ly)` ⇒ `chunk._working = chunk._working.Union(lx, ly, EngineConstants.DirtyRectPadding)`（padding 默认 1，可调；使被改动 cell 的邻居下帧被重检，架构 §5.4）。
...
### 4.1 寻址与数据结构
- [ ] `ChunkCoord.cs`：`readonly struct ChunkCoord(int X,int Y)` + `IEquatable`/`GetHashCode`（供 plan/07 hash-map 键）。（§3.1）
- [ ] `CellAddressing`：`static` 内联 `WorldToChunk`、`LocalIndex(wx,wy)`、`ChunkOf(wx)`，全用 `>>6`/`&63`，`[MethodImpl(AggressiveInlining)]`。（§3.1）
- [ ] `CellType.cs`：`enum CellType : byte { Empty, Solid, Powder, Liquid, Gas, Fire }`（架构 §7.2，plan/04 `MaterialDef.Type` 引用）。（§3.6）
- [ ] `CellFlags.cs`：常量 `Parity=1<<0,Settled=1<<1,Burning=1<<2,FreeFalling=1<<3,RigidOwned=1<<4`（bit5–7 备用）；内联辅助 `HasParity/SetParity/MatchesFrame`。（§3.4）
- [ ] `IRigidDamageSink` 钩子 + 在 movement/reaction 覆盖 `RigidOwned` cell 时调用（plan/06 契约,空实现退化为普通 CA）。（§3.4）
- [ ] `DirtyRect`：`readonly struct`，闭区间本地坐标，`IsEmpty/Union(lx,ly,pad)/Union(DirtyRect)/Empty/Full`，钳制 `[0,63]`，零分配值语义。（§3.2）
- [ ] `Chunk.cs`：`Material/Flags/Lifetime` POH 数组（`GC.AllocateArray(pinned:true)`），`Coord`、`_current/_working`、`_incoming[8]`、`ChunkState`、基址封装（`MemoryMarshal.GetArrayDataReference`）。（§3.2）
- [ ] `Chunk` 池化复用接口（`Reset(ChunkCoord)`/归还），稳态零分配；供 plan/07 驻留装卸复用。（§3.2）
- [ ] `CellGrid.cs`：世界坐标门面 `GetMaterial/SetMaterial/FlagsAt/...`，路由 `IChunkSource`；持 `MaterialPropsTable` 只读引用。（§3.2）
- [ ] `IChunkSource.cs`（seam）：`bool TryGetChunk(ChunkCoord,out Chunk)`、`ReadOnlySpan<Chunk> ResidentChunks`、`ResolveNeighborhood(...)`（3×3 驻留，由 plan/07 实现，border ring 保证）。（§3.2/§3.7）

### 4.2 NeighborWindow 漫游
- [ ] `NeighborWindow.cs`：`ref struct`，9 槽 `ref` 基址（Material/Flags/Lifetime），中心世界基 `_baseCx/_baseCy`。（§3.3）
- [ ] `NeighborWindow` 构造：从 `IChunkSource.ResolveNeighborhood` 一次性取 3×3 基址，零分配。（§3.3）
- [ ] `GetMaterial/SetMaterial/GetFlags/SetFlags/GetLifetime` 世界坐标访问，用 `Unsafe.Add(ref SelectBase(slot),local)` 消除 bounds-check。（§3.3）
- [ ] `Swap(wx1,wy1,wx2,wy2)`：交换 Material+Flags(保留瞬时位策略)+Lifetime，返回是否跨界（供 KeepAlive）。（§3.3）
- [ ] Disasmo/`DOTNET_JitDisasm` 验证热访问 `RNGCHKFAIL` 消失（架构 §12.6/§17.3）。（§3.3）

### 4.3 单缓冲 + parity
- [ ] `SimulationKernel._currentParity` + 每 `StepCa` 翻转本帧 parity 值（不清扫，架构 §5.3）。（§3.4）
- [ ] 扫描跳过 `(flags&Parity)==currentBit` 的 cell（本帧已处理）。（§3.4）
- [ ] 移动/反应时把 cell（及目标/双输入双输出）parity 写为本帧值。（§3.4）
- [ ] 跨 chunk 边界对：反应执行打 parity，后续 pass 因 parity 跳过（防双侧翻倍，架构 §7.4）。（§3.4）

### 4.4 4-pass checkerboard 调度
- [ ] `CheckerboardScheduler.cs`：4 个预分配复用桶 `Chunk[][]+int[] counts`，按 `(cx&1,cy&1)` 装 awake chunk，清零复用（零分配）。（§3.5）
- [ ] pass A→D：每遍经 `JobSystem` per-chunk task 并行 `ChunkUpdater.UpdateChunk`，遍末 `JobSystem.Barrier()`。（§3.5）
- [ ] 无锁不依赖任何 cell 级锁（代码审查 + 注释引用 §5.7/§5.8）。（§3.5）
- [ ] 单线程回退：`awakeCount<SingleThreadChunkThreshold` 时同序串行 4-pass、无 barrier。（§3.5）
- [ ] 每 pass 向 Core 诊断登记分项耗时（pass A–D，架构 §17.1）。（§3.5）

### 4.5 movement rules
- [ ] `ChunkUpdater.UpdateChunk`：bottom-up（y 自 `MaxY` 递减）扫描 `_current`，按 `Type` 分派 mover；跳过 Empty/parity 已达本帧。（§3.6）
- [ ] `PowderMover`：候选 下/左下/右下，密度位移 `Density[t]<Density[s]`，左右偏置逐帧交替；平地不铺开（休止角）。（§3.6）
- [ ] `LiquidMover`：先 powder 规则；再水平铺开 `min(Dispersion,MoveCap)`，密度分层，无全局压力场。（§3.6）
- [ ] `GasMover`：上/上对角/侧向铺开 + Core RNG 抖动（上升扩散）。（§3.6）
- [ ] Fire/Energy：不 swap，仅 lifetime + 反应钩子；Empty/Solid 不动。（§3.6）
- [ ] 成功移动后：写双方 parity、grow 双方 chunk `_working`、按需 KeepAlive。（§3.6/§3.7/§3.8）
- [ ] movement 只读 `MaterialPropsTable` 的 `Density/Dispersion/Type`（不在本程序集定义其语义）。（§3.6）

### 4.6 32px move cap 与 KeepAlive
- [ ] 所有 mover 目标探测钳制在 `MoveCap=32`（halo）内；liquid dispersion 显式 `min(Dispersion,MoveCap)`。（§3.7）
- [ ] 跨界写入经 `NeighborWindow` 写邻居数组（halo 内合法），同时对该邻居 grow `_working` + KeepAlive。（§3.7）
- [ ] `Chunk._incoming[8]` 入站槽 + `opposite(dir)` 折算邻居本地坐标 `Union`（每槽单写者，无锁）。（§3.7）
- [ ] cache-line 填充 `_incoming`/per-chunk 调度元数据防 false sharing（架构 §12.7）。（§3.7）
- [ ] 反应/移动跨界写恒在 von Neumann/32px halo 内（断言 + 注释引用不变式 #4）。（§3.7）
- [ ] `IChunkSource` 解析 3×3 邻居失败时断言 + 诊断（border ring 契约，架构 §3.4）。（§3.7）

### 4.7 dirty-rect 生命周期
- [ ] grow：`set/move/react` 触及即 `Union(lx,ly,DirtyRectPadding)`（padding 可调）。（§3.8）
- [ ] `SwapDirtyRects`（相位 6，单线程）：`_current=_working∪merge(_incoming)`；`_working=Empty`；清 `_incoming`；空则 `Sleeping`。（§3.8）
- [ ] sleeping chunk 不入桶、不迭代（零成本）；唤醒后重入调度。（§3.8）

### 4.8 钩子（不定义表）
- [ ] `MaterialPropsTable.cs`：SoA 只读视图（`Type/Density/Dispersion/ReactionStart/ReactionCount/DefaultLifetime`），由 plan/04 注入。（§3.9）
...
- [ ] **节点 1**：`feat(sim): CellGrid SoA 与 Chunk/DirtyRect 数据结构 + 寻址`（§4.1，对应清单 4.1 全部）。
  - `feat(sim): 实现 CellGrid(SoA)、Chunk、DirtyRect 与世界坐标寻址`
- [ ] **节点 2**：`feat(sim): NeighborWindow 3x3 漫游与 bounds-check 消除`（§4.2）。
- [ ] **节点 3**：`feat(sim): 单缓冲原地更新 + parity 时钟位 + bottom-up movement`（§4.3、§4.5 单线程路径，含 powder/liquid/gas）。
- [ ] **节点 4**：`feat(sim): per-chunk dirty-rect grow/shrink/swap 与 sleep`（§4.7）。
- [ ] **节点 5**：`feat(sim): KeepAlive 跨界唤醒（入站槽无锁）与 32px move cap`（§4.6）。
- [ ] **节点 6**：`feat(sim): 4-pass checkerboard 多线程调度 + 单线程回退`（§4.4，接 plan/02 JobSystem）。
