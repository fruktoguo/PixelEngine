...
### 4.11 结构破坏 `Damage` lane（demo-playability，[!] 阻塞于字节预算评审）
- [!] **阻塞：等每-cell 字节预算评审**——`Chunk` 追加 `byte[] Damage`（4KB/chunk，POH，同缓冲原地）使 per-cell 4B→5B（+25%）、常驻 chunk sim 态 16KB→20KB，触及架构 §7.1/§12.2 承重墙；须先对该预算做显式修订/评审记录（plan/03/07/16 三处数字一致）。保留退化方案（不加 lane、破坏改为「单次 damage vs Hardness 概率判定」，省内存但无渐进损伤/裂纹）作被否落点。（§3.2/§3.12）
- [ ] `Damage` lane 默认 0；`SetMaterial`/`DepositCell`/反应/相变/破坏写新 cell 一律归 0（逐条覆盖 §3.12 清 lane 枚举）。（§3.12）
- [ ] `MaterialPropsTable` 增 `Hardness`/`MaxIntegrity`/`RubbleTarget` 三列（by-name→id 由 plan/04 加载期解析，本内核只读 id，守 #8）。（§3.9）
- [ ] `ApplyStructuralDamage(wx,wy,dmg)`：驻留检查 → `RigidOwned` 查验优先 → 抗性吸收 → 破坏判定 → 破坏动作（转 RubbleTarget/Empty + 清 Damage + parity + dirty + KeepAlive + 碎屑请求 + MineYield 事件）。（§3.12）
- [ ] `RigidOwned` cell 绝不累加 `Damage`，经 `IRigidDamageSink.OnOwnedCellDamaged` 路由 plan/06（守 #5）。（§3.12）
...
- [ ] `HardnessAbsorb`/`IntegrityScale` 等破坏常量集中于 `EngineConstants`（plan/00 §7），不写死。（§3.12）
- [ ] `ApplyStructuralDamage`/`DamageCircle`/`DamageBeam` 公开 API 带中文 XML 注释（供 plan/11 facade/脚本 IntelliSense）。（§3.10）
...
- [ ] **破坏归零转碎块**：cell 破坏后材质转 `RubbleTarget`（或 `Empty`）、`Damage` 归 0、标 dirty + 跨界 KeepAlive、按 `DebrisCount` 入碎屑请求、`MineYield>0` 发采集事件。（§3.12）
- [ ] **`Damage` 强制不变式**：任一帧后 `Damage≠0` 只出现在存活 `Solid`；movement swap 不搬运 `Damage`（移动类型残留恒 0）；测试钩子扫描无违背。（§3.12）
- [ ] **刚体像素不累计伤害**：破坏命中 `RigidOwned` cell 时经 `IRigidDamageSink` 路由、`Damage` 不累加（守 #5）。（§3.12）
...
- [ ] **清 `Damage` 全路径**：§3.12 枚举的 6 条清 lane 路径逐条被覆盖，无跨材质幽灵伤害残留；material remap fallback（plan/07）后 `Damage` 归 0。（§3.12）
...
- [!] **节点 9（阻塞于字节预算评审）**：`feat(sim): per-cell Damage lane + ApplyStructuralDamage/DamageCircle/DamageBeam 破坏内核`（§4.11，含 MaterialPropsTable 抗性列、RigidOwned 路由、强制不变式与清 lane 全路径、Simulation→Rendering 只读通路、Dispersion clamp/FlowRate 别名）。落地前须先完成架构 §7.1/§12.2 字节预算评审记录（plan/03/07/16 数字一致）。
