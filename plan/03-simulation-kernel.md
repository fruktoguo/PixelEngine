...
- [ ] `DamageCircle(cx,cy,radius,dmg,falloff)`/`DamageBeam(x,y,dx,dy,length,dmgPerCell)`：跨 chunk 只处理已驻留段、逐 chunk 合并 dirty + 边界 KeepAlive；安全相位离散编辑不受 #4 halo 约束。（§3.12/§3.10）
- [ ] 强制不变式落地：`Damage≠0` 仅存活 `Solid`；swap 不搬运 `Damage`；断言 + 测试钩子校验（movement 后无移动类型残留 Damage）。（§3.12）
- [ ] Simulation→Rendering `Damage` 只读通路（`DamageSpan`/快照扩展）供 plan/08 裂纹叠色，只读不写（守 #7）。（§3.12）
- [ ] `Explode` 重构为组合 `DamageCircle` 的登记与底座（组合改写在 plan/05/13）。（§3.12）
...
- [ ] **破坏抗性差异化**：同一 `DamageCircle(dmg=X)` 下 `sand/dirt`（低 Hardness、MaxIntegrity 0/小）即时抛碎、`stone`（高 Hardness、大 MaxIntegrity）需累计多次/大当量、`metal` 近免疫小爆破；差异全由 `MaterialPropsTable` 抗性数据表达（改数值即改表现）。（§3.12）
...
- [ ] **`Damage` 强制不变式**：任一帧后 `Damage≠0` 只出现在存活 `Solid`；movement swap 不搬运 `Damage`（移动类型残留恒 0）；测试钩子扫描无违背。（§3.12）
- [x] **刚体像素不累计伤害**：破坏命中 `RigidOwned` cell 时经 `IRigidDamageSink` 路由、`Damage` 不累加（守 #5）。（§3.12）
- [ ] **跨 chunk 破坏裁剪**：大半径 `DamageCircle`/长 `DamageBeam` 跨多 chunk 时只作用已驻留段、逐 chunk 合并 dirty + KeepAlive，未驻留段安全跳过；离散安全相位编辑写入范围可超 32px 而不违 #4（#4 只约束并行 pass）。（§3.12/§3.10）
- [x] **清 `Damage` 全路径**：§3.12 枚举的 6 条清 lane 路径逐条被覆盖，无跨材质幽灵伤害残留；material remap fallback（plan/07）后 `Damage` 归 0。（§3.12）
- [ ] **`Damage` lane 零分配**：稳态 `ApplyStructuralDamage`/`DamageCircle` 无托管堆分配（`[MemoryDiagnoser]`）。（§3.12）
- [ ] **Dispersion clamp/FlowRate 别名**：加载期 `Dispersion` clamp 到 `[0,MoveCap]`、`FlowRate` 为其对外别名不新增字段；液体单步水平位移 ≤32px（守 #4）。（§3.6）
...
- [~] **节点 9（进行中）**：`feat(sim): per-cell Damage lane + ApplyStructuralDamage/DamageCircle/DamageBeam 破坏内核`（§4.11，已完成字节预算评审、Damage lane、抗性列、RigidOwned 路由、存档契约、清 lane 全路径与 plan/05 碎屑消费联动；仍待 Simulation→Rendering 只读通路、Dispersion clamp/FlowRate 别名与零分配基准）。
