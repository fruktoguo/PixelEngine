...
- **[!] `byte[] Damage`（4KB，等每-cell 字节预算评审）**：per-cell 累计结构伤害（`0`=未损；**非**满血值——满血阈值由 `plan/04` 材质字段 `MaxIntegrity` 表达）。默认 0，仅在单线程输入/安全相位经 `ApplyStructuralDamage`（§3.12）原地累加；写新 cell 时归 0。**只允许存在于不参与 movement 的存活 `Solid`**（强制不变式，§3.12）；`Powder/Liquid/Gas` 因 `MaxIntegrity==0` 即时破坏、永不累计。渲染相位 9（plan/08）只读此 lane 做裂纹叠色（§3.12 只读通路，不写回 cell，守 #7）。此 lane 触及架构 §7.1/§12.2 每-cell 字节预算承重墙（见下），**引入前须过显式预算评审**（§4.11）。
- **注意**：`Temperature`（1/4 分辨率热场）与 `Render`（BGRA8）**不在本文件**——温度场属相位 5（plan/04），render buffer 属相位 9（plan/08）。本内核既不分配也不存储它们（不变式 #7）。

每 chunk sim 态 ≈ 16KB（架构 §12.2），加元数据 ~18–20KB；与 plan/07 内存上限预算一致。**破坏模型引入 `Damage` lane 后**（§3.12，等字节预算评审）：per-cell **4B→5B（+25%）**、每常驻 chunk sim 态 **16KB→20KB**（`Material`8KB+`Flags`4KB+`Lifetime`4KB+`Damage`4KB），加元数据 ~22–24KB。此增量触及架构 §7.1/§12.2 字节预算承重墙，须对该预算做显式修订/评审记录后方可落地；**三处数字（plan/03 此处、plan/07 存档预算、plan/16 内存加固）须严格一致**。
...
- `ChunkCoord Coord`；`ushort[] Material; byte[] Flags; byte[] Lifetime;`（POH）；**破坏模型落地后追加 `byte[] Damage;`（POH，同缓冲原地，§3.12，等字节预算评审）**。
...
### 3.12 结构破坏模型：`Damage` lane 与 `ApplyStructuralDamage`（demo-playability 增补，[!] 阻塞于字节预算评审）

> 本节定义**内核侧**的持久破坏机制：per-cell `Damage` 累计伤害 lane（§3.2）+ 消费材质抗性（§3.9 新增 `Hardness`/`MaxIntegrity`/`RubbleTarget`）的破坏例程。材质字段语义与数值在 plan/04；碎屑粒子在 plan/05；`Damage` lane 存档契约在 plan/07；裂纹着色在 plan/08；武器/可玩循环在 plan/13。本节只落「lane 数据规则 + 破坏判定/动作 + 跨界/刚体路由 + 清 lane 全路径」。**整节触及架构 §7.1/§12.2 每-cell 字节预算承重墙，标 [!] 阻塞、须先过预算评审（见 §3.2 与 §4.11）。** 命名说明：本 lane 记录**累计已受伤害**（`Damage`），与 plan/04 材质字段 `MaxIntegrity`（满血阈值）配对；design 稿曾称其「Integrity lane」，语义同一，本内核统一用 `Damage` 以区分「累计伤害」与「满血阈值」。
...
### 4.11 结构破坏 `Damage` lane（demo-playability）
...
破坏模型（demo-playability，等 `Damage` lane 落地）验收增补：
...
破坏模型（demo-playability，§3.12，等字节预算评审）新增依赖：
