...
- [~] `ApplyStructuralDamage(wx,wy,dmg)`：驻留检查 → `RigidOwned` 查验优先 → 抗性吸收 → 破坏判定 → 破坏动作（转 RubbleTarget/Empty + 清 Damage + parity + dirty + KeepAlive + 碎屑请求 + MineYield 事件）。已落地内核 destruction sink 与 `CellDamageRubbleHandshakeTests`，覆盖 `DebrisCount`/`MineYield` 事件载荷；待 plan/05 `RequestDebris` 消费接入后闭合碎屑抛射请求。（§3.12）
...
- [~] **破坏归零转碎块**：cell 破坏后材质转 `RubbleTarget`（或 `Empty`）、`Damage` 归 0、标 dirty + 跨界 KeepAlive、按 `DebrisCount`/`MineYield` 发布 destruction event 已由 `CellDamageRubbleHandshakeTests` 覆盖；待 plan/05 `RequestDebris` 把 event 消费为真实碎屑粒子后闭合。（§3.12）
...
- [~] **节点 9（进行中）**：`feat(sim): per-cell Damage lane + ApplyStructuralDamage/DamageCircle/DamageBeam 破坏内核`（§4.11，已完成字节预算评审、Damage lane、抗性列、RigidOwned 路由、存档契约与清 lane 全路径；仍待 Simulation→Rendering 只读通路、Dispersion clamp/FlowRate 别名、碎屑/MineYield 联动与零分配基准）。
