...
- [x] 本文件承载 **Engine Core** 的 CPU 权威 CA 内核：64×64 chunk、单缓冲原地更新、per-chunk dirty rect、4-pass checkerboard、parity、32px move cap、KeepAlive 与相位 3/4/6/7 seam。
- [x] 本文件将早期 Demo 可玩性破坏口径抽象为 Engine Core 通用结构破坏能力：`Damage` lane、`ApplyStructuralDamage`、`DamageCircle`、`DamageBeam`、刚体损伤路由与碎屑 / 采集事件入口。
- [x] 本文件不承载材质字段语义、JSON 加载、温度扩散、自由粒子积分、渲染颜色、世界驻留、刚体物理或 Demo 玩法；这些由对应 leaf plan 实现并通过 seam 消费。

### 0.2 状态总览 checklist

- [x] CellGrid / Chunk SoA、NeighborWindow、单缓冲 + parity、4-pass checkerboard、movement rules、KeepAlive、dirty rect、相位接口与测试钩子已完成。
- [x] `Damage` lane 已落地：per-cell 4B→5B，常驻 chunk sim 态 16KB→20KB；预算口径要求与 plan/07 / plan/16 一致。
- [x] 结构破坏 API 已落地并登记证据：`ApplyStructuralDamage`、`DamageCircle`、`DamageBeam`、`IRigidDamageSink`、`Damage` 只读渲染通路。
- [x] 破坏与旧无条件 `Explode` 抛射语义已迁移为破坏驱动：抗性判定归材质数据，碎屑抛射归 plan/05，武器 facade 归 plan/11 / plan/13。
- [!] CA full-active 吞吐、目标硬件帧预算、硬件计数器与长跑性能证据仍归 plan/16 / M15；功能完成不等于性能目标达标。

### 0.3 已实现证据 checklist

- [x] Simulation 仅引用 Core、不反向引用 Rendering / Physics / Content / World 的依赖方向已作为 ProjectReference 证据登记。
- [x] `WorldEffectBoundaryConservationTests`、`CellDamageRubbleHandshakeTests`、`CellDamageResistanceTests`、`SimulationDataStructureTests`、`RenderBufferBuilderTests`、`ChunkSnapshotTests` 等已作为结构破坏与边界守恒证据登记。
- [x] `DispersionMoveCapTests` 与 `MaterialContentLoaderTests` 已作为 `Dispersion` clamp / `FlowRate` 别名和 32px move cap 合规证据登记。
- [x] `WorldEffectBoundaryConservationTests.StructuralDamageEntriesDoNotAllocateAfterWarmup` 已作为结构破坏零分配证据登记。
- [x] 反汇编 / BenchmarkDotNet / MemoryDiagnoser 证据已登记用于 bounds-check 消除、稳态 `StepCa` 零分配与单线程回退。

### 0.4 未完成目标 checklist

- [ ] 若未来新增 per-cell lane，必须重新做字节预算评审并同步 plan/00、plan/03、plan/07、plan/16，不能静默突破 5B/cell。
- [ ] 若未来把确定性模式产品化，需要由 plan/14 增加跨平台 oracle / replay / seed 稳定性验证；本文只保留确定性 seam 与测试钩子。

### 0.5 证据债 / 阻塞 checklist

- [!] 当前 CA 内核功能闭合不代表达到架构目标的 2–4M active cells / 8ms；该目标仍需 plan/16 目标硬件长跑和优化证据。
- [!] 本地短跑、scripted probe、局部 BenchmarkDotNet 不能替代目标硬件与真实 Demo 场景性能验收。
- [!] `Damage` lane 已增加 25% cell 内存预算，后续必须在 plan/16 继续跟踪 ResidentMemoryBytes、cache miss 与目标世界规模压力。
...
- [x] 结构破坏重点过滤：`DamageCircleDifferentiatesMaterialResistanceFromData|RunEjectionPassClearsSourceCellAndSpawnsParticle|WorldEffectBoundaryConservationTests|CellDamageRubbleHandshakeTests`。
- [x] 性能 / 零分配：`bench/PixelEngine.Benchmarks` 对应 Simulation / structural damage filters + BenchmarkDotNet MemoryDiagnoser。
- [!] M15 性能证据：目标硬件 CA throughput、hardware counters、长跑稳定性与 frame budget 报告。

### 0.7 依赖与下一闭合节点 checklist

- [x] 上游依赖：plan/01 工程骨架、plan/02 Core。
- [x] 下游消费：plan/04 材质语义、plan/05 粒子生命周期、plan/06 刚体损伤路由、plan/07 存档 Damage lane、plan/08 裂纹叠色、plan/11 / plan/13 武器 facade、plan/14 测试、plan/16 性能。
- [!] 下一闭合节点是 plan/16 的目标硬件性能债与 plan/13 / plan/20 的真实产品场景验证，不是继续扩大内核范围。
...
- 字段：`ref ushort _matBase0..8`、`ref byte _flagsBase0..8`、`ref byte _lifeBase0..8`（9 槽，slot = `(dcy+1)*3 + (dcx+1)`，`dcx,dcy ∈ {-1,0,1}`），中心 chunk 世界基 `_baseCx,_baseCy`。8 邻居由 border ring 保证驻留（架构 §3.4）；checkerboard 装桶阶段解析并验证 `ChunkNeighborhood`，热路径构造 `NeighborWindow` 时复用该邻域，不重复查表。
...
- **Powder**（`PowderMover`）：候选偏移有序 `正下自由落体(0,+n)`→`左下(-1,+1)`→`右下(+1,+1)`。正下方向连续探测最多 `MoveCap` 格，移动到同列最低可达空位，或在遇到第一个更轻且未被本帧处理的 cell 时执行密度置换；斜下仍只尝试一格。左下/右下的**先后顺序逐帧交替偏置**（按 `_currentParity` 或 per-row 计数翻转）消除左/右倾。平地**不**水平铺开 ⇒ 休止角（angle of repose）自然涌现（架构 §5.6/§1.2）。单次自由落体仍严格受 `MoveCap` 约束，满足不变式 #4。
...
**`Dispersion` 语义收紧与 `FlowRate` 别名（守 #4）**：真实的 32px move-cap 约束在 movement 层**已由** `min(Dispersion[material], MoveCap)`（liquid 水平铺开）与正下自由落体的 `min(距离, MoveCap)` 强制，**本内核不因 M14 showcase 支撑需求放宽此钳制**。plan/04 在加载期把 `Dispersion` 再 clamp 到 `[0, EngineConstants.MoveCap]` 只是**语义收紧**（保证数据源头即合规、失配数据不进热表），并把玩家/编辑器可读的 `FlowRate` 作为 `Dispersion` 的**对外语义别名**——**不新增独立 `flowRate` 字段、不改热表布局**，movement 热路径仍只读单一 `Dispersion` 列。任何声称的水平流速最终都收敛到 `min(Dispersion,MoveCap)`，不变式 #4 不被穿透。

### 3.7 32px move cap 与 KeepAlive 跨界唤醒（不变式 #4/#5，架构 §5.5/§5.8）

**32px move cap**（`EngineConstants.MoveCap=32`，架构 §5.8）：单次更新中任一像素净位移 ≤ 32px。powder 正下自由落体与 liquid dispersion 都显式 `min(距离, MoveCap)` 钳制；任何 mover 的目标探测都不超出中心 chunk 各方向 32px halo。**这是 checkerboard 无锁成立的承重墙**：halo(32)<chunk 间隔(64)，故同遍两线程写区域不重叠（§3.5）。反应只触 von Neumann 邻居（距离 1），远在 halo 内，跨界反应写入同样安全（不变式 #4、架构 §5.8/§7.4）。
...
- `MaterialPropsTable`（`MaterialPropsTable.cs`）：内核消费的**热 SoA 只读视图**，由 plan/04（Content）在加载期构建并注入。字段（按材质 id 索引）：`CellType[] Type`、`byte[] Density`、`byte[] Dispersion`、`int[] ReactionStart`、`byte[] ReactionCount`、`ushort[] DefaultLifetime`；**破坏模型追加三列（等字节预算/评审落地）：`byte[] Hardness`（抗性阈值，吸收小于阈值的单次伤害）、`ushort[] MaxIntegrity`（该材质固体满结构完整度，`0`=即时破坏）、`ushort[] RubbleTarget`（破坏产物材质 id，由 plan/04 在加载期把稳定 name→id 解析后填入，守 #8，本内核只读 id、绝不解析字符串）**。**字段语义全在 plan/04 定义**，本内核只读其值驱动 movement、反应钩子与 `ApplyStructuralDamage`（§3.12）；SoA 而非传完整 `MaterialDef` 是为 cache 友好（避免把未用字段拉进 cache line，架构 §7.1）。渲染专属的 `RenderStyle`/描边/流动等视觉字段**不进本表**（走 plan/08 自有材质视图，颜色不入内核，守 #7）。
...
- **[相位 1/3 安全相位] 结构破坏离散编辑入口**（Engine Core 通用结构破坏能力 / M14 showcase 支撑能力，§3.12）：`void ApplyStructuralDamage(int wx,int wy,int dmg)`、`void DamageCircle(int cx,int cy,int radius,int dmg,float falloff)`、`void DamageBeam(int x,int y,int dx,int dy,int length,int dmgPerCell)`。与 `DepositCell`/编辑器编辑同类——**单线程离散写入、不在并行 CA pass 内**，故**不受 32px halo 约束**（#4 只约束并行 movement/reaction 的跨界写；离散安全相位编辑可跨任意多 chunk）。实现只对**已驻留** chunk 生效（`IChunkSource.TryGetChunk` 失败即跳过该 cell/段），逐 chunk 累积并合并 dirty rect + 边界 KeepAlive。供 plan/05 粒子/plan/11 facade 的武器区域伤害/光束伤害消费。
...
### 3.12 结构破坏模型：`Damage` lane 与 `ApplyStructuralDamage`（Engine Core 通用结构破坏能力 / M14 showcase 支撑能力，字节预算评审已落地）

> 本节定义**内核侧**的持久破坏机制：per-cell `Damage` 累计伤害 lane（§3.2）+ 消费材质抗性（§3.9 新增 `Hardness`/`MaxIntegrity`/`RubbleTarget`）的破坏例程。材质字段语义与数值在 plan/04；碎屑粒子在 plan/05；`Damage` lane 存档契约在 plan/07；裂纹着色在 plan/08；武器/可玩循环在 plan/13。本节只落「lane 数据规则 + 破坏判定/动作 + 跨界/刚体路由 + 清 lane 全路径」。整节触及架构 §7.1/§12.2 每-cell 字节预算承重墙，预算评审与实现证据见 §4.11。命名说明：本 lane 记录**累计已受伤害**（`Damage`），与 plan/04 材质字段 `MaxIntegrity`（满血阈值）配对；design 稿曾称其「Integrity lane」，语义同一，本内核统一用 `Damage` 以区分「累计伤害」与「满血阈值」。

**强制不变式（新增，全内核破坏语义的地基）**：`Damage ≠ 0` 只允许存在于**参与不了 movement 的存活 `Solid` cell**。推论与保证：

- `Powder/Liquid/Gas`（会 swap 移动的类型）由 plan/04 强制 `MaxIntegrity==0` ⇒ 任意有效伤害即时破坏、`Damage` 永不累计，故 movement swap 搬运的 cell 其 `Damage` 恒为 0；swap 因此**无需搬运 `Damage`**（`NeighborWindow.Swap` 不触 `Damage` lane，热路径零改动）。
- `Solid` 不参与 movement（§3.6 只递减 lifetime / 触反应，不 swap），故带 `Damage` 的 cell 原地不动，`Damage` 与该 cell 生命周期绑定。
- 任何把 cell **变成新 cell** 的动作都必须把该位置 `Damage` 归 0（下方枚举），保证不变式不被违反、不产生「幽灵伤害」跨材质残留。

**`ApplyStructuralDamage(wx, wy, dmg)`（安全相位、单线程、离散编辑）**：

1. 经 `CellGrid`/`IChunkSource` 解析目标 chunk；**未驻留则直接跳过**（破坏只作用于已驻留世界，不触发流式装载）。
2. **先查 `CellFlags.RigidOwned`**：若命中（该 cell 属刚体像素，语义归 plan/06），**绝不在其上累加 `Damage`**，改经 `IRigidDamageSink.OnOwnedCellDamaged(wx,wy)` 路由到 plan/06 的 `RigidDamageQueue` 触发刚体形状重建（守 #5），本例程对该 cell 到此为止。
3. 读材质 `id` 与 `MaterialPropsTable` 的 `Hardness[id]`/`MaxIntegrity[id]`：`Empty`/静态不可破材质（plan/04 `Indestructible`）直接早退。
4. 抗性吸收：`effective = max(0, dmg - Hardness[id]*EngineConstants.HardnessAbsorb)`（`HardnessAbsorb` 为集中常量，不写死）；`effective==0` 早退（小伤害被硬材质完全吸收，不掉血）。
5. 破坏判定：`MaxIntegrity[id]==0` ⇒ 任意 `effective>0` **即时破坏**；否则 `Damage[local] = min(255, Damage[local] + scaledEffective)`（`IntegrityScale` 常量把 byte lane 映射到 `ushort MaxIntegrity` 域），当 `Damage*IntegrityScale ≥ MaxIntegrity` **触发破坏**，否则仅累计（cell 存活但受损，供渲染裂纹）。
6. **破坏动作**（一组原子操作，单缓冲原地）：把该 cell 材质写为 `RubbleTarget[id]`（`0`=`Empty` 即清空）；`Damage[local]=0`（新材质从满血起算）；写本帧 parity；grow 所属 chunk `_working` dirty rect；跨界则 KeepAlive 唤醒邻居（§3.7）；按材质 `DebrisCount` 向 plan/05 入**碎屑抛射请求**（cell→particle handshake，复用相位 7 语义但由破坏触发）；按材质 `MineYield>0` 发一次**采集事件**（供 plan/13 `GameDirector` 计采晶）。碎屑请求与采集事件经既有延迟命令/事件队列，不在本例程内直接跨系统调用。

**`DamageCircle` / `DamageBeam`（区域/光束破坏，跨 chunk 裁剪）**：

- `DamageCircle(cx,cy,radius,dmg,falloff)`：遍历 `[cx±radius, cy±radius]` 包围盒内 cell，按到圆心距离衰减 `dmg` 后对每 cell 调 `ApplyStructuralDamage`。大半径**跨多 chunk**：按 chunk 分块遍历，**只处理已驻留 chunk**（未驻留段整体跳过），每 chunk 内的破坏累积到该 chunk 的 dirty rect，**逐 chunk 合并** dirty 与边界 KeepAlive（不做一次性全局 rect，避免跨界 dirty 越权）。
- `DamageBeam(x,y,dx,dy,length,dmgPerCell)`：沿方向逐 cell 步进 `length` 格，对每个已驻留 cell 调 `ApplyStructuralDamage`（配套注热由 plan/04 `AddHeat` 单独走，本内核只做破坏部分）；穿透遇 `Indestructible`/未驻留即停或跳段。同样逐 chunk 合并 dirty。
- **不受 #4 halo 约束**：`DamageCircle/DamageBeam` 是单线程安全相位的**离散编辑**（同 `DepositCell`/编辑器画刷），不在并行 checkerboard pass 内执行，其写入范围可远超 32px——#4 只约束**并行 movement/reaction** 的跨界写落在 halo 内，离散安全相位编辑不在此约束域（见 §3.10）。破坏后受影响 chunk 下帧由 CA 重新迭代（dirty 已 grow + KeepAlive），坍塌/流动自然接管。

**`Explode` 重构为破坏驱动（登记，组合改写落点在 plan/05/13）**：现有「无条件抛射半径内全部 cell」的 `Explode`/`EjectionRequest` 改为内部组合 `DamageCircle`——半径内按材质抗性差异化破坏，破坏动作再触发碎屑抛射；由此 `sand/dirt` 一炸即碎、`stone` 需累计/大当量、`metal` 近免疫小爆破，**抗性真正生效**。本内核只提供 `DamageCircle` 底座；`Explode` 的组合改写与既有 `ExplosiveTool`/`PlayableProjectile` 抛射断言测试迁移在 plan/05/13/14 登记。

**Simulation→Rendering `Damage` 只读通路（供 plan/08 裂纹叠色）**：内核导出**只读** `Damage` lane 视图（`ReadOnlySpan<byte> DamageSpan(ChunkCoord)` 或经 `SnapshotChunk` 扩展），供渲染相位 9 按 `Damage/MaxIntegrity` 损伤度叠加暗色裂纹（plan/08 §3.4）。渲染**只读不写**（守 #7，颜色/裂纹不入 cell）；通路与现有只读 `Material`/`Flags` 消费同构，仍是 Rendering 只读消费 Simulation 暴露的 span，不引入 Simulation→Rendering 反向依赖。

**清 `Damage` 路径完整枚举**（保证强制不变式恒成立，实现须逐条覆盖）：

1. `SetMaterial`/`DepositCell`（相位 3 沉积）写入新材质 ⇒ 目标 `Damage=0`。
2. movement swap **不搬运** `Damage`（被搬运类型 `Damage` 恒 0，见不变式），swap 目标位无残留。
3. 反应/相变生成新材质（`IReactionExecutor`/`ILifetimeSink.OnExpired` 改写 cell）⇒ 新 cell `Damage=0`（钩子约定：改写材质即清 `Damage`）。
4. 破坏动作转 `RubbleTarget`/`Empty` ⇒ `Damage=0`（上述第 6 步）。
5. 刚体像素（`RigidOwned`）**从不累计** `Damage`（第 2 步路由）；plan/06 把刚体像素重新光栅化回 cell 时按写新 cell 处理 ⇒ `Damage=0`。
6. plan/07 存档读档 material remap 遇缺失材质 fallback 后 ⇒ 该 cell `Damage` 清 0（存档契约在 plan/07，本内核提供清 lane 入口）。
...
- [x] `NeighborWindow` 构造：复用 checkerboard 装桶阶段已解析的 `ChunkNeighborhood` 取得 3×3 基址，保留 `IChunkSource.ResolveNeighborhood` 直接构造入口，零分配。（§3.3）
...
- [x] 反应时把双输入/双输出 parity 写为本帧值。（§3.4/§3.9）
- [x] 跨 chunk 边界对：反应执行打 parity，后续 pass 因 parity 跳过（防双侧翻倍，架构 §7.4）。（§3.4）
...
- [x] `PowderMover`：候选 正下最多 `MoveCap` 自由落体/左下/右下，密度位移 `Density[t]<Density[s]`，左右偏置逐帧交替；平地不铺开（休止角）。（§3.6）
...
- [x] Fire/Energy lifetime + 反应钩子。（§3.6/§3.9）
- [x] 成功移动后：写双方 parity、grow 双方 chunk `_working`。（§3.6/§3.8）
- [x] 成功跨界移动后按需 KeepAlive。（§3.6/§3.7）
...
- [x] 反应跨界写恒在 von Neumann halo 内（断言 + 注释引用不变式 #4）。（§3.7/§3.9）
...
- [x] `IReactionExecutor.cs`（seam）：`TryReact(...)`；movement 后对 4 von Neumann 邻居对调用（`ReactionCount>0` 才调，惰性早退）。（§3.9）
- [x] lifetime：`Lifetime>0` 每 tick 递减；归零调 `ILifetimeSink.OnExpired`（seam）。（§3.9）
- [x] 不内嵌相变/温度数学（相位 5 在 plan/04），仅保证相位隔离。（§3.9）

### 4.9 帧相位接口
- [x] `SimulationKernel.DepositCell/MarkDirty`（相位 3 入口，供 plan/05）。（§3.10）
- [x] `SimulationKernel.StepCa(JobSystem)`（相位 4）。（§3.10）
- [x] `SimulationKernel.SwapDirtyRects()`（相位 6）。（§3.10）
- [x] `SimulationKernel.ReadAndClearCell(...)`（相位 7 入口，供 plan/05）。（§3.10）
- [x] 公开 API 全带中文 XML 文档注释（脚本 IntelliSense 依赖，`AGENTS.md §4`、plan/00 §7）。（§3.10）

### 4.10 测试钩子与诊断
- [x] `InternalsVisibleTo("PixelEngine.Simulation.Tests")`；`CountNonEmptyCells/SnapshotChunk`。（§3.11）
- [x] KeepAlive 计数器 + 被唤醒边界 chunk 列表导出（§17.2 叠层 + plan/14）。（§3.11）
- [x] 确定性模式开关（强制单线程 + 固定 RNG/parity 序，架构 §6.2），供 oracle 比对。（§3.11）
- [x] 反应守恒探针（每帧反应次数 + 边界反应对记录）。（§3.11）
- [x] 稳态帧零托管分配（`BenchmarkDotNet` `[MemoryDiagnoser]` 验证，`AGENTS.md §3`）。（§2/§3.5）

### 4.11 结构破坏 `Damage` lane（Engine Core 通用结构破坏能力 / M14 showcase 支撑能力）
- [x] **per-cell 字节预算评审落地**——`Chunk` 追加 `byte[] Damage`（4KB/chunk，POH，同缓冲原地）使 per-cell 4B→5B（+25%）、常驻 chunk sim 态 16KB→20KB；已同步 plan/03/07/16 预算口径与 `ChunkMemoryBudget`，本轮选择保留渐进损伤/裂纹能力，不采用概率退化方案。（§3.2/§3.12）
- [x] `Damage` lane 默认 0；`SetMaterial`/`DepositCell`/反应/相变/破坏写新 cell 一律归 0（逐条覆盖 §3.12 清 lane 枚举）。（§3.12）
- [x] `MaterialPropsTable` 增 `Hardness`/`MaxIntegrity`/`RubbleTarget` 三列（by-name→id 由 plan/04 加载期解析，本内核只读 id，守 #8）。（§3.9）
- [x] `ApplyStructuralDamage(wx,wy,dmg)`：驻留检查 → `RigidOwned` 查验优先 → 抗性吸收 → 破坏判定 → 破坏动作（转 RubbleTarget/Empty + 清 Damage + parity + dirty + KeepAlive + 碎屑请求 + MineYield 事件）。已落地内核 destruction sink 并由 `ParticleSystem.RequestDebris` 消费；`CellDamageRubbleHandshakeTests` 覆盖 `DebrisCount`/`MineYield` 事件载荷、真实碎屑粒子生成、未破坏不抛与 DebrisCount=0 不抛。（§3.12）
- [x] `RigidOwned` cell 绝不累加 `Damage`，经 `IRigidDamageSink.OnOwnedCellDamaged` 路由 plan/06（守 #5）。（§3.12）
- [x] `DamageCircle(cx,cy,radius,dmg,falloff)`/`DamageBeam(x,y,dx,dy,length,dmgPerCell)`：跨 chunk 只处理已驻留段、逐 chunk 合并 dirty + 边界 KeepAlive；安全相位离散编辑不受 #4 halo 约束。证据：`WorldEffectBoundaryConservationTests`、`CellDamageRubbleHandshakeTests`。（§3.12/§3.10）
- [x] 强制不变式落地：`Damage≠0` 仅存活于结构破坏允许的 `Solid`/`Powder` 且受 `MaxIntegrity` 约束；非破坏目标、`RigidOwned`、`Indestructible` 清 `Damage`；swap 不搬运 `Damage`。证据：`SimulationDataStructureTests.ApplyStructuralDamageDestroysSolidAndRoutesRigidOwned`、`NeighborWindowReadsWritesAndSwapsAcrossNeighborhood`、`CellDamageResistanceTests`。（§3.12）
- [x] Simulation→Rendering `Damage` 只读通路（`Chunk.Damage`/`ChunkSnapshot.Damage`/渲染构建读取）供 plan/08 裂纹叠色，只读不写（守 #7）。证据：`RenderBufferBuilderTests.BuildAppliesRenderStylesAndDamageWithoutMutatingCells`、`ChunkSnapshotTests`。（§3.12）
- [x] `Explode` 重构为组合 `DamageCircle` 的登记与底座（组合改写在 plan/05/13/11）。（§3.12）证据：`ScriptSimulationContextTests.WorldExplodeFlushesStructuralDamageDebrisAndRigidImpulse`、`PlayerControllerIntegrationTests.ExplosiveToolMiddleClickDamagesCellsAndQueuesLighting`。
- [x] `HardnessAbsorb`/`IntegrityScale` 等破坏常量集中于 `EngineConstants`（plan/00 §7），不写死。（§3.12）
- [x] `ApplyStructuralDamage`/`DamageCircle`/`DamageBeam` 公开 API 带中文 XML 注释（供 plan/11 facade/脚本 IntelliSense）。（§3.10）

---

## 5. 验收标准

- [x] **质量守恒（无反应）**：纯 movement 下全网格非空 cell 总数恒定，**含跨 chunk 边界**（捕获缝隙吞/复制像素，架构 §16.2，测试在 plan/14）。
- [x] **质量守恒（跨界移动）**：一柱沙/一片水跨越 chunk 边界下落/铺开，无消失、无复制、无抖动（不变式 #4/#5、架构 §19 R2）。
- [x] **parity 防重复**：单帧内任一 cell 至多移动一次、至多反应一次（构造「会被二次扫到」的场景验证 parity 跳过）。
- [x] **反应守恒（边界）**：双输出/定向反应在 chunk 边界对上**不翻倍、不丢失**（谁先扫到谁执行、另一侧 parity 跳过，架构 §7.4/§16.2）。
- [x] **KeepAlive 跨界唤醒**：雪崩/液体流跨 chunk 边界正确传播，不在缝隙处死掉；被触碰邻居下帧被重检（专项测试，架构 §5.5/§16.2）。
- [x] **休止角**：粉末在平地堆出稳定休止角、不水平自流；左右偏置交替无单侧漂移（架构 §5.6）。
- [x] **液体找平/分层**：水按 dispersion 水平找平；油浮于水（密度位移正确，架构 §5.6/§7.3）。
- [x] **气体上升扩散**：气体上翻 + 抖动扩散，非干净柱状（架构 §5.6）。
- [x] **bottom-up 一帧坍塌**：单柱沙在一遍扫描内自然坍塌到底（架构 §5.6）。
- [x] **静止近零成本**：完全沉降区 chunk `_current` 收缩为空并 `Sleeping`，下帧不迭代；活跃成本随活跃 chunk 数而非屏幕面积缩放（架构 §5.4，HUD/基准验证）。
- [x] **4-pass 无锁正确**：多线程结果与单线程 oracle 在统计性质上一致（质量守恒/堆形/分层）；无边界竞争损坏（架构 §16.2）。
- [x] **单线程回退**：活跃 chunk 低于阈值时回退单线程，行为与多线程一致、无 barrier 浪费（架构 §5.7/§19 R7）。
- [x] **32px move cap**：任一像素单 tick 净位移 ≤32px；liquid dispersion 被钳制；halo<间隔成立（不变式 #4，断言 + 测试）。
- [x] **跨界写恒在 halo 内**：所有移动/反应跨界写入落在 32px halo（von Neumann 反应距离 1），无越界（不变式 #4）。
- [x] **dirty-rect 双缓冲**：working/current 正确交换；grow padding 可调；空则 sleep、非空则 awake（架构 §5.4）。
- [x] **相位接口正确**：相位 3 沉积本帧即被 CA 看见；相位 7 抛射不被本帧 CA 误处理；相位 6 在帧边界单线程交换（架构 §3.3）。
- [x] **反应/lifetime 钩子**：`ReactionCount==0` 材质一次比较早退、不触反应数据；lifetime 归零调 `OnExpired`（架构 §7.4）。
- [x] **bounds-check 消除**：热路径反汇编无 `RNGCHKFAIL`（Disasmo/`DOTNET_JitDisasm` 证实，架构 §12.6/§17.3）。
- [x] **零分配**：稳态 `StepCa`+`SwapDirtyRects` 帧无托管堆分配（`[MemoryDiagnoser]`，`AGENTS.md §3`）。
- [x] **不变式合规**：单缓冲（#3）、4-pass 无锁（#2）、四基石（#1）、跨界 halo（#4）、颜色不入 cell（#7）均被代码与注释落实，无违背。
- [x] **依赖方向**：`Simulation` 仅引用 `Core`，不引用 Rendering/Physics/Content/World（ProjectReference 强制，plan/00 §5）。

破坏模型（Engine Core 通用结构破坏能力 / M14 showcase 支撑能力）验收增补：

- [x] **破坏抗性差异化**：同一 `DamageCircle(dmg=X)` 下 `sand/dirt`（低 Hardness、MaxIntegrity 0/小）即时抛碎、`stone`（高 Hardness、大 MaxIntegrity）需累计多次/大当量、`metal` 近免疫小爆破；差异全由 `MaterialPropsTable` 抗性数据表达（改数值即改表现）。证据：`CellDamageResistanceTests.DamageCircleDifferentiatesMaterialResistanceFromData`。（§3.12）
- [x] **破坏归零转碎块**：cell 破坏后材质转 `RubbleTarget`（或 `Empty`）、`Damage` 归 0、标 dirty + 跨界 KeepAlive、按 `DebrisCount`/`MineYield` 发布 destruction event 并由 `ParticleSystem.RequestDebris` 消费为真实碎屑粒子；`CellDamageRubbleHandshakeTests` 覆盖 rubble 不被二次清空、碎屑材质、DebrisCount=0 不抛与采集事件载荷。（§3.12）
- [x] **`Damage` 强制不变式**：任一帧后 `Damage≠0` 只出现在结构破坏允许的存活 `Solid`/`Powder` 且受 `MaxIntegrity` 约束；movement swap 不搬运 `Damage`（移动后残留恒 0）；测试钩子扫描无违背。证据：`NeighborWindowReadsWritesAndSwapsAcrossNeighborhood`、`ApplyStructuralDamageDestroysSolidAndRoutesRigidOwned`。（§3.12）
- [x] **刚体像素不累计伤害**：破坏命中 `RigidOwned` cell 时经 `IRigidDamageSink` 路由、`Damage` 不累加（守 #5）。（§3.12）
- [x] **跨 chunk 破坏裁剪**：大半径 `DamageCircle`/长 `DamageBeam` 跨多 chunk 时只作用已驻留段、逐 chunk 合并 dirty + KeepAlive，未驻留段安全跳过；离散安全相位编辑写入范围可超 32px 而不违 #4（#4 只约束并行 pass）。证据：`WorldEffectBoundaryConservationTests.DamageCircleAcrossFourChunkCornerConservesDestroyedRubbleAndDebris`、`DamageBeamAcrossHorizontalChunkBoundaryConservesDestroyedRubbleAndDebris`。（§3.12/§3.10）
- [x] **清 `Damage` 全路径**：§3.12 枚举的 6 条清 lane 路径逐条被覆盖，无跨材质幽灵伤害残留；material remap fallback（plan/07）后 `Damage` 归 0。（§3.12）
- [x] **`Damage` lane 零分配**：稳态 `ApplyStructuralDamage`/`DamageCircle`/`DamageBeam` 无托管堆分配。证据：`WorldEffectBoundaryConservationTests.StructuralDamageEntriesDoNotAllocateAfterWarmup`。（§3.12）
- [x] **Dispersion clamp/FlowRate 别名**：加载期 `Dispersion` clamp 到 `[0,MoveCap]`、`FlowRate` 为其对外别名不新增字段；液体单步水平位移 ≤32px（守 #4）。证据：`DispersionMoveCapTests`、`MaterialContentLoaderTests`。（§3.6）
...
破坏模型（Engine Core 通用结构破坏能力 / M14 showcase 支撑能力，§3.12）新增依赖：

- `plan/04`：在 `MaterialPropsTable` 注入 `Hardness`/`MaxIntegrity`/`RubbleTarget`（by-name→id 解析，守 #8），并定义 `Indestructible`/碎屑/`MineYield` 字段语义。
- `plan/05`：消费破坏触发的碎屑抛射请求（cell→particle handshake）与 `MineYield` 采集事件转发；`Explode` 组合改写落点。
- `plan/06`：经 `IRigidDamageSink` 接收 `RigidOwned` cell 被破坏的形状重建路由（守 #5）。
- `plan/07`：定义 `Damage` lane 的存档契约（持久 lane、chunk 二进制格式版本迁移、material remap fallback 清 0）——字节预算/存档承重墙评审的对接方，三处预算数字一致。
- `plan/08`：只读 `Damage` lane 做裂纹叠色（Simulation→Rendering 只读通路，守 #7）。
- `plan/11`/`plan/13`：经 facade 暴露 `ApplyStructuralDamage`/`DamageCircle`/`DamageBeam` 供武器/可玩循环。
...
- [x] **节点 7**：`feat(sim): 反应/lifetime 钩子 seam 与 MaterialPropsTable 消费`（§4.8）。
- [x] **节点 8**：`feat(sim): 帧相位 3/4/6/7 内核接口与测试/诊断钩子`（§4.9、§4.10）。
- [x] **节点 9**：`feat(sim): per-cell Damage lane + ApplyStructuralDamage/DamageCircle/DamageBeam 破坏内核`（§4.11，已完成字节预算评审、Damage lane、抗性列、RigidOwned 路由、存档契约、清 lane 全路径、Simulation→Rendering 只读通路、Dispersion clamp/FlowRate 别名、零分配测试与 plan/05 碎屑消费联动）。
