...
- [x] `IChunkSource.cs`（seam）：`bool TryGetChunk(ChunkCoord,out Chunk)`、`ReadOnlySpan<Chunk> ResidentChunks`、`ResolveNeighborhood(...)`（3×3 驻留，由 plan/07 实现，border ring 保证）。（§3.2/§3.7）
