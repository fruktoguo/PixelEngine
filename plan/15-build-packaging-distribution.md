...
> **DOC-002 产物路径校正**：后文 `artifacts/publish`、`artifacts/package` 和 staging 路径是 publish/audit 命令的可再生输出参数，不是稳定证据链接；发行完成只能由 canonical `REL-*` 及稳定 manifest/hash 报告证明。
...
- [x] GitHub Release 上传报告约束已写入预检：`workflow_run` 必须来自 tag push，且必须覆盖 package asset、唯一 `SHA256SUMS`、每个 asset 的 SHA256 与绑定同一 release tag 的 `browser_download_url/<asset>`；只写 success、缺少同源 `workflow` / `run_attempt`、缺下载 URL、URL 指向其它 tag 或 `workflow_dispatch` 不能冒充完成。
...
- [x] 本机正式输出目录已落地：`tools/update-final-output.ps1` 要求已跟踪工作树干净，防止未提交源码 / 计划 / 工具改动被生成到“正式版”；该门禁使用 `git status --porcelain --untracked-files=no`，不会被已忽略或未跟踪的 `最终输出/`、`artifacts/` 产物误挡。脚本先在 `artifacts/final-output-staging/<timestamp>/` 构建并验证 EditorShell 默认工作台、编辑器出包链路和 Demo 窗口短跑，全部通过后先调用 `tools/verify-final-output.ps1` 独立审计待发布目录，审计通过后才原子替换 `最终输出/编辑器`、`最终输出/游戏Demo` 与 `_验证记录/manifest.json`；`最终输出/` 只作为本机正式产物入口并已加入 `.gitignore`。正式输出根目录会生成 `SHA256SUMS`，覆盖编辑器、Demo、验证截图/日志、`_验证记录/manifest.json` 与 `README.txt`，manifest 以 `checksumFile='SHA256SUMS'`、`sourceWorktreePolicy='tracked-clean-required'` 与 `sourceTrackedWorktreeClean=true` 记录完整性清单位置和源码来源门禁，便于人工验收绑定到已验证产物；`tools/verify-final-output.ps1` 可在不重新打包、不替换目录的情况下校验现有 `最终输出/`，先比对目录文件集与 `SHA256SUMS` 清单，再逐项重算 SHA256，确认 manifest 绑定当前 HEAD、入口文件 / 验证记录存在、编辑器默认工作台 probe stdout 含 `completed=True` / `succeeded=True` / `build_completed=True` / `build_ok=True` 成功摘要、Demo probe stdout 含 `window_frame_probe` 与 `PixelEngine.Demo` marker、Demo build-result 为 `ok=true`、编辑器符号清理策略生效，并拒绝未登记额外文件、登记了不存在文件或缺少 probe 成功 marker 的不完整清单；`HostingProjectDisciplineTests.FinalOutputVerifierRejectsDirectoryAndChecksumSetDrift` 与 `FinalOutputVerifierRejectsProbeLogsMissingSuccessMarkers` 已构造最小正式输出目录真实运行 verifier，覆盖 clean 目录通过、未登记文件失败、缺失登记失败、编辑器 probe 日志缺成功摘要失败与 Demo probe 日志缺窗口 marker 失败。这只是本机正式输出证据链，不替代 tag Release 的 package-level `SHA256SUMS` / GitHub upload 证据。正式输出编辑器目录默认清理 `.pdb` / `.xml` 开发元数据，并在 manifest 记录 `editorSymbolsIncluded=false` 与 `editorDeveloperMetadataPolicy='pdb-and-xml-pruned'`；需要诊断符号时必须显式传 `-IncludeEditorSymbols` 重新生成。正式输出 Demo 默认请求 Web-first UI 产品主路径 `RmlUi`，并在 manifest / build-result 记录请求后端；若目标上下文或 native 不满足 `RmlUiNativeProfileGate`，运行时仍显式回退 `ManagedFallback` 并记录 `GameUiBackendSelection`，不把 fallback 冒充 RmlUi/GLES/Ultralight 验收。正式输出更新脚本的发布 / 验证子进程已显式 `UseShellExecute=false` + `CreateNoWindow=true`，配合 EditorShell / Demo `OutputType=WinExe` 防止正式应用验证或 Build And Run 链路弹出控制台窗口。

**开发者快速输出边界**：仓库根 `一键构建.bat` 经 `tools/run-build-one-click.ps1` 调用 `tools/update-final-output-fast.ps1`，只执行 authoritative native build、Editor publish 与 Demo R2R publish/package，并在两份入口都存在后原子替换根 `最终输出/`。该模式显式跳过 test、产品 probe、verifier、自动化 SDK/CLI 与正式 checksum，允许从当前开发工作树快速得到 `编辑器/PixelEngine.exe` 和 `游戏Demo/PixelEngine Demo.exe`；必须写 `_快速构建/manifest.json` 且 `verified/testsRun/probesRun/verifierRun` 全为 `false`，不得引用为正式 evidence。完整发布仍只能由 `tools/update-final-output.ps1` 生成并覆盖快速目录。
...
- [x] UI native 打包边界已登记：Web-first UI Runtime 的 RmlUi 与 Ultralight 若启用，只能 dynamic-only 落 `runtimes/<rid>/native/`，纳入许可和 SHA256SUMS，不进入 Box2D dual-build；当前 RmlUi native shim 仅为 desktop GL3 renderer，ANGLE/GLES native profile 未闭合时必须经 profile gate 回退 ManagedFallback；NativeAOT publish 显式排除 `PixelEngine.UI.Native.dll` 并回退 ManagedFallback，2026-07-09 本机 `win-x64` R2R+AOT 发行审计已验证 R2R 包含 UI native、AOT 不携带动态 UI native；当前 Ultralight optional profile inactive，发行审计拒绝未激活的 `Ultralight` / `WebCore` / `AppCore` native 混入。
...
- [ ] 为 Web-first UI Runtime native 后端补许可、体积、AOT 绑定、fallback 和发行 gate 证据，尤其 RmlUi ANGLE/GLES native profile 与 Ultralight 可选高保真后端；RmlUi 缺真实 GLES3 renderer/loader/context 函数表验证时必须回退 ManagedFallback，Ultralight optional profile inactive 时也必须自动回退 ManagedFallback，且不得把 RmlUi / Ultralight native 文件混入包当成发行闭合，不得把 Ultralight native 混入包当成发行闭合。
...
- [x] 本机正式输出更新命令：`pwsh -NoProfile -File tools/update-final-output.ps1`，输出 `最终输出/编辑器`、`最终输出/游戏Demo` 与 `最终输出/_验证记录/manifest.json`；只有全部验证和替换前独立 verifier 审计通过后才替换正式目录，默认 `-DemoRuntimeUiBackend RmlUi`，可显式覆盖为 `ManagedFallback`/`Ultralight` 用于回归诊断。本机正式输出审计命令：`pwsh -NoProfile -File tools/verify-final-output.ps1`，只读取现有 `最终输出/` 并校验 manifest / checksum / 入口 / 验证记录 / probe stdout 成功 marker，不触发 publish 或目录替换；`tools/update-final-output.ps1` 在原子替换前也会调用同一 verifier 防止更新链路与独立审计口径漂移。
...
- [!] 最终上传证据路径：GitHub Release upload markdown 必须列出 `uploaded_asset_count=packageCount+1`、全部 package asset hash、唯一 `SHA256SUMS` hash，以及每个 package / `SHA256SUMS` 的 `browser_download_url/<asset>`；下载 URL 必须指向与 `workflow_run.ref` 相同的 release tag。
