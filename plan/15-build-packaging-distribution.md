...
- [!] 阻塞：6 个 RID（win-x64/win-arm64/linux-x64/linux-arm64/osx-x64/osx-arm64）的 R2R 通道产物全部构建成功并产出（架构 §15）。本机已验证 `win-x64/r2r`，其余 RID 需 release workflow 或对应目标 runner 产物闭合；`tools/release-evidence-preflight.ps1` 已提供 release evidence manifest 预检，缺证据时保持阻塞，证据齐全也仅为 `pending_review`，证据见 `docs/release-reports/2026-07-02-win-x64-publish.md`。
...
- [!] 阻塞：版本与命名：所有产物按 `PixelEngine-Demo-<version>-<rid>-<channel>` 命名，`InformationalVersion` 含 git sha，构建确定性可复现（同输入同产物 hash）。本机已验证 `PixelEngine-Demo-0.1.0-win-x64-{r2r,aot}.zip` 命名；tag 覆盖、git sha 与确定性 hash 需 release workflow 产物复核，release evidence manifest 需提供 `deterministicHashReport` 后才能进入人工复核。
