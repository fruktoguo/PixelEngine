...
- [x] GitHub Release 上传报告约束已写入预检：`workflow_run` 必须来自 tag push，且必须覆盖 package asset、唯一 `SHA256SUMS`、每个 asset 的 SHA256 与 `browser_download_url/<asset>`；只写 success、缺少同源 `workflow` / `run_attempt`、缺下载 URL 或 `workflow_dispatch` 不能冒充完成。
...
- [!] 最终上传证据路径：GitHub Release upload markdown 必须列出 `uploaded_asset_count=packageCount+1`、全部 package asset hash、唯一 `SHA256SUMS` hash，以及每个 package / `SHA256SUMS` 的 `browser_download_url/<asset>`。
