...
最终用户下载包采用「玩家友好启动布局」：压缩包根目录只放清晰的启动入口（如 `PixelEngine Demo.cmd` / `PixelEngine Demo.sh`）、README / 许可证 / 校验说明等少量面向玩家的文件；真实 .NET 发布输出放入 `app/` 子目录。这样不会破坏 .NET host 对同目录 dll/deps/runtimeconfig/native 依赖的默认探测，也避免玩家解压后在根目录被大量 `.dll`、`.pdb`、`.xml` 淹没。launcher 只负责跳转到 `app/PixelEngine.Demo(.exe)` 并保持工作目录 / `content/` 定位正确；发行审计必须验证包根目录不直接堆放托管程序集与符号文档文件。

R2R 通道（非单文件，保留可排错布局）：

```
PixelEngine-<version>-<rid>/
├─ PixelEngine Demo.cmd / PixelEngine Demo.sh   # 玩家启动入口
...
├─ PixelEngine Demo.cmd / PixelEngine Demo.sh
...
`content/`（materials.json、reactions.json、材质纹理、音效、默认场景）随每个产物原样打包到运行目录 `app/content/`（架构 §16.3、§11）。打包脚本以开发态 `content/` 为单一真相源拷贝，不重排、不改名（材质稳定字符串键的可移植性不受打包影响，不变式 #8、架构 §11.2）。产物内 `app/content/` 与开发态目录结构一致，launcher 切到 `app/` 后引擎仍以相对可执行路径定位，保证 R2R/AOT 两通道、6 RID 下加载行为一致。
...
- [x] 调整 `tools/package.ps1` / `tools/package.sh` 输出玩家友好启动布局：包根目录只保留 launcher、README、包内 `SHA256SUMS` 与 `app/` 子目录，真实 `dotnet publish` 产物与 `content/` 进入 `app/`；Windows 生成 `PixelEngine Demo.cmd`，Linux/macOS 生成可执行 `PixelEngine Demo.sh`，launcher 会切到 `app/` 后启动真实入口，直接运行 `app/PixelEngine.Demo(.exe)` 也保持内容定位正确。
...
- [x] `tools/package.*` 以开发态 `content/` 为单一源拷入每产物运行目录 `app/content/`，不重排/不改名（不变式 #8、架构 §11.2、§16.3）。
- [x] 验证本机 `win-x64` R2R/AOT 包内 `app/content/` 结构与开发态一致，含 `materials.json`/`reactions.json`/纹理/音效/场景（§3.9）。
- [x] 验证 R2R/AOT × 6 RID 产物内 `app/content/` 结构与开发态一致，引擎按相对路径定位成功（§3.9；随发布 CI 节点落实）。
...
- [!] 阻塞：内容打包：每个产物的运行目录 `app/content/` 含 materials.json/reactions.json/纹理/音效/默认场景，结构与开发态一致，引擎加载成功（架构 §16.3、§11）。本机 `win-x64` 双通道已由 smoke 与 artifact audit 验证；“每个产物”需 6 RID 全矩阵。
- [x] 玩家友好启动布局：解压后的包根目录只有清晰启动入口和少量说明/校验文件，`.dll`、`.pdb`、`.xml`、`.deps.json`、`.runtimeconfig.json` 等运行时依赖位于 `app/` 子目录；从 launcher 启动 Demo 成功，直接进入 `app/PixelEngine.Demo(.exe)` 也成功；发行审计会拒绝运行时依赖堆在包根目录。本机已用现有 `win-x64` R2R publish 产物重新打包到 `artifacts/package-layout-current`，`tools/audit-release-artifacts.ps1 -PublishRoot artifacts/publish -PackageRoot artifacts/package-layout-current` 通过；包输出目录仅有 `PixelEngine-Demo-0.1.0-win-x64-r2r.zip` 与 `SHA256SUMS`，不再遗留 `staging/`；解压后根目录仅有 `app/`、`PixelEngine Demo.cmd`、`README.txt`、`SHA256SUMS`，且根目录 launcher `PixelEngine Demo.cmd --smoke` exit=0。
