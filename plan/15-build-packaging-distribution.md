...
- [x] 本机正式输出更新命令：`pwsh -NoProfile -File tools/update-final-output.ps1`，输出 `最终输出/编辑器`、`最终输出/游戏Demo` 与 `最终输出/_验证记录/manifest.json`；只有全部验证通过后才替换正式目录，默认 `-DemoRuntimeUiBackend RmlUi`，可显式覆盖为 `ManagedFallback`/`Ultralight` 用于回归诊断。本机正式输出审计命令：`pwsh -NoProfile -File tools/verify-final-output.ps1`，只读取现有 `最终输出/` 并校验 manifest / checksum / 入口 / 验证记录 / probe stdout 成功 marker，不触发 publish 或目录替换。
