...
- [ ] 调整 `tools/package.ps1` / `tools/package.sh` 输出玩家友好启动布局：包根目录只保留 launcher 与说明文件，真实 `dotnet publish` 产物进入 `app/` 子目录，launcher 保持工作目录与 `content/` 定位正确。
...
- [ ] 玩家友好启动布局：解压后的包根目录只有清晰启动入口和少量说明/校验文件，`.dll`、`.pdb`、`.xml`、`.deps.json`、`.runtimeconfig.json` 等运行时依赖位于 `app/` 子目录；从 launcher 启动 Demo 成功，直接进入 `app/PixelEngine.Demo(.exe)` 也成功；发行审计会拒绝运行时依赖堆在包根目录。
