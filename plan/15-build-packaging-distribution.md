...
- [x] 玩家友好启动布局：解压后的包根目录只有清晰启动入口和少量说明/校验文件，`.dll`、`.pdb`、`.xml`、`.deps.json`、`.runtimeconfig.json` 等运行时依赖位于 `app/` 子目录；从 launcher 启动 Demo 成功，直接进入 `app/PixelEngine.Demo(.exe)` 也成功；发行审计会拒绝运行时依赖堆在包根目录。本机已用现有 `win-x64` R2R/AOT publish 产物重新打包到 `artifacts/package-layout-check`，`tools/audit-release-artifacts.ps1 -PublishRoot artifacts/publish -PackageRoot artifacts/package-layout-check` 对两个包通过；解压后根目录仅有 `app/`、`PixelEngine Demo.cmd`、`README.txt`、`SHA256SUMS`，且 R2R/AOT 的根目录 launcher 与 `app/PixelEngine.Demo.exe --smoke` 均 exit=0。
