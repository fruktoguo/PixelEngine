...
- [~] plan/20 **HTML UI（PixelEngine.UI）光栅化/合成落相位10、仅在 UI 脏或有动画时执行**；`GameUiHost` 已用真实 dirty/animation 门控后端 Composite/DrawGui，静态屏 `ui.paint=0`，静态 UI phase/clean composite/draw skip/空闲输入泵已由 `GameUiAllocationBenchmarks` 以 `MemoryDiagnoser` 证实稳态零托管分配；`UiDirtyRectCollector` 已提供 headless 纯几何脏矩形收集/相邻重叠合并/最小包围，`UiDirtyRectMergeTests` 覆盖正确性且不触 GL 上传；`UiOffscreenSurfacePresenter` 已复用 Rendering `UiOverlayTexture` / `UiPresentContext.UploadOverlayTexture` 完成离屏 BGRA8 surface dirty rect 上传与 textured quad 合成，真实窗口 smoke 读回 framebuffer 证明 `ui.upload` 真实发生；过载时 UI present cadence 会按质量档位降到 2/3/4 帧间隔，且 UI update/event drain 仍逐渲染帧执行，UI 尖刺只丢渲染帧、绝不拖慢 sim 或触发 accumulator 追帧（守 #6）；sim 频率降级仍由 §4.9 一至五级独立驱动，Ultralight native 后端本体仍待真实依赖/绑定激活。[plan/20 · §4.3/不变式 #6]
...
- [~] Demo **熔岩矿洞逃生**可玩循环（采 3 水晶充能 + 上涨熔岩 + 限时抵达出口 + 密集破坏/碎屑/粒子 + 大面积熔岩流动）已接入真实窗口 `window_frame_probe` 负载矩阵，作为「**高活跃 + 持续破坏**」稳态场景输出 destruction events、lava active area、sim Hz 与 avg/p50/p95/p99/max；本机 `--scripted-window-demo --no-vsync --window-ticks 260` 样本已验证采样链路和字段自洽。仍需目标硬件/硬件计数器证据验证破坏 API 离散写 + Damage 平面读写下 CA/render 帧预算达标、上涨熔岩相变热场不打爆二级热场降级、per-cell +25% 预算不导致 L2/L3 争用使多核加速曲线转 sub-linear（延迟+分支口径，非带宽）。[plan/14/13 · §12.7/§12.8/§17.3]
...
- [~] 性能 HUD/debug overlay 增「**UI**」相位计时口径（相位10：ImGui host 派发 + plan/20 PixelEngine.UI 光栅化/合成耗时），与 CPU busy、GPU elapsed、present/vsync wait 分列；HUD 已显示 `ui.update`/`ui.paint`/`ui.upload`/`ui.composite`、present cadence 与跳过帧数，静止 UI 帧 `ui.paint=0` 有单测覆盖，无上传型后端 `ui.upload=0`；静态 UI phase/clean composite/draw skip/空闲输入泵已由 `GameUiAllocationBenchmarks` 以 `MemoryDiagnoser` 证实稳态零托管分配；headless `UiDirtyRectCollector` 的纯几何合并已覆盖，离屏 surface 的 `ui.upload` 真实 GL 上传与 framebuffer 合成已由 `UiOffscreenSurfacePresenterSmokeTests` 覆盖；Ultralight native 后端仍待后续切片，需避免混入 sim/render 帧判读。[plan/12/20 · §17.1]
- [x] 熔岩矿洞可玩循环下 debug overlay/HUD 与 `window_frame_probe` 的负载计数（活跃 chunk/cell、粒子、刚体、破坏事件/帧、上涨熔岩活跃面积、当前 sim 频率）随场景真实变化并纳入滚动窗口 avg/p50/p95/p99/max 与固定/随负载成本结构展示；`DemoWindowFrameTimeProbeTests.BuildSummaryReportsWarmupExcludedPercentilesAndLoadCounters`、`PerformanceHudPanelTests.BuildSampleAggregatesProfilerCountersAndRuntimeDiagnostics` 覆盖字段来源，真实窗口输出已包含 `destruction_events_*`、`lava_active_area_*`、`sim_hz_*`。[plan/12/14 · §17.1/§17.3]
...
- [~] **UI 相位计时与不追帧**：性能 HUD 含独立「UI」相位计时（相位10），并分列 `ui.update`/`ui.paint`/`ui.upload`/`ui.composite`；plan/20 HTML UI 光栅化/合成已落相位10、静态屏 `ui.paint=0`，无上传型后端 `ui.upload=0`，且静态 UI phase/clean composite/draw skip/空闲输入泵已由 `GameUiAllocationBenchmarks` 以 `MemoryDiagnoser` 证实稳态零托管分配；离屏 surface dirty rect 上传底座已通过真实窗口 smoke 证明可记录 `ui.upload` 并合成到 framebuffer；过载 UI present cadence 降频已接入并在 HUD 显示 interval/skipped，UI update/event drain 不降频；Ultralight native 后端、熔岩矿洞逃生可玩循环高活跃 profiling 负载与目标帧预算实测仍待闭合，最终必须证明 UI 尖刺只丢渲染帧、绝不拖慢 sim 或 accumulator 追帧（守 #6）。[§17.1/§4.3/§12.7/不变式 #6]
