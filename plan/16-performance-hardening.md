...
- [ ] BenchmarkDotNet `[DisassemblyDiagnoser]` 守门，反汇编基线纳入回归。[plan/14 · §17.3]
...
### 4.8 dirty-rect 真生效
- [ ] 静止 chunk working rect 收缩为空、进入 sleep、下帧零迭代。[plan/03 · §5.4]
- [ ] 基准对比「满屏静止」vs「满屏激活」帧时间，前者 sim 成本 ≈ 0。[plan/14 · §5.4/风险 R1]
- [ ] dirty rect 双缓冲（working/current）、cell 单缓冲。[plan/03 · §5.4/不变式 #3]
- [ ] KeepAlive 跨界正确传播且不致 rect 收不回（无永久唤醒）。[plan/03 · §5.5/风险 R2]
- [ ] §17.2 dirty-rect 叠层目视确认 sleeping 区不再迭代。[plan/12 · §17.2]

### 4.9 过载降级链（绝不追帧）
- [ ] 一级：降低/关闭全分辨率热场（每 N 帧或仅接触式火传播）。[plan/04 · §4.3]
- [ ] 二级：降低光照质量（关 RC/bloom，回退 fog-of-war+emissive）。[plan/08/09 · §4.3]
- [ ] 三级：远离相机的活跃 chunk 降频（隔帧更新外圈）。[plan/03 · §4.3]
- [ ] 四级：整体 sim 降 30Hz（相位 3–8 每两帧一次，render 每帧，dt=1/30）。[plan/03 · §4.2/§4.3]
- [ ] 五级兜底：接受 <60fps 真实减速，**绝不 accumulator 追帧**。[plan/02 · §4.1/不变式 #6]
- [ ] 降级由 Core 诊断计时器驱动（连续超预算帧触发）。[plan/02 · §4.3/§17.1]
- [ ] 每帧成本上限节流落实（形状重建每刚体每帧≤1次、合并像素移除、CCL off-thread）。[plan/06 · §4.3/§8.4/风险 R4]

### 4.10 大世界内存上限
- [ ] 常驻世界设硬上限（建议 ≤512MB，可配置）。[plan/07 · §12.2]
- [ ] LRU 驱逐（到相机距离 + 闲置时长），超上限提前驱逐最远 sleeping chunk 落盘。[plan/07 · §12.2]
- [ ] chunk payload RLE+LZ4 压制磁盘增长。[plan/07 · §11.3/§12.2]
- [ ] 长时间漫游压测常驻内存稳定在上限内、不无界增长。[plan/14 · §12.2]
...
### 4.12 profiling 工具链
- [ ] BenchmarkDotNet 接入（`[MemoryDiagnoser]` + `[DisassemblyDiagnoser]`）作 CI perf 门禁。[plan/14 · §17.3]
- [ ] `DOTNET_JitDisasm` + Disasmo/Rider 反汇编流程文档化、可复现。[plan/14 · §12.6/§17.3]
...
- [ ] **稳态零分配**：CA/粒子/render buffer/反应/温度/序列化六条热路径 `MemoryDiagnoser` 全报 `0 B`；热路径静态核查无 LINQ/闭包/装箱/迭代器。[§12.4/AGENTS §3]
- [ ] **多线程齐全**：CA checkerboard、Box2D task 桥、render buffer、CCL/形状重建、粒子积分、温度 stencil、序列化字节准备七项均经持久线程池并行；无每帧 `Parallel.For`；活跃任务少时单线程回退生效。[§5.7/§12.7/§14.2/风险 R7]
