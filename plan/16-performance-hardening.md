...
> **DOC-002 性能证据校正**：后文 `artifacts/benchmark-run-ca-*` 目录只代表历史 Dry 入口探针；正式规模数据与未达目标结论以最新 [`PERF-003` 稳定报告](../docs/evidence-2026-07-18-perf-003-ca-throughput.md) 为准，目标硬件状态以 canonical `PERF-*` 为准。
...
2026-07-18 本机固定稳态对照：在相同 2,166,784-cell / 8-worker profile 上按 baseline→optimized→baseline→optimized 运行，每轮 2 launches × 15 warmups × 30 measured iterations；优化前为 12.303–14.029ms，优化后为 10.603–11.664ms，均为 0 B。保守按 11.664ms 折算约 1.486M cells/8ms，仍不足 2M 下限，详见 `docs/evidence-2026-07-18-perf-003-ca-throughput.md`。
...
2026-07-18 垂直 movement 调用图复测：新增 8,192 个独立单帧 kernel 的 Typical Dirty 基准，以超过 100ms 的 workload 消除短 iteration 与连续帧 dirty 收缩；共享 guard 的 SoA/dirty/state/parity 在每轮前后 fail-closed 校验。`c18bc33d` 把 `TryMoveDown` 的首格非空路径从 3,936B 缩至 1,967B，1,972B 空列扫描 helper 显式 `NoInlining`。clean full-active A8/B9 为 10.435/9.841ms，99.9% CI 不重叠，Typical Dirty 为 22.105/20.133us；B9 mean/upper CI 仍只约 1.761M/1.726M cells/8ms。详见 `docs/evidence-2026-07-18-perf-003-trymovedown-call-graph.md`，PERF-003 继续阻塞。
