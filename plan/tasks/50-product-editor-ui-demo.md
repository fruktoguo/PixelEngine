...
- [~] `DEMO-008` 复现 Noita 八个主路径 biome、程序化遭遇点、侧区、秘密连接和跨区捷径。（2026-07-23 按真实游玩反馈恢复进行：先修出生 AABB 与权威材质图/非交互视觉层分离）
  - 优先级：P0。
  - 依赖：`DEMO-007`。
  ...
  - 当前进度：`DEMO-007` 的悬空退化碎条与武器命中可读性回归已完成复验；八区语法、Portal/Holy Mountain、12 个固定地标与 WorldStreamer 长路线继续作为功能基线。Build `17130612` 的 70×48 色图、129 种实际 biome、全部 3360 个宏格、固定出生山体/altar/Laboratory 已由 `ff9898bf` 接入并逐格反查 `Mismatches=0`；`d2abcfd6` 锁定 15 组 STB Herringbone Wang 模板、20 个 main/side reference biome 绑定和来源 SHA256。`CollectWangMarkerAnchors` 以固定容量锚点暴露 Wang marker；`NoitaWangMarkerContentSystem` 按 run seed 和玩家附近窗口创建 overlay prop、VFX、点光源及 oil/acid/fire `MaterialEmitter`、smoke/fire/crystal `SparkEmitter`，并按探索状态和逐帧 0.5 秒 warmup 门控。
  ...
  - 2026-07-23 真实游玩回归：参考出生 `(227,-85)` 是角色中心，但 Demo 曾把它作为 6x12 AABB 左上角，导致底部三行与 `soil` 重叠；修复需保持完整出生 AABB 为空，并锁定 `left_entrance.png` 仅作权威材质图、`*_background.png`/`*_visual.png` 仅作非交互渲染层。
  - 2026-07-25 材质纹理检查点：`feb957aa` 在通用 Rendering/Hosting 内容装载路径接入纯托管材质纹理 provider，并从 Build `17130612` 的规范化来源生成 28 张带逐文件 SHA256 provenance 的 Wang 材质纹理，覆盖 41 个精确材质；深层视口环境可见度收敛为 `112`，玩家点光继续局部提亮。真实 Editor 1280x720 artifact `f8c7ea73...d8451`、Console 0/0/0 与 9 个定向测试通过。画面仍暴露 Wang tile atlas 被直接表现为密集碎片地形的问题；下一节点必须恢复 pixel scene、background/visual、marker、vegetation 和实体的组合规则，不能以继续放大 atlas 代替地图 parity。
  - `DEMO-008` 仍未完成：marker prop 升级为真实敌人/可拾取道具/物理陷阱、剩余 biome/fixed/random pixel scene、完整材料/生态、Noita 原始 RNG 序列、同 seed 全区域截图矩阵和组合后无软锁长路线仍待闭合；现有 framebuffer 只证明接线与坐标，不宣称视觉 parity 完成。
  - 2026-07-24 完整复刻扩展启动：本机 Steam Build `17130612` 的 `data/materials.xml` 统计为 468 条材料声明、466 个唯一材料名、325 条 `Reaction` 与 5 条 `ReqReaction`；Demo 当前仅 25 种材料。后续节点必须将来源定义继承、稳定 name、CellType/相变/反应/结构语义全部规范化并接入 Demo，不能用代表材质或空壳条目凑数；地图验收同时增加地下总纵深、横向可探索范围、每区材料覆盖和结构/生态覆盖矩阵。
  - 2026-07-24 显示/物理尺度边界：三个 runtime Canvas 的参考设计分辨率统一迁移为 `2560×1440`，全部 XHTML 坐标同步转换；设置页可在 `1280×720`、`1920×1080`、`2560×1440` 间调整真实窗口 presentation。内部 world/camera surface 继续固定为 `720×480`，窗口分辨率不得改变 Simulation cell、角色 AABB、碰撞、相机逻辑视野或地图拓扑尺度。
  - 2026-07-25 世界内容目录节点：`tools/extract-noita-world-content.ps1` 从 Build `17130612` 隔离参考树规范化 146 个 biome、640 层材料、317 层植被、785 个 Lua spawn function、12 个 spliced scene、91 个 buffered scene、17 张定位背景、2232 个 biome_impl 文件和 229 个 vegetation 文件，并锁定来源 SHA256；Wang semantic 恢复 `1 source pixel = 1 world cell`，取消人为 5 倍粗化。后续实现必须由该目录闭合背景、花草、交互物和 pixel scene 覆盖，不再扩充少量手写代表项。
  - 2026-07-25 纯 C# marker 节点：785 条来源注册、243 个唯一函数经 `tools/generate-noita-marker-rules.ps1` 预编译为 `NoitaMarkerRules.Generated.cs`，运行时由 C# rule catalog 显式分派 PixelScene/Vegetation/Loot/Prop/Encounter/Trigger/Effect；玩家包禁止携带或执行 Lua，目录一致性测试要求所有来源函数都有 C# rule 且注册次数逐项一致。
  - 2026-07-25 纯 C# Pixel Scene 目录节点：91 个全局 buffered scene 的世界坐标、尺寸、clean/biome/edge 标志及 material/colors/background 三层路径与 SHA256 经 `tools/generate-noita-pixel-scenes.ps1` 预编译为 `NoitaPixelScenes.Generated.cs`；30 个 material、22 个 colors、6 个 background 资产描述均可由运行时 C# catalog 查询，不读取参考 XML/Lua。实际语义像素与背景绘制尚未接线，本节点不作为视觉完成证据。
  - 2026-07-25 全局 material Pixel Scene 接线节点：30 张 material PNG 经 Build `17130612` 材料色表构建时转换为 Brotli + decoded SHA256 的纯 C# 掩码，13 种 marker 色保持为空并保留计数；运行时在 Wang/BitmapCaves 之前按原始绝对坐标和 `1 source pixel = 1 world cell` 覆盖地形，forge 场景已有真实 chunk 回归。colors/background 与 marker 实体尚未接线，仍不宣称视觉 parity 完成。
  - 2026-07-25 全局 Pixel Scene 视觉节点：6 张 background 与 22 张 colors 进入 Demo 自有内容包并逐项记录 Build/source/content SHA256、原始 world 坐标和像素尺寸；运行时分别在权威 cell 后方与 cell 上方/光照前合成，现有出生山体 4 层加全局 scene 28 层共 32 层。marker 实体与其余 biome/随机 scene 仍待闭合，本节点需真实 Player framebuffer 后才可作为视觉证据。
  - 2026-07-25 全局 Pixel Scene marker 节点：`tools/extract-noita-pixel-scene-markers.ps1` 将 30 张 material PNG 中全部 43 个非材质像素编译为纯 C# 世界锚点，逐项保留 scene/local/world 坐标、ARGB、唯一 `RegisterSpawnFunction`、来源脚本；12 个未注册黄色内建点显式保持 `builtin-or-unresolved`。`NoitaWangMarkerContentSystem` 在同一固定容量玩家窗口中先收集全局 scene marker、再填充 Wang marker，forge `(1537,6042)` 已解析为 `spawn_forge_check`。当前仍复用通用 prop/VFX profile，尚未把每个函数升级为对应的真实交互实体。
  - 2026-07-25 marker 驻留修复：实机长路线发现旧实现把历史 marker 永久累计到 128 槽，出生区与 forge 两次扫描即可耗尽容量，后续区域不再生成内容。现改为固定容量活动窗口：每槽绑定 prop/gameplay 实体，扫描时销毁离窗实体、紧凑并复用槽位；内存仍固定，长路线不再受历史区域数量限制。
  - 2026-07-25 Wang 精确材质节点：`e48a8114` 将 15 组 Wang/BitmapCaves 的六类代表语义升级为逐组连续精确材质槽，41 个实际使用的稳定材质名全部进入 Demo 自有 62 材质目录；显式深层出生点同步无限世界 `InitialFocus`，温度相变和脚本 cell 写入在流式 3x3 邻域未补齐时延后。Snowcastle 真实 Editor Play 跨过旧崩溃窗口且 Console 0/0/0，但单色 baseColor、材质纹理/噪声、雾化与后处理仍未达到参考视觉，`DEMO-008` 保持 `[~]`。
  - 2026-07-25 Wang MaterialComponent 节点：PWH3 逐像素保留 Primary 灰度 density；15 套当前接线 biome 的 `wang_map_width/height` 与 75 个 MaterialComponent 层按源顺序进入强类型目录；全量 146 biome/640 层引用的 46 种材质也已并入生成输入，最终 71 种 Noita terrain/scene 所需材质、63 份带 hash 的真实纹理编译为 118 材质运行时包。C# 求值器已接入主灰度场、独立 material index 噪声通道、Y 限制、add_perlin、rare perlin/polka，BitmapCaves 只覆盖结构语义而继续消费 Wang density；14 个目录/实际 chunk 定向测试通过，真实 Campaign Player 360 帧截图为 `artifacts/noita-material-layer-runtime/campaign-fixed.bmp`。当前 1080x720 无 VSync 约 20 FPS，瓶颈仍在 GpuUploadRender/RenderBufferBuild，且稀有场算法与完整 biome topology 总集仍需继续校准，`DEMO-008` 保持 `[~]`。
  - 2026-07-25 Snowcastle 随机 Pixel Scene 节点：`129ce294` 将 Wang 内建红/黄 marker 按 Build `17130612` 的两张来源权重表确定性选择 12 个场景，25 种材质掩码进入 CPU 权威网格，10 张 background/visual 经视口动态层合成，旧紫色占位被抑制；Demo 10/10、Hosting 1/1、Editor Shell 0 warning/error，Snowcastle benchmark 为 `2.603 ms/chunk`。本轮实机 framebuffer 未成功定位深层场景，明确不作为视觉完成证据；详见 `docs/evidence-2026-07-25-demo-008-snowcastle-random-pixel-scenes.md`。
  - 2026-07-25 VegetationComponent 节点：Build `17130612` 的 317 层 vegetation、105 张成熟 sprite 与 12 种材质经安全 WAK 解包和离线生成器固化为 Demo 自有 PNG、Brotli alpha mask 与纯 C# catalog；运行时按全局 seed/grid、reference biome 和 Wang/BitmapCaves/Pixel Scene 实际空实边界确定性放置，非 visual 层写入权威 cell，visual 层进入固定容量视口组合。7 个定向测试、8 场景短基准和 180 帧真实 Player 均通过；深层煤矿权威植被由 chunk 测试证明，当前 Player 帧只证明默认 Campaign 地表植被渲染接线，不冒充煤矿视觉 parity。详见 `docs/evidence-2026-07-25-demo-008-noita-vegetation.md`。
  - 2026-07-25 Biome background 节点：129 个 reference biome 的 weather background、四向 edge、neighbor 继承和 edge priority 经离线生成器固化为 111 张 Demo 自有 PNG、逐文件 SHA256 catalog 与纯 C# 定义；`PlayableWorldDirector` 补齐动态视口 provider 转发，使 Snowcastle、植被和背景真正进入正式 Player 渲染链。煤矿真实 Player 120 帧截图确认背景接线，但也证明 Wang/pixel-scene 仍以缩略图式过密结构占满局部视口；下一节点必须校准 topology/Wang 世界坐标、相机 pixels-per-cell 与 scene 激活范围，不能把该帧冒充地图尺度 parity。详见 `docs/evidence-2026-07-25-demo-008-noita-biome-backgrounds.md`。
  - 2026-07-25 Wang 颜色语义纠偏节点：离线生成器不再让普通 `Graphics color` 抢占 Wang 灰度基础拓扑，修复 `ffffffff -> trailer_text` 导致煤矿高亮白色碎条的问题；BitmapCaves 仍按独立上下文保留 `505050 -> coal`。15 组目录、86 材质与 37 张实际纹理重新生成，10 个定向测试和真实煤矿 Player 帧通过。画面仍证明 256 Wang 拓扑直接逐 cell 落入 512 世界会产生过密小洞；下一节点必须复原 XML `<Materials>` 分层填充，不以相机或最近邻 20 倍放大代替。详见 `docs/evidence-2026-07-25-demo-008-wang-color-semantics.md`。
  - 2026-07-25 全 Wang biome 编译节点：Build `17130612` 中 28 个具有真实 `wang_template_file` PNG 的 biome 已全部进入强类型目录与运行时选择路径，覆盖 33 个 reference biome 绑定、142 个 MaterialComponent 层和 79 种直接引用材质；`RandomColor` 非二值输出按世界 seed/坐标确定性选择，liquidcave 的五种魔法液保持独立材质。BitmapCaves 的 `$[0-999]` 资源模式按 9 个放置组/1008 个变体编译，winter-caves 首组 1000 变体不再被错误展开成 1000 个结构规则；固定容量空间索引扩为 `ushort`，仍不在采样热路径分配。最终 Demo 包为 128 材质、71 张带来源 hash 的纹理，18 个 Wang/BitmapCaves/世界目录定向测试通过。尚未闭合无模板与程序化 biome、全部随机 Pixel Scene 和真实 Player 尺度/视觉矩阵，`DEMO-008` 保持 `[~]`。
  - 2026-07-25 非主路径 Wang 接线节点：修复 `CompileWorldTopology` 与 marker 收集器只识别 `main-biome/side-biome` 的分类遗漏；the-sky、clouds、pyramid、liquidcave、winter-caves、sandcave、lake-deep、meat、robobase、the-end 这 10 个具有真实 Wang 来源但属于 empty/clouds/surface/water/generic 类型的 reference biome，现统一优先消费已编译 Wang、BitmapCaves、MaterialComponent 与 RandomColor，未绑定区域才回退到类型化 C# 程序规则。Demo Release build 0 warning / 0 error，19 个相关目录、地形与 marker 定向测试通过；非主路径旧六类 semantic 的专用 palette 仍需随 99 区材质 profile 一并消除。
  - 2026-07-25 全 biome MaterialComponent 目录节点：`tools/generate-noita-biome-material-layers.ps1` 将 Build `17130612` 的 146 个 biome、640 个 MaterialComponent 层离线编译为强类型 C#，逐 sourcePath 保留稳定材质名、density 范围、独立 material index、Y 限制、add_perlin 与 rare perlin/polka 参数；运行时不读取参考 XML/Lua 或 1MB 审计 JSON。`CompileWorldTopology` 对无 Wang reference biome 绑定同源 profile，程序化 generic cave 与 generic mountain 的实心密度已进入共用零分配层求值器。Demo Release build 0 warning / 0 error，19 个相关目录/Wang/BitmapCaves 回归通过；solid、fixed structure、Holy Mountain、水/熔岩等类型仍需按各自拓扑密度继续接线。
  - 2026-07-25 非 Wang 类型化材质节点：solid、water 与非 Wang generic structure 不再整宏格返回统一 `BoundaryStone/Water/Stone`，而是用各自 reference biome salt 的连续 density 消费已编译 MaterialComponent；实际 world topology 宏格验证 `solid-wall` 生成 `rock_hard`，`lake` 同时生成 water 与 mud/sand_static。Holy Mountain 继续保留真实 terrain mask；lavalake 的熔岩主体确认来自固定 Pixel Scene，未错误地用周边岩层覆盖。21 个 Wang/BitmapCaves/世界目录与实际地形分布快速测试通过，Demo Release build 0 warning / 0 error；下一节点转入 fixed/static Pixel Scene 资产。
  - 2026-07-25 spliced Pixel Scene 地形节点：全局 `_pixel_scenes.xml` 引用的 12 个 spliced XML 已展开为 80 个固定场景，与 91 个直接 buffered scene 合并为 171 个强类型定义；纯 C# FastLZ 解码器离线读取 80 份 `.plz`，包含负尺寸翻转语义，最终生成 110 份带 SHA256 的 Brotli 材质 mask。lavalake2、boss arena、tree、water cave、moon、gourd room 等固定结构现按原始 world cell 坐标覆盖所有 topology 类型，blood、bone_static、cheese_static、mud、sand_petrify、snow_sticky 保持独立稳定材质；Demo 包扩为 130 材质、72 张纹理。9 个受影响目录/采样/实际 chunk 快速测试通过；spliced colors/background、非材质 marker 与对应真实交互实体仍待后续节点，`DEMO-008` 保持 `[~]`。

- [!] `DEMO-009` 实现 Noita Wand / Spell 数据、施法求值、库存与 Wand 编辑系统。（暂停：按用户当前节奏先闭合 UI 与 `DEMO-008` 地图生成；解除条件：当前地图视觉/碰撞差距完成复验后恢复 Wand 线）
  ...
  - 验收：catalog 以稳定字符串键入盘并在加载期完整校验；Wand 完整支持 Shuffle、Spells/Cast、Cast Delay、Recharge Time、Mana Max、Mana Charge Speed、Capacity、Spread、Always Cast 与 speed multiplier；Spell 覆盖 projectile、modifier、multicast/draw、trigger/timer/death-trigger、material、utility、passive、limited-use 与特殊类别，并用版本化 parity matrix 跟踪参考目录覆盖；有界 evaluator 对 draw、递归、payload、投射物和世界效果 fail-closed，稳态 cast 零托管分配；四 Wand + 四 item 的拾取/丢弃/换装/库存/编辑/HUD/存取均可用；所有效果只经公开 API 延迟到安全相位；性质测试、组合测试、分配基准、参考组合用例和真实输入证据通过。

- [ ] `DEMO-010` 复现 Noita 敌人角色、统一战斗/status/stain、战利品、金币与 biome 生态。
  - 优先级：P0。
  - 依赖：`DEMO-007`、`BASE-006`、`BASE-011`。
  - 设计来源：`docs/PixelEngine-原创Roguelite战役设计.md` §6；`plan/13-demo-game.md` §3.19。
  - 验收：`content/enemies.json` 按 biome 定义参考敌人角色、移动、感知、攻击、faction/friendly-fire、抗性、材料交互、血液/尸体、掉落和 spawn budget，并由 parity matrix 跟踪目录与行为覆盖；玩家与敌人共享统一伤害/status/stain/projectile/world-effect 合同；实体流送与卸载状态明确，常驻数量/寻路/查询有硬预算，稳态无逐实体分配；金币与掉落由 run seed 确定且 UI/拾取真实改写 run state；AI、战斗、材料互动、性能与真实窗口遭遇证据通过。

- [ ] `DEMO-011` 实现 Holy Mountain 恢复、Spell refresh、商店、Wand 编辑和 Perk 闭环。
  - 优先级：P0。
  - 依赖：`DEMO-008`、`DEMO-009`、`DEMO-010`。
  - 设计来源：`docs/PixelEngine-原创Roguelite战役设计.md` §3、§6；`plan/13-demo-game.md` §3.19。
  - 验收：每个 Holy Mountain 完整提供 full-health regeneration、Spell refresher、Spell/Wand 商店、库存对比、Wand 编辑、Perk 三选一、reroll、训练雕像、水池、出口坍塌与保护区破坏后果；库存、价格、重抽与候选由 run seed/数据生成，交易原子化且失败有反馈；选择后未选项关闭并持久进入 run state；离开/绕行/破坏走统一世界规则；Web-first UI 与 ManagedFallback 功能等价，键鼠/手柄可用；自动化、参考布局 parity 与真实输入闭环通过。

- [ ] `DEMO-012` 实现 The Laboratory、Sampo、Kolmisilma、完成/死亡结算与下一轮。
  - 优先级：P0。
  - 依赖：`DEMO-008`、`DEMO-009`、`DEMO-010`、`DEMO-011`。
  - 设计来源：`docs/PixelEngine-原创Roguelite战役设计.md` §8；`plan/13-demo-game.md` §3.20。
  - 验收：玩家取得 Sampo 后触发 Kolmisilma，Boss 行为与强度由 Orb/quest 状态驱动，攻击真实作用于材料、projectile、场地和玩家；普通完成、永久死亡、继续探索和数据化下一轮条件完整可达；RunSummary 显示真实统计/Wand/Spell/Perk 并可开始干净新 run 或返回菜单；所有结果可靠清理旧 world/script/entity/UI/physics/particle/audio/event/ALC 状态；headless 全流程、真实 Player 通关/死亡/下一轮和泄漏长跑证据通过。

- [ ] `DEMO-013` 完成 Noita 高保真复刻全流程平衡、扩展世界、parity 证据与最终输出。
  - 优先级：P0。
  - 依赖：`DEMO-007`–`DEMO-012`、`DEMO-001`–`DEMO-003` 的可在本机闭合项；外部 reviewer/硬件缺口按 `SCOPE-005` 如实标注，不得假绿。
  - 设计来源：`docs/PixelEngine-原创Roguelite战役设计.md` §10；`docs/PixelEngine-核心目标与产品定位.md` §7。
  - 验收：默认 Campaign 可从主菜单连续完成八个 biome、七个 Holy Mountain、Wand 构筑、战斗、Kolmisilma 和结算，Sandbox 仍完整可选；地表、侧区、秘密连接、Orb/quest、parallel worlds 与 New Game+ 按 parity matrix 记录实现或明确差异；难度、经济、掉落、构筑和敌人组合无已知必现软锁，输入/字幕/闪光与可读性设置可用；目标场景性能、稳态分配、长跑生命周期、全 solution 测试、真实 PixelEngine Editor/Player 路线、同源参考/目标截图与状态证据、evidence index、资产 provenance、clean final-output、安装包与独立 verifier 全通过。
