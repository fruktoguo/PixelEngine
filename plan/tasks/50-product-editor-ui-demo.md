...
- [~] `DEMO-008` 复现 Noita 八个主路径 biome、程序化遭遇点、侧区、秘密连接和跨区捷径。
  ...
  - 当前进度：`DEMO-007` 的悬空退化碎条与武器命中可读性回归已完成复验；八区语法、Portal/Holy Mountain 玩法覆盖、12 个固定地标及正式 WorldStreamer 长路线继续作为功能基线。2026-07-22 核对用户本机 Steam Noita Build `17130612` 官方解包数据后，旧统一 512-cell region、128-cell Holy Mountain、一维按 Y 分类、连续摆动竖井和随机 Laboratory 已确认不具备地图 parity。当前节点恢复可变 1024/1536/2048 主区与七个 512-cell 层间节点；`biomes.json` v6 完整记录 129 种实际颜色/biome 和 48 行色图，全部 3360 个宏格逐格反查 `Mismatches=0`；出生山体、hall/right/top/floating island 与四个 altar 变体使用来源 hash 锁定的语义掩码，东南侧 `boss_arena` 2600×1600/416 万像素轮廓映射为八类 Demo 材质，旧直筒主轴和手写 Laboratory 替代均已删除。`ff9898bf` detached clean worktree 已通过 native build、solution Release build 0 warning / 0 error、Demo 193 passed / 1 explicit GL skip、两次真实 RmlUi Player framebuffer；七场景 InProcess ShortRun 为 81.60–744.55 us/chunk、MemoryDiagnoser 0–7 B。完成证据：`docs/evidence-2026-07-23-demo-008-noita-full-map-topology.md`。`DEMO-008` 仍未完成：普通 biome Wang/BitmapCaves、背景/spawn marker、其余固定场景、生态实体和同 seed 全区域截图矩阵仍待闭合。参考 provenance 与精确坐标见 `docs/reference-noita-build-17130612-map-topology.md`。
