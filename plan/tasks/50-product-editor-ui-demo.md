...
- [~] `DEMO-008` 复现 Noita 八个主路径 biome、程序化遭遇点、侧区、秘密连接和跨区捷径。
  ...
  - 当前进度：`DEMO-007` 的悬空退化碎条与武器命中可读性回归已完成复验；八区语法、Portal/Holy Mountain、12 个固定地标与 WorldStreamer 长路线继续作为功能基线。Build `17130612` 的 70×48 色图、129 种实际 biome、全部 3360 个宏格、固定出生山体/altar/Laboratory 已由 `ff9898bf` 接入并逐格反查 `Mismatches=0`。随后 `d2abcfd6` 通过可重现提取器锁定 15 组 STB Herringbone Wang 模板、20 个 main/side reference biome 绑定和 PNG/XML/Lua/material/license SHA256；运行时在 `Describe` 阶段解压编译，chunk 热循环按全局坐标无分配采样，pixel scene 保持最高优先级，marker 色暂为空而不误生成为前景。detached clean worktree 已通过 native build、solution 0 warning / 0 error、Demo 197 passed / 1 explicit GL skip、真实 RmlUi Player framebuffer；七场景 InProcess ShortRun 为 73.42–657.33 us/chunk、MemoryDiagnoser 0–7 B。证据：`docs/evidence-2026-07-23-demo-008-noita-full-map-topology.md`、`docs/evidence-2026-07-23-demo-008-noita-wang-terrain.md`。`DEMO-008` 仍未完成：BitmapCaves、背景、marker 实体、剩余 fixed/random pixel scene、完整材料/生态、Noita 原始 RNG 序列、同 seed 全区域截图矩阵和组合后无软锁长路线仍待闭合。
