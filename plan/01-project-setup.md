...
- [ ] 新增 `global.json`，锁 SDK 至 `10.0.x`（`rollForward: latestFeature`），与本机 10.0.108 一致
- [ ] 新增 `nuget.config`，固定单一源 `nuget.org`（配合架构 R10 锁源）
...
- [ ] 在 `Directory.Build.props` 加 `TreatWarningsAsErrors` 仅 `'$(CI)'=='true'` 时为 true（plan/00 §1）
- [ ] 在 `Directory.Build.props` 用 `<IsHotPath>` 条件设 `AllowUnsafeBlocks=true` 与 Release `Optimize=true`（plan/00 §6）
- [ ] 新增 `Directory.Build.targets`：条件导入 `native/PixelEngine.Native.targets`（仅 Interop）与统一收尾属性
- [ ] 新增 `Directory.Packages.props`：`ManagePackageVersionsCentrally=true`，按 §2 表锁定全部 `PackageVersion`（每条注「需核对」）
- [ ] 补全 `.gitignore`：`bin/`、`obj/`、`native/out/`、`runtimes/`、`BenchmarkDotNet.Artifacts/`、`*.user`、存档 / 产物（AGENTS.md §6）
- [ ] 在 `.editorconfig` 追加分析器 severity：全局 code-style=warning；热路径项目把零分配 / SIMD 诊断提升为 `error`（plan/00 §6）
- [ ] 新增 `.gitattributes`：规范行尾、native 二进制标 binary
- [ ] 新增 `.vscode/extensions.json`（推荐 `ms-dotnettools.csdevkit`、`ms-dotnettools.csharp`、`ms-vscode.cmake-tools`）、`settings.json`、`launch.json`

解决方案与 src 项目（12 个）：

- [ ] 新增 `PixelEngine.sln`，建 `src`/`demo`/`tests`/`bench`/`Build` solution folders，并把 `native/`、`tools/`、`.github/`、`Directory.*.props`、`global.json` 挂为 solution items
- [ ] 创建 `src/PixelEngine.Core/PixelEngine.Core.csproj`（Library，`IsHotPath=true`），无 ProjectReference
- [ ] 在 `src/PixelEngine.Core/EngineConstants.cs` 建 `public static partial class EngineConstants` **空壳**（带 XML 文档头，无常量值；常量在 plan/02）
- [ ] 创建 `src/PixelEngine.Interop/PixelEngine.Interop.csproj`（Library，`IsHotPath=true`），引用 Core；条件导入 native targets
- [ ] 创建 `src/PixelEngine.Simulation/PixelEngine.Simulation.csproj`（Library，`IsHotPath=true`），引用 Core
- [ ] 创建 `src/PixelEngine.Content/PixelEngine.Content.csproj`（Library），引用 Core、Simulation
- [ ] 创建 `src/PixelEngine.Serialization/PixelEngine.Serialization.csproj`（Library），引用 Core、Simulation、Content；引 `K4os.Compression.LZ4`
- [ ] 创建 `src/PixelEngine.World/PixelEngine.World.csproj`（Library），引用 Core、Simulation、Serialization
- [ ] 创建 `src/PixelEngine.Physics/PixelEngine.Physics.csproj`（Library，`IsHotPath=true`），引用 Core、Interop、Simulation
- [ ] 创建 `src/PixelEngine.Rendering/PixelEngine.Rendering.csproj`（Library，`IsHotPath=true`），引用 Core、Simulation、World；引 `Silk.NET.Windowing/Input/OpenGL`
- [ ] 创建 `src/PixelEngine.Audio/PixelEngine.Audio.csproj`（Library），引用 Core、Content；引 `Silk.NET.OpenAL`
- [ ] 创建 `src/PixelEngine.Scripting/PixelEngine.Scripting.csproj`（Library），引用 Core、Simulation、Physics、World、Content；引 `Microsoft.CodeAnalysis.CSharp`
- [ ] 创建 `src/PixelEngine.Editor/PixelEngine.Editor.csproj`（Library），引用全部 9 个子系统（Core/Simulation/Physics/World/Serialization/Content/Rendering/Audio/Scripting）；引 `Hexa.NET.ImGui(.Backends/.ImGuizmo/.ImPlot/.ImNodes)`
- [ ] 创建 `src/PixelEngine.Hosting/PixelEngine.Hosting.csproj`（Library），引用全部 11 个其它引擎项目
- [ ] `dotnet build` 后用 `dotnet list <proj> reference` 逐项核对依赖方向与 §3.2 表完全一致、无反向、无环

demo / tests / bench：

- [ ] 创建 `demo/PixelEngine.Demo/PixelEngine.Demo.csproj`（Exe），**仅**引用 Hosting
- [ ] 写 `demo/PixelEngine.Demo/Program.cs`：打印 banner（名称 + 版本 + 目标 RID），退出 0；注释预留 Silk.NET 空窗口切换入口
- [ ] 创建 `tests/PixelEngine.Simulation.Tests/`（Test 项目 + `SmokeTests.cs` 装配冒烟 `[Fact]`），引用 Simulation
- [ ] 创建 `tests/PixelEngine.Physics.Tests/`（同上），引用 Physics
- [ ] 创建 `tests/PixelEngine.Serialization.Tests/`（同上），引用 Serialization
- [ ] 创建 `tests/PixelEngine.Scripting.Tests/`（同上），引用 Scripting
- [ ] 创建 `bench/PixelEngine.Benchmarks/PixelEngine.Benchmarks.csproj`（Exe，`IsHotPath=true`），引用 Core、Simulation、Physics、Serialization；引 `BenchmarkDotNet`
- [ ] 写 `bench/PixelEngine.Benchmarks/Program.cs`：`BenchmarkSwitcher.FromAssembly(...).Run(args)`（空基准集合）
...
- [ ] `dotnet build PixelEngine.sln -c Release` 全 18 项目通过、零警告
- [ ] `dotnet test` 四个测试项目全绿（装配冒烟 `[Fact]` 通过）
- [ ] `dotnet run --project demo/PixelEngine.Demo -c Release` 打印 banner 并退出 0
- [ ] `dotnet run --project bench/PixelEngine.Benchmarks -c Release` 正常启动 BenchmarkSwitcher（空集合）
- [ ] 本机执行 `tools/build-native`（当前 RID），产出 Box2D 静态 + 动态库于约定目录

## 5. 验收标准

- [ ] `PixelEngine.sln` 含且仅含 plan/00 §5 的 18 个项目（12 src + 1 demo + 4 tests + 1 bench），路径与命名与 §3.2 表一致
- [ ] 每个项目的 `ProjectReference` 与 §3.2 表逐字一致，依赖方向满足 `Demo → Hosting → {子系统} → Interop → Core`，`dotnet list reference` 核验无任何反向依赖、无循环
- [ ] `Simulation` 不引用 `Rendering`/`Physics`；`Demo` 仅引用 `Hosting`；`Interop` 为唯一持有 native 集成的项目（不变式 10）
- [ ] `Directory.Build.props` 生效且被全部项目继承：`net10.0`、C# 14、Nullable、ImplicitUsings、Deterministic、`GenerateDocumentationFile`、分析器开启
- [ ] 热路径项目 {Core, Interop, Simulation, Physics, Rendering, Benchmarks} 启用 `AllowUnsafeBlocks`；非热路径项目未启用
- [ ] `TreatWarningsAsErrors` 仅在 CI（`CI=true`）生效，本机开发态不因 warning 阻断
- [ ] `Directory.Packages.props` 启用 CPM，§2 表全部包集中锁定版本；各 `.csproj` 的 `PackageReference` 均不带内联版本；每个版本号附「需核对」标注
...
- [ ] `.vscode/extensions.json` 推荐 C# Dev Kit 与 CMake Tools；`tools/` 构建脚本目录存在
- [ ] `.github/workflows/ci.yml` 存在并定义 6-RID 矩阵 build+test 与 R2R/AOT 双路径 publish 冒烟两段（覆盖架构 R5）
- [ ] `dotnet build PixelEngine.sln -c Release` 在本机零警告通过（全空项目）
- [ ] `dotnet test` 四个测试项目全部通过
- [ ] `dotnet run --project demo/PixelEngine.Demo -c Release` 起一个空窗口或打印 banner 并以 0 退出
- [ ] `dotnet run --project bench/PixelEngine.Benchmarks -c Release` 正常启动且不崩
- [ ] `src/` 下除 `Core/EngineConstants.cs` 空壳外无任何业务 `.cs`；`EngineConstants` 仅声明类型、无常量值（常量归 plan/02）
- [ ] 本文档技术栈与依赖与 plan/00 无冲突，未引入 plan/00 §4 之外的选型，未违背 AGENTS.md §1 任一不变式
...
- [ ] 节点 1：仓库工程化骨架。提交 `global.json`、`nuget.config`、`Directory.Build.props/.targets`、`Directory.Packages.props`、`.gitattributes`、`.vscode/`、补全的 `.gitignore`/`.editorconfig`。提交信息：`build(core): 建立全局 MSBuild 属性、CPM 与工程化配置`
- [ ] 节点 2：解决方案与全部空项目。提交 `PixelEngine.sln` 与 18 个 `.csproj` 骨架、`EngineConstants` 空壳、Demo/tests/bench 最小可运行壳；本机 `dotnet build`/`test`/`run` 冒烟通过。提交信息：`build(core): 建立解决方案与全部项目骨架及依赖方向`
