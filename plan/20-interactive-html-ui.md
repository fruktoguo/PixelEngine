...
- [~] `RmlUiBackend : IGameUiBackend`：已实现 Initialize/LoadDocument(资产→内存 RML)/SetScreenStack(Show/Hide)/Update/Resize/Composite(render)、Feed* 输入转发、DOM `HitTest` 基础命中并接入 Hosting `UiBackendKind.RmlUi`，真实窗口 smoke 已验证文档渲染、基础输入注入与非 modal 空白透传；SetModelValue/DrainEvents 仍待数据桥节点（§3.1、§3.2、§3.5）
