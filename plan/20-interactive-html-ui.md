...
- [~] 节点 8：`feat(ui): 诊断计时 + 事件驱动重绘 + UI cadence 解耦 + 零分配加固 + 降级接入`（§3.3、§3.9、§3.12）。已完成 `ui.update`/`ui.paint`/`ui.upload`/`ui.composite` 计时口径、UI cadence 解耦、静态屏 dirty 门控、禁用/降级基线、UI 零分配 BDN 与过载 UI present cadence 降级；Rendering 侧真实 `UiOverlayTexture` 脏矩形上传契约已闭合，`UiOffscreenSurfacePresenter` 已闭合离屏 surface → dirty upload → textured quad 合成底座，native/Ultralight 后端本体仍待真实依赖/绑定闭合。
