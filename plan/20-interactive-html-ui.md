...
- [~] `RmlUiBackend : IGameUiBackend`：已实现 Initialize/LoadDocument(资产→内存 RML)/SetScreenStack(Show/Hide)/Update/Resize/Composite(render) 并接入 Hosting `UiBackendKind.RmlUi`，真实窗口 smoke 已验证文档渲染；Feed*/DOM HitTest/SetModelValue/DrainEvents 仍待输入与数据桥节点（§3.1、§3.2）
