...
- [x] `GameUiModelBridge` 已按当前文档声明 path 推送 `IUiModel`，隐藏屏幕的 stale model binding 会在下一次绑定或推送时裁剪，不继续占用绑定容量或向不可见文档推送；RmlUi 官方 `DataModelConstructor` 已支持 Empty/Boolean/Int64/Double 标量、dotted path、重复 path 去重和稳定变量名映射。
...
- [!] **Demo 产品面打磨**：HUD/main-menu/settings/pause/result 已有 Web-first content/UI/controller/test 闭合：HUD 暂停按钮进入 `pause`，暂停/设置返回/继续/重开/退出经 `GameUiDemoController` 路由到公开 Runtime facade，Mission 胜负结算会推送 `result` 并发布 numeric result paths，result modal 的重开/退出按钮 headless 路由会清理模态状态；main-menu/settings/inventory/dialog/HUD/pause/result 七屏均显式声明 `data-screen` 与 `data-contract`，`GameUiDemoController` 集中暴露 HUD / result model path 名单，`DemoUiContentTests` 覆盖七屏 manifest、ManagedFallback 显示/交互、headless 屏幕/数据契约、ManagedFallback model path 枚举/写入/读回、公开脚本 UI 服务事件链路、任务胜利 / 失败结算写入与 result modal 控制 facade 闭环。阻塞：真实窗口透明 UI 产品面视频、输入手感、完整通关/真实点击链路与人工体验确认仍未完成。
