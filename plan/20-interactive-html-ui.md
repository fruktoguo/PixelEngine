...
- [x] `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter FullyQualifiedName~GameUi|FullyQualifiedName~InputArbitrator|FullyQualifiedName~DisabledGameUi` 覆盖 Hosting 装配、输入仲裁与禁用零开销；本轮定向运行 `dotnet test D:/BaiduSyncdisk/YuoProject/ClaudeCode/PixelEngine/tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~GameUiServiceBridgeTests"` 通过 9/9，补齐无 manifest 约定路径最终 canonical containment 回归，锁定 `.`、`screens/..`、绝对路径、`..` 逃逸不加载额外 document，且 `screens/hud` / `screens/hud.xhtml` 正常载入。
