...
- [~] native 核 dynamic-only、未静态链、未进 Box2D dual-build；首期 `win-x64`(+可选 `win-arm64`) 的 `runtimes/<rid>/native/` 含 UI native，其余 RID dormant；AOT 通道与未激活 RID 回退 `ManagedFallbackBackend` 且大 UI 仍可用。当前已由 `PixelEngine.UiNative.targets` 禁用 `<NativeLibrary>`、CMake 仅产 `SHARED`、release RID active/dormant 配置、R2R/AOT PowerShell+Bash 发行审计脚本和负向 package 测试锁定；真实 win-x64/win-arm64 release artifact 与 Ultralight profile 证据仍未闭合，因此不标完成（§3.10、§7，配合 plan/15）
