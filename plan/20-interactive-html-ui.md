...
- [x] `GameUiPhaseDriver` 挂相位 0/1：已接入 `Update(renderDt)`、预分配事件 drain、`IGameUiService.UiEventRaised` 派发与 `GameUiModelBridge` 模型推送，UI cadence 不受 sim 降频/TimeScale 影响；`Invoke` 与 UI→世界写入延迟命令队列归节点 7 继续收口（§3.3、§3.4）
