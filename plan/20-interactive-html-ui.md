...
- [x] Demo Web-first HUD 已接入真实任务与武器状态：`GameUiDemoController.OnUpdate` 通过脚本公开 `IGameUiService.SetValue` 发布生命、武器槽位、弹药、冷却/热量、水晶进度、剩余时间、水位危险度与分数；结算 `result` modal 已覆盖任务失败后自动弹出、暂停 runtime、`restart_game` / `quit_game` 走公开 Runtime facade 并清理模态状态；`hud.xhtml` 使用 numeric model paths 与稳定中文 label，保留 `PlayableHud`/`IGuiContext` fallback 诊断路径，且不把未完成的真实窗口点击链路、RmlUi 字符串池能力或真实窗口产品验收标为完成。
...
- [!] **Demo 产品面打磨**：HUD/main-menu/settings/pause/result 已有 Web-first content/UI/controller/test 闭合：HUD 暂停按钮进入 `pause`，暂停/设置返回/继续/重开/退出经 `GameUiDemoController` 路由到公开 Runtime facade，Mission 结算会推送 `result` 并发布 numeric result paths，result modal 的重开/退出按钮 headless 路由会清理模态状态；HUD / pause / result 显式声明 `data-screen` 与 `data-contract`，`GameUiDemoController` 集中暴露 HUD / result model path 名单，`DemoUiContentTests` 覆盖七屏 manifest、ManagedFallback 显示/交互、headless 屏幕/数据契约、ManagedFallback model path 枚举/写入/读回、公开脚本 UI 服务事件链路、任务结算写入与 result modal 控制 facade 闭环。阻塞：真实窗口透明 UI 产品面视频、输入手感、完整通关/真实点击链路与人工体验确认仍未完成。
