...
- [~] `[LibraryImport]` 绑定 shim 的 C-API：已接入 `PixelEngine.UI.Native` resolver、真实 RmlUi 版本探活、GL 注入与 renderer 生命周期入口；context/document/model/input/render、事件/数据回调用 `[UnmanagedCallersOnly]` 待后续切片（§2、§3.4）
