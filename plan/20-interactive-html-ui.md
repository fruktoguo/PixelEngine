...
- [x] vendor RmlUi + FreeType 到 `native/rmlui/`/`native/freetype/`：已以 submodule 固定 RmlUi 6.2 与 FreeType 2.14.3，CMake 接入 `PixelEngine.UI.Native` dynamic-only 目标并静态链接 RmlUi/FreeType，`build-native` 同步到 `runtimes/<rid>/native/`；字体注册、GL 函数注入与桌面 GL3 renderer shim 已落地，ANGLE/GLES 双 profile 仍由节点 4 单独阻塞（§2、§3.10、§7）
- [x] C++ shim：已编入 RmlUi 官方 `RenderInterface_GL3`，以 native handle 管理 renderer create/destroy/viewport 并接入最小 `SystemInterface`；已在 `peui_native_render` 外加 scoped GL 状态守卫，覆盖 program/VAO/VBO/EBO、draw/read framebuffer、active texture 与最多 32 个 texture2D 绑定、blend/scissor/depth/cull/viewport/scissor/unpack alignment，并用真实窗口 GL smoke 验证 render 后恢复；ANGLE/GLES 双 profile 仍由节点 4 单独阻塞（§3.1、§3.2）
- [x] shim GL 函数指针注入：已由 `RenderWindow.TryGetProcAddress` 把当前 Silk.NET/OpenGL context 的 resolver 传给 native `gladLoadGLUserPtr`，并用真实窗口 smoke 验 renderer 创建；Hosting 已在 RmlUi native 不可用或当前 context 为 GLES/ANGLE 时显式回退 ManagedFallback，并注册 `GameUiBackendSelection` 记录请求/实际后端与原因；ANGLE/GLES native renderer profile 仍由节点 4 单独阻塞（§3.2、§3.12）
...
- [x] `[LibraryImport]` 绑定 shim 的 C-API：已接入 `PixelEngine.UI.Native` resolver、真实 RmlUi 版本探活、GL 注入、renderer/context/document/update/render 生命周期、字体注册、鼠标/滚轮/键盘/committed text 输入、DOM `HitTest`、model set/get/copy、action invoke 与事件 drain；IME composition 当前在托管抽象层保留，RmlUi native shim 暂安全忽略预编辑状态，不把 committed text 冒充 composition（§2、§3.4、§3.5）
- [x] `RmlUiBackend : IGameUiBackend`：已实现 Initialize/LoadDocument(资产→内存 RML)/SetScreenStack(Show/Hide)/Update/Resize/Composite(render)、Feed* 输入转发、DOM `HitTest` 基础命中并接入 Hosting `UiBackendKind.RmlUi`；已接入 PixelEngine DOM 绑定桥（`data-model`/`path` → `SetModelValue/TryGetModelValue`，`data-event-click`/`action`/`data-event-change` → `DrainEvents`，`InvokeAction` → native DOM action payload 应用），并为每个文档创建 RmlUi 官方 `DataModelConstructor`，把 `data-model`/`path` 映射为合法变量名后用 `BindFunc` 接入 Empty/Boolean/Int64/Double 标量、dotted path 与重复 path 去重；真实窗口 smoke 已验证文档渲染、基础输入注入、非 modal 空白透传、模型值 set/get、action invoke、点击事件入队、DataModel 标量绑定、committed text 注入，以及 `<img data-image>` 通过受限 PNG→TGA 缓存进入 RmlUi GL3 renderer 合成；ANGLE/GLES native profile 仍由节点 4 单独阻塞（§3.1、§3.2、§3.4、§3.5）
...
输入路由与仲裁：
- [~] `UiInputRouter`：已接入 Hosting 窗口输入源、Silk `KeyChar` 文本环形缓冲、backend `Feed*` 与 `HitTest` capture，并在脚本输入采样前屏蔽游戏通道；已实现 UI 键盘焦点保持（鼠标移出仍接收键盘/文本）、外部点击失焦透传、失焦/上游拦截时释放已按下 UI key edge 并排空陈旧文本；已加固 committed text 多字符顺序、控制字符过滤、上游返回值越界 clamp，且保持稳态零分配；已新增 `UiTextComposition`、`IUiInputSource.CaptureTextComposition`、`IGameUiBackend.FeedTextComposition` 与 `GameUiHost.FeedTextComposition`，router 会把 committed text 与 IME composition 预编辑状态分离转发，并在 composition 失活或上游拦截时清除；`RenderWindowUiInputSource` 明确因 Silk `KeyChar` 只提供 committed text 而返回 inactive composition，避免把 KeyChar 冒充 IME。真实平台 composition 事件接入仍待后续窗口/输入后端支持（§3.5）
- [~] UI 捕获门（与 `EditorInputGate` 同构）：已新增 Hosting 中性 `IEditorInputCaptureSource` 并由 EditorShell 注册，脚本输入路由按 Editor capture → 大 UI capture → Game 顺序合并；大 UI pump 已接收上游 capture 门，Editor/Gui 捕获时不再向大 UI 注入同帧输入；IME composition 抽象已落地但真实平台 composition 事件、候选/预编辑可视化与三后端 `HitTest` 完整一致性仍待后续切片（§3.5）
...
- [~] `UiModelBridge`：已完成 RmlUiBackend 的真实 DOM 数据/事件桥、ManagedFallback 托管模型语义对齐、后端 `CopyModelPaths` 与 Hosting `GameUiModelBridge`（相位 1 前按当前 UI 文档声明 path 从 `IUiModel` 读取并推送）；UI→游戏 世界写入已通过 `UiEvent`→`ScriptEventBus`→`ScriptSimulationContext` 延迟命令队列测试覆盖；`UiModelPathName` 已把 dotted path 映射为合法 ASCII 变量名并追加稳定 hash 后缀规避 `.`/`_` 规范化碰撞，`UiContractTests.UiModelPathNameMapsDottedPathsToLegalCollisionResistantVariables` 覆盖 dotted path、underscore path、中文路径、数字开头路径、空路径拒绝与 `UiPathId` 稳定性；RmlUi 官方 `DataModelConstructor` 已按文档独立模型接入 Empty/Boolean/Int64/Double 标量并由真实窗口 smoke 覆盖 dotted path、重复 path 去重与 set/get；Ultralight JS 对象桥仍待后续切片（Ultralight 归节点 10，可选 profile）（§3.4）
- [~] `UiDiagnostics`：已把 `ui.update`（模型推送/后端 Update/事件 drain）、`ui.paint`（后端实际光栅化/绘制）、`ui.upload`（离屏纹理/脏矩形上传；无上传型后端为 0）与 `ui.composite`（RmlUi/ManagedFallback/脚本 GUI present 层合成）接入 `FrameSubPhase`、`EngineCounters` 与 plan/12 性能 HUD；静态无脏屏 `ui.paint=0` 已有单测覆盖；`GameUiAllocationBenchmarks` 已用 BenchmarkDotNet `MemoryDiagnoser` 覆盖静态 UI phase、clean composite/draw skip 与空闲输入泵，ShortRun 4 项 `Allocated == 0 B`；`UiDirtyRectCollector` 已提供不触 GL 的 headless 脏矩形收集/相邻重叠合并/最小包围，`UiDirtyRectMergeTests` 覆盖几何正确性；过载时 `GameUiHost` 会按质量档位把 UI present cadence 降到 2/3/4 帧间隔，且 `UiPresentationIntervalFrames`/`UiSkippedPresentationFrames` 在 HUD 可见；Rendering 已提供 `UiOverlayTexture` / `UiPresentContext.UploadOverlayTexture` 真实 GL 脏矩形上传入口并计入 `ui.upload`，`UiOffscreenSurfacePresenter` 已把离屏 BGRA8 surface 的 dirty rect 上传与 textured quad 合成接入同一 `UiPresentContext`，真实窗口 smoke 读回 framebuffer 验证通过；Ultralight native 后端本体仍待真实依赖/绑定激活（§3.9）
...
内容与资产：
- [~] `content/ui/` 结构 + `ui-manifest.json` 加载器：已实现纯 I/O + STJ 源生成 manifest 解析、screen id→资产路径映射、preload 标记、重复 id/路径逃逸/缺失文件校验，并接入 `GameUiServiceBridge` 优先按清单解析屏幕；`GameUiServiceBridge` 已在创建时预载 manifest 中 `preload: true` 的屏幕文档且不改变可见屏栈，`BridgePreloadsManifestScreensWithoutShowingThem` 覆盖预载屏复用与非预载屏按需加载；已通过 `UiAssetDirectories` 暴露规范化 `fonts/`、`images/` 目录契约，并支持 `images[]` 图片资产清单的重复 id、路径逃逸、缺失文件校验与 preload 标记；manifest `images[].preload` 已经由 `GameUiHost.PreloadImage` 转发给支持后端，ManagedFallback 会提前调用 GUI host `LoadImage` 上传/缓存，RmlUi 会提前完成 PNG→TGA 缓存，`PreloadImageRoutesToSupportingBackend` 与 `BridgePreloadsManifestImagesThroughManagedFallbackBackend` 覆盖；ManagedFallback 已可从 `content/ui/images/` 解码 8-bit PNG、读取真实尺寸并经当前 `RenderWindow.Gl` 上传 Texture2D 后绘制，`ManagedFallbackDrawsImageControlFromImagesDirectory` 覆盖 `data-image` → PNG → GUI image 调用；RmlUi 已在载入文档时把受限目录内 PNG 转为 GL3 renderer 原生支持的临时 TGA 并改写 `src`，`RmlUiGlBootstrapSmokeTests` 真实窗口覆盖 `<img data-image>` 合成。Ultralight 图片资源加载与脏矩形上传仍待后续切片（§3.8）
- [~] `ManagedUiLayout`：已实现 XHTML 子集 text/button/checkbox/progress/image、`data-event-*`/模型路径契约与根窗口盒模型（`x/y/width/height`、`data-*`、`style left/top/width/height` → `Gui.SetNextWindow`）；`<img>`/`<image>` 支持 `src` 或 `data-image`，并通过共享解析器强制解析到 `content/ui/images/`；RmlUi 后端已复用同一目录约束完成图片消费，完整 CSS 盒模型与 Ultralight 后端图片消费仍待后续切片（§3.8、§3.1）

可选后端（Ultralight profile，gated）：
- [ ] `UltralightBackend : IGameUiBackend`：CPU 离屏 BGRA8 → 脏矩形 `glTexSubImage2D` 上传 → 全屏 alpha quad；JS 全局对象桥（§3.1、§3.4）。当前已完成可复用的 `UiOffscreenSurfacePresenter` 底座（真实上传/合成已验证），但仓库仍无 Ultralight native 依赖、surface API、JS bridge 与许可/发行 gate，不能伪装为后端完成。
- [~] Ultralight native + UltralightNet 绑定登记 plan/00 §4.1 门控、dynamic-only；AOT 不友好时门控排除并回退 `ManagedFallbackBackend`（§5、§7）。当前仓库仍无 Ultralight native 依赖/绑定，Hosting 已把 `UiBackendKind.Ultralight` 作为未激活 profile 显式回退 `ManagedFallbackBackend` 并记录 `GameUiBackendSelection`，`UltralightGameUiBackendFallsBackToManagedWhenGlSmokeIsEnabled` 用真实窗口覆盖；native/绑定/许可 gate 仍未完成。

打包与发行（配合 plan/15）：
- [~] `PixelEngine.UI.Native` 及可选 Ultralight native 落 `runtimes/<rid>/native/`，dynamic-only，不进 Box2D dual-build；首期仅 `win-x64`(+可选 `win-arm64`) 激活，其余 dormant。RmlUi 主后端已通过 `PixelEngine.UiNative.targets`/CMake `SHARED`/`build-native` 复制到 `runtimes/<rid>/native/`，`release-rids.json` 已锁定 win-x64/win-arm64 active、其余 dormant；`audit-release-artifacts.ps1/.sh` 已强制 R2R 包含 UI native、AOT 不携带动态 UI native 并回退 ManagedFallback，`HtmlUiNativePackagingUsesDynamicOnlyRuntimeLayout`、`PackageScriptPlacesRuntimeFilesUnderAppAndAuditRejectsRootClutter`、`PowerShellReleaseArtifactAuditRejectsUiNativeInAotPackage`、`BashReleaseArtifactAuditRequiresUiNativeInR2RPackage` 与 `BashReleaseArtifactAuditRejectsUiNativeInAotPackage` 覆盖 PowerShell/Bash 行为。Ultralight native 与真实 release artifact 证据仍待后续切片（§3.10、§7）
...
- [!] 输入三级仲裁正确：编辑器(Edit) > 大 UI(命中) > 游戏；UI 命中不透明元素时游戏不消费该输入，反之透传；文本/IME 焦点路由正确；切后端仲裁行为不变（§3.5）。阻塞：Editor→大 UI→Game 的上游捕获门、KeyChar committed text、RmlUi/ManagedFallback HitTest 已接入并测试；committed text 已覆盖多字符顺序、控制字符过滤、上游拦截 drain 与越界 clamp；IME composition 抽象已落地并与 committed text 分离，当前 Silk KeyChar 只覆盖提交文本且 `RenderWindowUiInputSource` 显式返回 inactive composition；真实平台 composition 事件、候选/预编辑可视化与 UltralightBackend HitTest 仍未激活，不能伪装为完成。
- [x] C#↔UI 双向：游戏态变化经 `SetValue`/`BindModel`/data-model 反映到 UI，UI 交互（如"开始游戏"）产生 `UiEvent` 并经脚本事件总线在相位 1 派发，事件触发的世界写入经延迟队列落正确相位；禁用 HTML UI 时 `GameUi` 为 no-op 且调用安全；`Invoke` 已接入 ManagedFallback 与 RmlUi native action 通道，RmlUi 官方 `DataModelConstructor` 标量绑定已覆盖 Empty/Boolean/Int64/Double、dotted path 与重复 path 去重，并通过托管单测 + 真实窗口 RmlUi smoke 验证（§3.4、#6）
- [~] 事件驱动重绘生效：ManagedFallback/RmlUi 的 `IsDirty` 已改为真实 dirty/animation，`GameUiHost` 只在 dirty/animation 时调用后端 Composite/DrawGui，静态屏稳态 `ui.paint=0` 已由 `GameUiHostTests`/`ManagedFallbackBackendTests` 覆盖；HUD 数值变化触发重绘已覆盖；离屏 surface 公用 presenter 的局部 dirty rect 上传与合成已由 `UiOffscreenSurfacePresenterSmokeTests` 真实窗口覆盖，RmlUi 几何路径无上传，Ultralight native 后端接入仍待后续切片（§3.2、§3.9）
- [x] UI cadence 与 sim 频率解耦：sim 降到 30Hz 时 UI 动画仍按渲染 cadence 平滑推进、不加倍不卡顿；UI 尖刺只掉渲染帧、sim 固定步长不受累（§3.3、#6）。已用 `GameUiPhaseDriverUpdatesEveryRenderFrameAndDrainsEvents` 覆盖 sim 跳帧与 TimeScale<1 时 UI 仍消费未缩放 render dt；`OverloadThrottlesGameUiPresentWithoutSkippingUiUpdate` 覆盖过载时只降低 UI present cadence、不跳过 UI update/event drain。
...
- [~] native 核 dynamic-only、未静态链、未进 Box2D dual-build；首期 `win-x64`(+可选 `win-arm64`) 的 `runtimes/<rid>/native/` 含 UI native，其余 RID dormant；AOT 通道与未激活 RID 回退 `ManagedFallbackBackend` 且大 UI 仍可用。当前已由 `PixelEngine.UiNative.targets` 禁用 `<NativeLibrary>`、CMake 仅产 `SHARED`、release RID active/dormant 配置、R2R/AOT PowerShell+Bash 发行审计脚本和负向 package 测试锁定（含 PowerShell AOT 禁带 UI native 实际执行测试）；真实 win-x64/win-arm64 release artifact 与 Ultralight profile 证据仍未闭合，因此不标完成（§3.10、§7，配合 plan/15）
- [x] 禁用大 UI 开关后主循环无 UI 开销；降级路线可切到"全 ManagedFallback"或"静态屏 RmlUi + ManagedFallback HUD"，均不改 Demo 逻辑。`DisabledGameUiDoesNotRegisterRuntimeServicesWhenWindowIsAttached` 已用真实窗口证明禁用时不注册 `GameUiHost`/`GameUiPhaseDriver`/`IGameUiService`/`GameUiBackendSelection`，且 `ui.update`/`ui.composite` 计数保持 0；`GameUiBackendSelectionIsRecordedWhenGlSmokeIsEnabled` 覆盖 RmlUi 不可用/GLES 时显式回退 ManagedFallback；`UltralightGameUiBackendFallsBackToManagedWhenGlSmokeIsEnabled` 覆盖未激活 Ultralight profile 不崩溃、不伪造后端，而是记录原因并回退 ManagedFallback；`DemoGameUiControllerIsSafeWhenGameUiServiceIsNoop` 验证 Demo 控制器落到 Noop 服务不改玩法逻辑（§3.12）
...
- [x] 公开 API 全带中文 XML 文档注释已由 `UiContractTests.UiPublicApiMembersHaveChineseXmlDocumentation` 全量扫描 `src/PixelEngine.UI` 顶层源码锁定，要求 public 类型 / 成员前置中文 XML 且禁止 `<inheritdoc />` 充当公开 API 文档；内部 ManagedFallback 布局 DTO 与 source-gen 上下文已收窄为 `internal`，避免误暴露为 UI 公共面；UI 逻辑相位稳态零托管堆分配已由 `GameUiAllocationBenchmarks` 覆盖，BenchmarkDotNet ShortRun `MemoryDiagnoser` 在静态 UI phase、clean composite/draw skip 与空闲输入泵 4 项均报告 `Allocated == 0 B`（§3.9）
...
- [!] 节点 6：`feat(ui): 输入三级仲裁(编辑器>大UI>游戏) + WantCapture 门 + 三后端 HitTest 一致`（§3.5、§3.6）。阻塞：已修复上游 capture 门与 UI key/text 状态清理；committed text 已加固多字符顺序、控制字符过滤、上游拦截 drain 与越界 clamp；IME composition 抽象已新增并与 committed text 分离，Silk `KeyChar` 路径显式保持 inactive composition；真实平台 composition 事件/预编辑可视化与 UltralightBackend HitTest 尚无真实实现入口。
- [x] 节点 7：`feat(ui): IGameUiService(契约在 Scripting)C#↔UI 双向数据/事件桥 + 相位安全写入`（§3.4）
- [~] 节点 8：`feat(ui): 诊断计时 + 事件驱动重绘 + UI cadence 解耦 + 零分配加固 + 降级接入`（§3.3、§3.9、§3.12）。已完成 `ui.update`/`ui.paint`/`ui.upload`/`ui.composite` 计时口径、UI cadence 解耦、静态屏 dirty 门控、禁用/降级基线、UI 零分配 BDN、过载 UI present cadence 降级，以及 RmlUi/GLES/未激活 Ultralight profile 的 `GameUiBackendSelection` 显式回退；Rendering 侧真实 `UiOverlayTexture` 脏矩形上传契约已闭合，`UiOffscreenSurfacePresenter` 已闭合离屏 surface → dirty upload → textured quad 合成底座，native/Ultralight 后端本体仍待真实依赖/绑定闭合。
