...
- [x] `BackendConformanceTests` 已补齐 `IGameUiBackend` headless 共享契约自动化：锁定 Initialize/Resize/LoadDocument/SetScreenStack、ManagedFallback 非交互透明区 pass-through / 交互控件 capture、HitTest→`UiInputCapture` 语义、FeedText 与 FeedTextComposition 分离、CopyModelPaths 缓冲不足稳定截断、DrainEvents destination capacity / ring overflow 边界；该契约只约束当前可本地执行的 ManagedFallback / host 输入路径，不替代 RmlUi native smoke、UltralightBackend 本体、真实平台 IME composition 或真实窗口产品验收。
...
- [x] `dotnet test tests/PixelEngine.UI.Tests/PixelEngine.UI.Tests.csproj -c Release --filter FullyQualifiedName~GameUiHostTests|FullyQualifiedName~UiInputRouterTests|FullyQualifiedName~GameUiServiceBridgeTests|FullyQualifiedName~BackendConformance` 覆盖宿主、输入、服务桥与后端一致性基线；`BackendConformanceTests` 额外锁定 `IGameUiBackend` headless 共享契约、ManagedFallback 非交互透明区 pass-through / 交互控件 capture、committed text / composition 分离、CopyModelPaths 截断、DrainEvents 边界与临时 XHTML 清理；`UiInputRouterTests` 额外锁定 committed text 不冒充 IME composition，以及 `UiTextCompositionCapabilities` 从输入源透传诊断。2026-07-07 本地运行 `dotnet test tests/PixelEngine.UI.Tests/PixelEngine.UI.Tests.csproj -c Release --filter "FullyQualifiedName~BackendConformance|FullyQualifiedName~ManagedFallbackBackendTests|FullyQualifiedName~GameUiHostTests|FullyQualifiedName~UiInputRouterTests"` 通过 32/32。
