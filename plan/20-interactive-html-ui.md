...
- [x] `dotnet test tests/PixelEngine.UI.Tests/PixelEngine.UI.Tests.csproj -c Release --filter FullyQualifiedName~GameUiHostTests|FullyQualifiedName~UiInputRouterTests|FullyQualifiedName~GameUiServiceBridgeTests|FullyQualifiedName~BackendConformance` 覆盖宿主、输入、服务桥与后端一致性基线；`BackendConformanceTests` 额外锁定 `IGameUiBackend` headless 共享契约、ManagedFallback 非交互透明区 pass-through / 交互控件 capture、committed text / composition 分离、SetModelValue/TryGetModelValue、InvokeAction、UnloadDocument 后安全边界、非法 viewport finite-positive DPI 诊断、CopyModelPaths 截断、DrainEvents 边界与临时 XHTML 清理；`RmlUiGlBootstrapSmokeTests` 额外登记 RmlUi modal unload 后不再 capture 输入的真实后端 smoke；`UiInputRouterTests` 额外锁定 committed text 不冒充 IME composition，以及 `UiTextCompositionCapabilities` 从输入源透传诊断。2026-07-07 本地运行 `dotnet test tests/PixelEngine.UI.Tests/PixelEngine.UI.Tests.csproj -c Release --filter "FullyQualifiedName~BackendConformance|FullyQualifiedName~ManagedFallbackBackendTests|FullyQualifiedName~GameUiHostTests|FullyQualifiedName~UiInputRouterTests|FullyQualifiedName~RmlUiGlBootstrapSmokeTests"` 通过 39/39。
