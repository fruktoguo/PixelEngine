...
模块骨架与后端抽象：
- [ ] 建 `src/PixelEngine.UI/PixelEngine.UI.csproj`，`TargetFramework=net10.0`，依赖 `Gui`/`Rendering`/`Core`（依赖方向遵 plan/00 §5，无反向，无 Editor/Scripting 边），仅 native 互操作/纹理上传所需最小 unsafe 块开 `AllowUnsafeBlocks`（§2）
- [ ] 定义 `IGameUiBackend`、`UiValue`/`UiEvent`/`UiDocumentSource`/`UiHitResult`/`UiViewport`/`UiBackendKind` 等 blittable 契约与句柄类型（§3.1）
- [ ] 在 `PixelEngine.Scripting` 声明 `IGameUiService` 及脚本可见契约（`IWorldEffects` 同栈同款），Hosting 实现并挂 `EngineContext.GameUi`（§2、§3.4）
- [ ] `GameUiHost` 门面 + `UiDocumentManager`（屏栈/显隐/模态/返回栈）（§3.1、§3.8）
- [ ] 可编译/运行时开关整体禁用大 UI，禁用后零开销（§1.4、§3.3）

基线后端（纯托管，永远可用）：
- [ ] `ManagedFallbackBackend : IGameUiBackend`：`ManagedUiLayout`（HTML→抽象控件树）→ plan/19 `PixelEngine.Gui` 中性 host 即时模式绘制；`Composite` 提交 draw data；`HitTest`/`WantCapture` 对齐 Gui host（§3.1、§3.11）
- [ ] 与既有玩家 GUI facade 调和：共享同一 Gui host 实例，`DemoHud`/`PauseMenu`/`PlayableHud`/`Behaviour.OnGui` 不重写、不产生平行 API（§3.11）

主后端（RmlUi，Windows 优先）：
- [ ] vendor RmlUi + FreeType 到 `native/rmlui/`，CMake 静态链 FreeType 进单一 `PixelEngine.UI.Native` 动态库；**首期仅 `win-x64`（可含 `win-arm64`）toolchain，dynamic-only，不做 static 变体**；其余 RID 保留矩阵位 dormant（§2、§3.10、§7）
- [ ] C++ shim：实现 RmlUi `RenderInterface`（复用当前 GL 上下文直接 draw，无逐 draw-call 托管回调）、`SystemInterface`、`FontEngineInterface`(FreeType)；进出遵 GL 状态 save/restore 协议（§3.1、§3.2）
- [ ] shim GL 函数指针注入：托管侧把 plan/08 已解析的最小 GL 入口表传入 shim，桌面 GL 与 ANGLE/GLES3 共用同一 loader；双 profile 着色器（330 core / 300 es）（§3.2）
- [ ] `[LibraryImport]` 绑定 shim 的 C-API（context/document/model/input/render）；事件/数据回调用 `[UnmanagedCallersOnly]`（AOT 友好）（§2、§3.4）
- [ ] `RmlUiBackend : IGameUiBackend`：LoadDocument/Update/IsDirty/Feed*/HitTest/SetModelValue/DrainEvents/Composite（几何重放路径）（§3.1、§3.2）

合成与相位集成：
- [ ] **plan/08 新增** `RenderPipeline.RegisterUiLayer(int order, IUiCompositeLayer)` 显式带序号 UI 层注册（替代多播订阅顺序）；本模块此项为硬前置（§3.2、§8）
- [ ] `UiLayerCompositor : IUiCompositeLayer` 以 `order=100` 注册；仅在脏/动画时光栅化，合成后恢复 GL 状态、启用 alpha blend（§3.2）
- [ ] 编辑器 `EditorRenderBridge` 以 `order=200` 注册，保证 UI(100)<编辑器(200)（§3.2、§3.6）
- [ ] `GameUiPhaseDriver` 挂相位 0/1：输入泵入、游戏态→UI、UI 事件派发、`Update(renderDt)`；UI cadence 只认渲染 dt、sim 降频不影响（§3.3）
- [ ] `EngineBuilder.EnableGameUi(bool)` + `UseUiBackend(UiBackendKind)`；禁用零开销（§3.3）

输入路由与仲裁：
- [ ] `UiInputRouter`：Silk.NET 输入 → `backend.Feed*`；由 `HitTest`+焦点算 `UiWantCaptureMouse/Keyboard`（§3.5）
- [ ] UI 捕获门（与 `EditorInputGate` 同构）：编辑器 → 大 UI → 游戏 三级仲裁；文本/IME 焦点路由；三后端 `HitTest` 语义一致（§3.5）

C#↔UI 通信：
- [ ] `IGameUiService` 后端实现（Hosting）：ShowScreen/HideScreen/PushModal/BindModel/SetValue/TryGetValue/UiEventRaised/Invoke（§3.4）
- [ ] `UiModelBridge`：RmlUi data-model / Ultralight JS 对象 / ManagedFallback 托管模型三映射；UI→游戏 世界写入经 plan/11 延迟命令队列落正确相位（§3.4）
...
内容与资产：
- [ ] `content/ui/` 结构 + `ui-manifest.json` 加载器（纯 I/O + STJ 源生成，无反射回退）（§3.8）
...
- [ ] 大 UI 在 plan/08 同一 GL 上下文、世界渲染之后经 `RegisterUiLayer(100,…)` 合成、编辑器 ImGui(200) 在其上；合成后 GL 状态被正确恢复（含 shim 侧 save/restore），世界画面无被 UI 污染（§3.2）
...
- [ ] C#↔UI 双向：游戏态变化经 `SetValue`/data-model 反映到 UI；UI 交互（如"开始游戏"）产生 `UiEvent` 并在相位 1 派发；世界写入经延迟队列落正确相位、不破坏帧节奏（§3.4、#6）
...
- [ ] 节点 1：`feat(ui): PixelEngine.UI 骨架 + IGameUiBackend 抽象 + 文档/屏管理`（§3.1、§3.8）
- [ ] 节点 2：`feat(ui): ManagedFallbackBackend 纯托管基线(复用 PixelEngine.Gui host，统一现有 IGuiContext 玩家 HUD)`（§3.1、§3.11）
...
- [ ] 节点 7：`feat(ui): IGameUiService(契约在 Scripting)C#↔UI 双向数据/事件桥 + 相位安全写入`（§3.4）
