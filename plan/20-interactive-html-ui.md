...
- [~] `UiDiagnostics`：已把 `ui.update`（模型推送/后端 Update/事件 drain）、`ui.paint`（后端实际光栅化/绘制/纹理上传）与 `ui.composite`（RmlUi/ManagedFallback/脚本 GUI present 层合成）接入 `FrameSubPhase`、`EngineCounters` 与 plan/12 性能 HUD；静态无脏屏 `ui.paint=0` 已有单测覆盖；UI 过载降级联动与零分配 BDN 仍待后续切片（§3.9）
...
- [~] 节点 8：`feat(ui): 诊断计时 + 事件驱动重绘 + UI cadence 解耦 + 零分配加固 + 降级接入`（§3.3、§3.9、§3.12）。已完成 `ui.update`/`ui.paint`/`ui.composite` 计时、UI cadence 解耦、静态屏 dirty 门控与禁用/降级基线；零分配 BDN、UI 过载降级与 native/Ultralight 脏矩形上传仍待闭合。
