...
C#↔UI 通信：
- [~] `IGameUiService` 后端实现（Hosting）：已接入 ShowScreen/HideScreen/PushModal/BindModel/SetValue/TryGetValue/UiEventRaised 与 Scripting 注入；禁用 HTML UI 时 `IScriptContext.GameUi` / `ScriptSimulationContext.GameUi` 返回 `NoopGameUiService`，脚本调用安全静默且不回传假数据；Invoke 仍等待后端/native invoke 通道（§3.4）
...
- [~] C#↔UI 双向：游戏态变化经 `SetValue`/`BindModel`/data-model 反映到 UI，UI 交互（如"开始游戏"）产生 `UiEvent` 并在相位 1 派发；禁用 HTML UI 时 `GameUi` 为 no-op 且调用安全；`Invoke` 与 UI 事件触发世界写入经延迟队列落正确相位仍待后续切片（§3.4、#6）
