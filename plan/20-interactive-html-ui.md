...
- [~] `GameUiPhaseDriver` 挂相位 0/1：已接入 `Update(renderDt)`、预分配事件 drain 与 `IGameUiService.UiEventRaised` 派发，UI cadence 不受 sim 降频/TimeScale 影响；游戏态→UI 模型桥待后续切片（§3.3）
- [x] `EngineBuilder.EnableGameUi(bool)` + `UseUiBackend(UiBackendKind)`；禁用零开销（§3.3）

输入路由与仲裁：
- [~] `UiInputRouter`：已接入 Hosting 窗口输入源、Silk `KeyChar` 文本环形缓冲、backend `Feed*` 与 `HitTest` capture，并在脚本输入采样前屏蔽游戏通道；IME、焦点保持与 Editor 优先级统一门待后续切片（§3.5）
- [~] UI 捕获门（与 `EditorInputGate` 同构）：已新增 Hosting 中性 `IEditorInputCaptureSource` 并由 EditorShell 注册，脚本输入路由按 Editor capture → 大 UI capture → Game 顺序合并；IME、焦点保持与三后端 `HitTest` 完整一致性仍待后续切片（§3.5）

C#↔UI 通信：
- [~] `IGameUiService` 后端实现（Hosting）：已接入 ShowScreen/HideScreen/PushModal/SetValue/TryGetValue/UiEventRaised 与 Scripting 注入；BindModel/Invoke 仍等待 UiModelBridge/后端 invoke 通道（§3.4）
- [~] `UiModelBridge`：已完成 RmlUiBackend 的真实 DOM 数据/事件桥与 ManagedFallback 托管模型语义对齐；RmlUi 官方 `DataModelConstructor`、dotted path 到合法变量名映射、Ultralight JS 对象桥、统一 `UiModelBridge` 调度与 UI→游戏 世界写入经 plan/11 延迟命令队列落正确相位仍待后续切片（§3.4）
