...
- [~] `UiDiagnostics`：已把 `ui.update`（模型推送/后端 Update/事件 drain）、`ui.paint`（后端实际光栅化/绘制/纹理上传）与 `ui.composite`（RmlUi/ManagedFallback/脚本 GUI present 层合成）接入 `FrameSubPhase`、`EngineCounters` 与 plan/12 性能 HUD；静态无脏屏 `ui.paint=0` 已有单测覆盖；`GameUiAllocationBenchmarks` 已用 BenchmarkDotNet `MemoryDiagnoser` 覆盖静态 UI phase、clean composite/draw skip 与空闲输入泵，ShortRun 4 项 `Allocated == 0 B`；过载时 `GameUiHost` 会按质量档位把 UI present cadence 降到 2/3/4 帧间隔，且 `UiPresentationIntervalFrames`/`UiSkippedPresentationFrames` 在 HUD 可见；native/Ultralight 独立 `ui.upload` 与脏矩形上传仍待后续切片（§3.9）
