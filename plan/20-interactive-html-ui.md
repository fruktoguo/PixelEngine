...
- [~] 与既有玩家 GUI facade 调和：共享同一 Gui host 实例，`DemoHud`/`PauseMenu`/`PlayableHud`/`Behaviour.OnGui` 不重写、不产生平行 API（§3.11）。已新增 `PixelEngine.Gui` 中性 `IGuiDrawContext`/`DrawManagedFrame`，但脚本 HUD 与 ManagedFallback 同帧聚合仍待 Hosting 桥接。
...
- [~] 节点 2：`feat(ui): ManagedFallbackBackend 纯托管基线(复用 PixelEngine.Gui host，统一现有 IGuiContext 玩家 HUD)`（§3.1、§3.11）。后端闭环已完成；统一现有 `IGuiContext` 玩家 HUD 的 Hosting 聚合待后续切片。
