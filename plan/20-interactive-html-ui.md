...
- [~] 事件驱动重绘生效：ManagedFallback/RmlUi 的 `IsDirty` 已改为真实 dirty/animation，`GameUiHost` 只在 dirty/animation 时调用后端 Composite/DrawGui，静态屏稳态 `ui.paint=0` 已由 `GameUiHostTests`/`ManagedFallbackBackendTests` 覆盖；HUD 数值变化触发重绘已覆盖，RmlUi/Ultralight 局部脏矩形上传仍待后续切片（§3.2、§3.9）
...
- [~] 节点 8：`feat(ui): 诊断计时 + 事件驱动重绘 + UI cadence 解耦 + 零分配加固 + 降级接入`（§3.3、§3.9、§3.12）。已完成 `ui.update`/`ui.paint`/`ui.upload`/`ui.composite` 计时口径、UI cadence 解耦、静态屏 dirty 门控、禁用/降级基线、UI 零分配 BDN 与过载 UI present cadence 降级；Rendering 侧真实 `UiOverlayTexture` 脏矩形上传契约已闭合，native/Ultralight 后端调用链仍待闭合。
