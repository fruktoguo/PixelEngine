...
- [x] `dotnet test tests/PixelEngine.Hosting.Tests/PixelEngine.Hosting.Tests.csproj -c Release --filter "FullyQualifiedName~EditorShellGameViewContractTests|FullyQualifiedName~InputArbitratorTests"` 覆盖 EditorShell Game View image rect / fit scale / panel-local→viewport 映射、面板空白区阻断、图像内透明区 pass-through、交互 UI capture、`GameViewUiInputSource` mapped viewport 坐标进入 `UiInputRouter` hit-test / pointer feed 与输入仲裁回归，2026-07-07 本地通过 17/17。
