...
目标硬件长基准完成后，可提供 `-EvidenceManifestPath <json>`。manifest 使用 `schemaVersion: 1`，并且只能包含 `targetHardwareReport`、`cpuProbeReport`、`gpuProbeReport`、`comparisonReport` 四个 evidence scope；每个 entry 必须声明 `path` 与 `sha256`，脚本会重新计算文件 SHA256 并比对。

`targetHardwareReport` 必须包含机器可读字段 `targetGpuName`、`targetGpuDriver`、`gpuBackend`、`operatingSystem`、`cpuName`、`dotnetVersion`、`gitCommit`、`particleCount` 与 `benchmarkRunId`，且 `particleCount` 必须与 probe 的 `requested_count` 一致。`cpuProbeReport`、`gpuProbeReport` 与 `comparisonReport` 也必须声明同一个 `benchmarkRunId` 与 `gitCommit`，避免把不同提交或不同采样 run 的片段拼成一份伪证据。`cpuProbeReport` 与 `gpuProbeReport` 必须各自包含一行 `particle_frame_probe` summary，且 `requested_count` / `active_count` 至少为 100000、二者相等、`measured_frames` 至少为 300；CPU 报告必须是 `mode=cpu`、`particle_stamp_avg_ms>0`、`gpu_particle_avg_ms=0`，GPU 报告必须是 `mode=gpu`、`gpu_available=True`、`gpu_particle_avg_ms>0`、`particle_stamp_avg_ms=0`，避免把 CPU 回退或混淆输出当作目标 GPU 证据。

`comparisonReport` 必须包含机器可读字段 `gpuFasterThanCpu: true`、`cpuWallAvgMs`、`gpuWallAvgMs`、`speedupRatio`、`measuredFrames`、`sampleSeconds`；其中 `cpuWallAvgMs` / `gpuWallAvgMs` 必须与 CPU/GPU probe summary 的 `wall_avg_ms` 一致（允许极小格式化容差），`cpuWallAvgMs` 必须大于 `gpuWallAvgMs`，`speedupRatio` 必须大于 1 且等于 `cpuWallAvgMs / gpuWallAvgMs` 的重算值，`measuredFrames` 必须至少为 300 且不大于 CPU/GPU probe 报告的 `measured_frames`，`sampleSeconds` 必须至少为 10 秒且不能大于 CPU/GPU probe summary 的 `sample_seconds`，避免手写 comparison 夸大采样时长。缺 scope 时报告 `blocked_missing_target_gpu_scope_evidence`；schema、未知 scope、缺文件、sha256 不匹配、硬件字段缺失、probe summary 不达标或 comparison 语义不达标等清单错误会写出 `blocked_invalid_target_gpu_evidence` 报告并以 5 退出；证据齐全时报告 `target_gpu_evidence_attached_pending_review`，仍需人工复核目标硬件、采样时长、驱动状态和报告结论，不能自动勾选 plan/09。
