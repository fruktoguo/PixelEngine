...
`tools/PixelEngine.Tools.ManagedNativeLeakDetector` 是当前本机 managed 证据生成器：它生成 `evidence.json` 与 `gl`/`openal`/`box2d`/`alc` 四个 report，并能被 `tools/native-leak-preflight.ps1 -EvidenceManifestPath ... -AllowBlocked` 索引到 `detector_evidence_attached_pending_review`。其中 OpenAL 通过真实 `OpenAlDevice` 或 OpenAL 不可用时的 `NullAudioBackend` 采集 live object 计数，Box2D 通过 owned `PhysicsSystem` shutdown 后的 `LiveBodyCount` 采集，ALC 通过 `ScriptHotReloadController` 多次 reload 后的 unload/GC 计数采集；GL scope 会先尝试创建真实 `RenderWindow` GL context，并分配/释放 `GlTexture`、`GlBuffer`、`Framebuffer` 与 `ShaderProgram` 等 Rendering-owned wrapper 资源，成功时写入 `coverage=gl_context_rendering_wrappers` 并读取 `GlResourceTracker.Snapshot().Total`。若当前宿主无法创建 GL context，则明确降级写入 `managed_no_gl_context` 与失败原因。两种 managed 覆盖都仍需要外部 GL driver 级 native leak detector 或人工工具报告闭合 plan/18。
