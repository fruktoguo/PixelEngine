...
更严格的工具级证据入口是 `-EvidenceManifestPath`。manifest 需要使用 `schemaVersion: 1`，只允许 `gl`、`openal`、`box2d` 与 `alc` 四类 scope，并为各自 report 文件声明 `sha256`；脚本会重新计算文件 SHA256 并比对。每份 detector report 除 `scope`、`detector`、`conclusion=no_leaks` 外，还必须声明对应释放后 live-object 计数为 0：`glObjectsLiveAfterShutdown`、`openAlObjectsLiveAfterShutdown`、`box2DBodiesLiveAfterShutdown`、`alcLoadContextsAliveAfterUnload`。OpenAL 后端已暴露 `LiveSourceCount`、`LiveBufferCount` 与 `LiveObjectCount`，供本机 detector 在 `AudioSystem.Shutdown()` 后采集 `openAlObjectsLiveAfterShutdown`。四类证据齐全时状态为 `detector_evidence_attached_pending_review`，仍不自动代表无泄漏验收通过；未传 `-AllowBlocked` 时保持非零退出，避免把 pending review 当成 plan/18 验收完成。缺任一 scope/report/hash 时状态为 `blocked_missing_scope_evidence`；JSON、schema、未知 scope、hash 不匹配或 live-object 计数缺失/非 0 时状态为 `blocked_invalid_native_leak_evidence`。
