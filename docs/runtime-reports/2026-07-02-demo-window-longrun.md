...
`tools/PixelEngine.Tools.ManagedNativeLeakDetector` 是当前本机 managed 证据生成器：它生成 `evidence.json` 与 `gl`/`openal`/`box2d`/`alc` 四个 report，并能被 `tools/native-leak-preflight.ps1 -EvidenceManifestPath ... -AllowBlocked` 索引到 `detector_evidence_attached_pending_review`。其中 OpenAL 通过真实 `OpenAlDevice` 或 OpenAL 不可用时的 `NullAudioBackend` 采集 live object 计数，Box2D 通过 owned `PhysicsSystem` shutdown 后的 `LiveBodyCount` 采集，ALC 通过 `ScriptHotReloadController` 多次 reload 后的 unload/GC 计数采集；GL scope 读取 `GlResourceTracker.Snapshot().Total` 作为引擎 GL 封装层 in-process live count，并明确写入 `managed_no_gl_context`，说明托管 detector 没创建 GL context，仍需要外部 GL driver 级 native leak detector 或人工工具报告闭合 plan/18。
