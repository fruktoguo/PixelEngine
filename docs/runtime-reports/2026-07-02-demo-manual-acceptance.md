...
该入口会运行默认可玩程序化场景、主场景、route-attempt、Editor 窗口、通关、生命、相机、反应/温度、音频、粒子/光照十个 scripted window probe，并要求各自输出关键摘要 marker，例如 `player_visual=present`、`playable_shots=`、`fps=`、`frame_p99_ms=`、`frame_low1_fps=`、`720x480`、`brush_material=`、`pause_open=False`、`editor_enabled=True`、`editor_running=True`、`editor_bridge_frames=`、`goal_reached=True`（通关专项 probe）、`damage_events=`、`camera_followed=True`、`reactions_observed=True`、`audio_probe_one_shot_played=True` 与 `particle_light_probe_depleted=True`。脚本还会解析摘要键值并检查语义阈值，例如默认可玩场景必须至少开火一次、产生粒子、积累相机/地面/空中样本并确认空中控制；route-attempt 必须有足够帧样本、玩家横向位移和渲染相机同步；editor-window 必须通过 `apps/PixelEngine.Editor.Shell` 的 `--window-ticks` / `--scripted-probe` 入口启动并观测到 Editor app 初始化、面板注册、Editor render bridge 实际绘制帧和渲染相机同步，Demo 进程不再提供 `--editor` 内嵌编辑器入口；goal/health/camera/reaction/audio/particle-light 专项 probe 分别要求通关、受伤、相机跟随、反应/相变、音频限流和粒子光照字段达到最小值。每个 probe 还会传入 `--capture-frame <probe>/capture.bmp`，并在报告中记录 `capture.bmp` 的 path、sha256、byte size、width、height、bits-per-pixel 与 `capture_unique_visible_pixels`；没有写出有效 BMP，或截图为全透明、黑/白空白、近似纯色画面，会直接失败。其中默认可玩程序化场景 probe 覆盖首屏玩家可见、相机跟随、固定内部渲染分辨率、按住左键发射破坏弹、长窗口帧率诊断与自由粒子进入真实窗口链路；route-attempt 使用 `--scripted-window-route` 在默认关卡中执行较长脚本化路线，只要求输出 `goal_reached=` 字段与长跑状态，用来捕捉移动、碰撞、破坏重建与触发器同步问题；editor-window 只证明 Editor/HUD 真实窗口链路不是空跑，仍不能替代 `hudMenuEditorVideo` 中的人工菜单点击与 dockspace 操作证据。这些 marker、语义阈值与截图只证明真实窗口相位链路中的机器探针仍可复现，不证明视觉质量、听感、手感或真实玩家体验通过。
...
最小 manifest / entry 结构示例：

```json
{
  "reviewSessionId": "session-20260704-demo-001",
  "gitCommit": "<git-sha>",
  "evidence": [
    {
