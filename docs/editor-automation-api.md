...
CLI 的每次启动都是一条独立连接；连接断开时 Server 会丢弃尚未 commit 的 transaction。
因此多个 `pixelengine-editor` 进程不能用“一个 begin、若干 `--transaction`、一个 commit”拼接
事务。CLI 的原子写入口是单进程 `transaction execute --plan-file`，它在同一认证 session 内
完成 begin→按序 stage→commit，任一步失败会在同一 session 尝试 rollback：

```json
{
  "schemaVersion": 1,
  "name": "Create authoring objects",
  "leaseMilliseconds": 60000,
  "idempotencyKey": "authoring-batch-42",
  "operations": [
    {
      "method": "hierarchy.gameObject.create",
      "payload": { "schemaVersion": 1, "name": "Automation Marker" },
      "idempotencyKey": "authoring-batch-42-create-marker"
    }
  ]
}
```
...
```powershell
pixelengine-editor --discovery-root $discovery `
  --scopes 'editor.read,editor.control,project.write' --output json `
  transaction execute --plan-file transaction.json
```

计划严格拒绝未知字段，允许 1..256 个 operation、1..300000 ms lease；每个 operation 必须是
descriptor 声明可 transaction staging 的 write，并有批内唯一幂等 key。需要逐步控制 begin、
stage、commit 的调用方应使用同一个长连接 `.NET Client`；`--transaction <id>` 只适用于该
长连接 session 内 descriptor 标记为 reversible 的 operation。
...
`tools/update-final-output.ps1` 会将已发布且验证过的自动化表面放入：

```text
最终输出/自动化/CLI/pixelengine-editor.exe
最终输出/自动化/SDK/PixelEngine.Editor.Automation.Protocol.<version>.nupkg
最终输出/自动化/SDK/PixelEngine.Editor.Automation.Client.<version>.nupkg
最终输出/自动化/Schema/
最终输出/自动化/文档/editor-automation-api.md
最终输出/自动化/Skill/pixelengine-editor/
最终输出/_验证记录/editor-automation-e2e/report.json
```

替换正式目录前，脚本必须启动全新的外部 Editor，并且每个动作都通过独立
`pixelengine-editor` 进程完成 transaction→Undo/Redo→两次 Play/debug/Stop→再次修改与保存
→Build→Player 启动/存活验证/终止→Editor 公共退出。E2E 不允许 skipped；独立 verifier 会
重算发布矩阵、NuGet/Skill 文件集、Editor/CLI 身份以及每次 CLI operation stdout/stderr 的
SHA256。详见 [`docs/final-output.md`](final-output.md)。
