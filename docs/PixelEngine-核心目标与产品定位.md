...
- **不是传统 UGUI 复刻**：GameObject、Inspector、Prefab 等 authoring 体验可类 Unity；游戏 UI 技术方向应以 Web-first 透明 HTML/CSS 为主。
...
- **Game View**：运行态游戏视图，显示最终玩家看到的画面与透明 HTML UI。
- **Console**：脚本错误、构建日志、资源导入、诊断输出。
- **Toolbar**：Play / Pause / Step、保存、构建入口、模式状态。
...
- **Player Settings**：窗口标题、分辨率、VSync、图标、版本号、启动场景、输入默认、运行时 UI 配置、发行通道。
