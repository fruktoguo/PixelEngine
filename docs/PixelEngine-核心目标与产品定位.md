...
- **不是大型内容型游戏项目**：Demo 应是完整、可玩、有反馈的开放式无限沙盒，但不以堆关卡、剧情、成长、武器数量或 roguelite 内容量为目标。
...
- **Scene View**：独立编辑相机视图，支持点选、gizmo、画刷和调试叠层；Edit 时读取 authoring world，Play/Paused 时无缝切到同一活动 runtime world，不得停留在进入 Play 前的静态副本。
- **Game View**：游戏相机与最终 presentation 视图；Edit 时持续显示当前 authoring world 的游戏构图预览而不是黑屏，并忽略尚未由 runtime 脚本 reveal 的 fog mask，Play/Paused 时恢复真实 lighting/fog 并显示最终玩家看到的 runtime world 与透明 HTML UI；顶部提供 Free Aspect、常用宽高比、固定分辨率、自定义分辨率、显示缩放、最大化/还原与 Maximize On Play。
...
- Play 模式进入时从 authoring 模型物化到运行时；Scene View 与 Game View 保留各自相机、presentation 和 Editor overlay，但必须读取同一个当前世界状态。Scene View 对 runtime entity Transform、Behaviour 字段和像素世界的编辑属于临时 Play 修改，立即反映到 Game View，退出 Play 时随临时快照回滚到编辑态。
...
Demo 不是「越简单越好」，而是**完整但聚焦**。它应该是一个可持续探索的开放式无限沙盒样本，用程序化内容证明引擎能力、编辑器工作流、UI runtime、打包发行和公开 API dogfood。

### 7.1 Demo 的定位

Demo 应承担四个职责：

1. **玩家视角**：它应该真的能玩，有开始、持续探索、反馈、死亡后重生、暂停、设置、重开、退出；不设置终局胜利条件。
...
Demo 可以有完整的探索与破坏 loop，但不应被内容量绑架。

可以做：

- 一个由 seed 驱动、按 chunk 动态生成的无限世界；
- 几种足以覆盖引擎能力的武器 / 工具；
- 可持续出现但不强制完成的材质、环境危险与地貌展示点；
- 一套完整 UI；
- 充分的反馈与打磨。
...
- 这个游戏能不能完整开始、持续向任意水平方向探索，并在死亡或重开后回到可玩状态？
