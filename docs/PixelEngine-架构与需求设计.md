...
> 版本：v2.4（Showcase Demo 转为 Noita 高保真复刻，Engine Core 继续保持通用）
> 作者：Lead Architect
...
本项目是一个自研 2D 像素游戏引擎，其 **WORLD（世界模拟）层对标 Noita（Nolla Games 的 "Falling Everything" 引擎）**。引擎必须复刻 Noita 的世界技术核心：每个屏幕像素都是一个被独立模拟、且可参与碰撞的物质单元（material cell），具体包括 falling-sand 细胞自动机（cellular automata, CA）、自由粒子（free particles）、像素级精确碰撞（pixel-perfect collision）、细胞生命周期（cell lifecycle）、以及材质反应（material reactions）。产品交付物由四个面组成：可复用引擎内核、面向资深 Unity 用户的 Unity-like Editor、面向玩家的 Web-first 透明 HTML UI Runtime，以及 Showcase Demo Game。Showcase Demo Game 在 `DEMO-006` 的流式无限沙盒地基上实现 Noita 高保真复刻战役：八区纵深主路径、Holy Mountain、Wand/Spell 构筑、敌人/经济、Kolmisilma、永久死亡与下一轮；同时保留可单独选择的无终点 Infinite Sandbox。

**引擎内核不复刻或硬编码 Noita 的法杖 / 法术内容**。复刻构筑以 Demo 层的 `Wand/Spell` 数据、预编译 catalog 和有界命令求值器实现，只通过 Scripting/Hosting 的公开世界效果、投射物、粒子、音频和 UI API 驱动引擎。引擎仍保持无玩法内核，材质 / 反应继续使用收敛的数据驱动表；若 Demo 需要通用能力，补的是可复用公开 API，而不是 Noita 专属类型或内部后门。完整产品边界见 `PixelEngine-原创Roguelite战役设计.md`（历史文件名）。
...
我们**不追求**的对标维度：bit 级可复现的实时模拟（Noita 实时 sim 本身就是非确定性的，见 §6）和真实 Navier-Stokes 流体（Noita 是 fake pressure，我们也是）。Demo 的地图 parity 需要复现 Wang tile + authored pixel scene 的可观察区域语法、主路径和秘密连接；实现可使用 PixelEngine 自己的确定性生成器，只要同 seed 初始 chunk 可复现、跨 chunk 连续，并通过参考路线截图/状态矩阵证明布局语义等价。
...
2026-07-18 本机固定稳态对照（详见 `docs/evidence-2026-07-18-perf-003-ca-throughput.md`）使用 2 launches × 15 warmups × 30 measured iterations 并按 baseline→optimized→baseline→optimized 交错运行；优化后为 10.603–11.664ms，保守按较慢轮折算约 1.486M active cells / 8ms。该进展仍比 2M 下限少约 25.69%，不改变本节目标，也不构成六 RID 代表硬件达标证据。
...
- Wand/Spell 求值在加载期把稳定字符串键编译为紧凑 runtime id，稳态使用固定容量 command buffer，禁止 LINQ、装箱、闭包和逐 cast 分配；所有世界写入延迟到 Hosting 规定的安全相位。
...
- 具体复刻流程、内容 parity 边界和任务顺序以 `PixelEngine-原创Roguelite战役设计.md` 与 `plan/tasks/` 为准。
...
2026-07-18 热路径收紧后的保守规模校准为约 5.383ns/active cell（2,166,784 cells / 11.664ms）；同配置两轮 optimized 为 10.603ms 与 11.664ms，两轮 baseline 为 12.303ms 与 14.029ms。主机噪声使本机结果不能代替目标硬件矩阵，且保守吞吐仍只有约 1.486M cells/8ms，因此 PERF-003 继续未闭合。

2026-07-18 独立初态校准修正了上述连续帧 workload 可能随 dirty 收缩的问题：每次计时推进 16 个互不共享、每轮重置为 full-dirty 的 2,166,784-cell kernel。干净基线 `5a98c988` 为 12.152ms，中心 target/local 直达优化 `e020f476` 为 11.634ms（约 5.369ns/active cell），两轮 99.9% CI 不重叠且均为 0 B；但 8ms 仍只有约 1.490M cells，低于 2M 下限 25.50%。该本机 win-x64 结果不能替代六 RID 代表硬件，因此 §12.8 数量级目标和 PERF-003 阻塞状态均不变。

2026-07-18 packed material lane 进一步把 CA update 同时消费的 type/density/dispersion/reaction/custom gate 派生为每材质 32-bit lane，EventPipe 确认逐 cell `ReactionCountOf` / `PropertyFlagsOf` frame 从 `ChunkUpdater` 热栈消失。可检出提交 `5355a5b9` 的 B4 为 10.809ms（约 4.988ns/active cell、0 B），按 mean 折算约 1.604M cells/8ms；但两 launch 分裂为 10.07/11.63ms、`mValue=3.67`，且 99.9% CI 与 11.344ms 干净基线小幅重叠。该轮只能证明机制进展和保守 target gap，不能把探索性 10.274ms 快值或本机双峰报告升级为 2M/8ms 达标；§12.8 目标与 PERF-003 阻塞状态不变。

2026-07-18 液体 movement 调用图进一步让 liquid 直接复用 powder helper，只在非零 dispersion 时进入 `NoInlining` 水平扫描；FullOpts 汇编将 `UpdateChunk` 控制在 3,254B，拒绝了会膨胀到 6,264B 的无门控内联方案。clean `9d9a9dc8` B6 为 10.253ms（约 4.732ns/active cell、0 B、`mValue=2`），约 1.691M cells/8ms；99.9% upper CI 口径约 1.665M。相对 10.284ms clean baseline 的 0.30% 差异 CI 未分离，因此只登记提交态能力和 JIT 机制，不把微小差异当统计结论；§12.8 目标与 PERF-003 阻塞状态不变。

2026-07-18 垂直 movement 调用图进一步把首格为空后的 32px 扫描隔离为 `NoInlining` helper；`TryMoveDown` FullOpts 从 3,936B 缩到 1,967B，独立 helper 为 1,972B，密集材质的首格碰撞路径不再携带 cold loop。clean `1eefa4a3` A8 与 `c18bc33d` B9 的 full-active 均值为 10.435/9.841ms，99.9% CI 10.287–10.584ms / 9.641–10.041ms 不重叠；8,192 个独立单帧 kernel 的 Typical Dirty 同时由 22.105us 降至 20.133us。B9 按 mean/upper CI 仍只有约 1.761M/1.726M cells/8ms，低于 2M 下限 11.93% / 13.68%，且本机不能替代六 RID 代表硬件，因此 §12.8 目标和 PERF-003 阻塞状态不变。

2026-07-18 列占用索引进一步为每个 64×64 chunk 增加 64 列×2 个 32-bit half 的派生位图，以 512B/chunk（0.125B/cell）把垂直空列的最多 31 次 strided Material 读取改为 mask + `TrailingZeroCount`；索引不入盘、不替代 CPU Material SoA 权威状态，raw alias 会失效并只在 active center/south 邻域重建。clean `5de61121` B13 为 9.266ms（约 4.276ns/active cell、0 B），相对同 fixture control 10.887ms 改善 14.89%，99.9% CI 9.106–9.426ms / 10.134–11.640ms 不重叠；Typical Dirty 同时由 20.061us 降至 14.506us。B13 按 mean/upper CI 仍只有约 1.871M/1.839M cells/8ms，低于 2M 下限 6.46% / 8.05%，且本机不能替代六 RID 代表硬件，因此 §12.8 目标和 PERF-003 阻塞状态不变。
