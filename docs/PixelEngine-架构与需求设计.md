...
热网格状态合计 ~5–7MB（不含 render buffer），加 render buffer ~17MB。**关键纪律：渲染颜色不存进 sim cell**；颜色由「材质纹理按世界坐标采样 + 温度 glow」在渲染相位生成，per-cell 零颜色存储 [高]。若需便宜噪声替代纹理，可加 1 字节 `colorVariant`。
