...
2026-07-10 本机正式规模校准（详见 `docs/evidence-2026-07-10-perf-003-ca-throughput.md`）新增 2,166,784-cell `FullActive2M`：Ryzen 7 5800X / .NET 10.0.8 / 4 worker 的 `StepJobSystem` 平均 16.575ms，折算约 1.046M active cells / 8ms；该结果仍低于 2M 下限，不改变本节目标，也不构成目标硬件达标证据。
...
2026-07-10 2M 规模校准：`FullActive2M` 平均约 7.65ns/active cell（2,166,784 cells / 16.575ms），扩大工作集后优于旧小夹具的线性换算，但仍不足以在 8ms 内完成 2M 更新；该差距保留为 PERF-003 与目标硬件证据的未闭合项。
